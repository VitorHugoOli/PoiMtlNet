# Session Handoff — 2026-04-27 (post-Phase-1 substrate validation + post-F49 architecture attribution)
...
> **Two complementary tracks both at paper-grade.** This handoff covers the substrate-side (Phase 1, 5-leg study Check2HGI vs HGI) AND the architecture-side (F49 3-way decomposition under H3-alt). They sit at different layers and are mutually reinforcing — Phase 1 says *which embedding* is necessary; F49 says *what the architecture does* with it. Together they describe the MTL win as **interactional architecture × substrate**, not transfer-driven.

## 0 · One-minute summary

### 0.1 · Phase 1 — substrate-side (this section)
...
- **Four paper-quality substrate findings landed today:**
...
- **Open Phase-2 work**: Phase 2 (FL + CA + TX substrate replication) is now authorised. See [`PHASE2_TRACKER.md`](PHASE2_TRACKER.md) for the launch queue. FL data is on disk and ready; CA + TX need upstream pipelines (F22/F23 in legacy tracker).

### 0.2 · F49 — architecture-side (parallel track)

- **F49 3-way λ=0 decomposition is COMPLETE on AL+AZ+FL n=5.** Under the H3-alt champion regime (`--scheduler constant --cat-lr 1e-3 --reg-lr 3e-3 --shared-lr 1e-3`), the H3-alt reg lift on AL is **purely architectural** (encoder-frozen λ=0 = 74.85 ± 2.38 vs STL F21c 68.37 ± 2.66 → +6.48 pp arch alone, ~2.7σ). Cat-supervision transfer is null on all 3 states (≤|0.75| pp). See [`research/F49_LAMBDA0_DECOMPOSITION_RESULTS.md`](research/F49_LAMBDA0_DECOMPOSITION_RESULTS.md). Plan in [`research/F49_LAMBDA0_DECOMPOSITION_GAP.md`](research/F49_LAMBDA0_DECOMPOSITION_GAP.md).

- **Two F49 paper-grade findings:**
  1. **Layer 1 — cat-supervision transfer is small.** ≤ |0.75| pp on AL/AZ/FL n=5 (AL −0.32, AZ +0.75, FL −0.52). **Refutes the legacy "+14.2 pp transfer at FL" claim** from `archive/research_pre_b5/CHAIN_FINDINGS_2026-04-20.md` by ≥9σ on FL alone, ≥18σ aggregate. Conventional MTL "cat→reg transfer" framing is empirically dead. The H3-alt reg lift is *not* transfer-driven.
  2. **Layer 2 (methodological) — loss-side `task_weight=0` ablation is unsound under cross-attention MTL.** The silenced task's encoder still co-adapts via attention K/V projections (`cross_ba(Q=b, K=a, V=a)` in `_CrossAttnBlock.forward`). Encoder-frozen isolation is the only clean architectural decomposition. Verified by 4 passing regression tests in `tests/test_regression/test_mtlnet_crossattn_lambda0_gradflow.py`.

- **Per-state mechanism patterns (paper-grade for AL/AZ; FL absolute Δ vs STL pending F37):**

| State | (frozen − STL) **arch** | (loss − frozen) **co-adapt** | (Full − loss) **transfer** |
|---|---:|---:|---:|
| AL (5f, ~2.7σ) | **+6.48 pp** ✓ | +0.09 (null) | −0.32 (null) |
| AZ (5f, ~3.7σ) | **−6.02 pp** ✓ | +1.98 (positive) | +0.75 (small) |
| FL (5f, F49c) | TBD pending F37 | +8.27 (~0.68σ; direction matches AL/AZ) | −0.52 (~0.34σ null) |

- **F49b reproduction gate PASSED:** AL `static_weight λ=0 + max_lr=3e-3 + OneCycleLR + next_gru` 5f×50ep → `top10_acc_indist = 53.18 ± 4.56` vs legacy 52.27 ± 5.03 (Δ +0.91 pp at ~0.13σ; σ-tight). F49 infra reproduces the legacy protocol cleanly; the H3-alt-regime numbers are not artefacts of code drift.

- **CH18 sharpened, CH19 added** in `CLAIMS_AND_HYPOTHESES.md` (both Tier A); **CONCERNS C12 resolved**.

- **Open F49 work** (paper-blocking):
  - **F37 STL `next_getnext_hard` reg 5f on FL** (4050-assigned, separate workflow) — closes Layer 3 absolute architectural Δ on FL.
  - **Paired Wilcoxon p-values** on F49 cells per fold (~30 min on existing JSONs) — tightens AL/AZ "transfer ≈ 0" + "co-adapt ≈ 0" claims with formal stats.

### 0.3 · How the two tracks compose for the paper

- **Substrate (Phase 1):** "Check2HGI is the right embedding because it carries per-visit context that HGI cannot, and substituting HGI breaks MTL B3."
- **Architecture (F49):** "MTL B3's reg lift over STL is from the cross-attention architecture itself, not from cat-supervision transfer. Loss-side ablation under cross-attn is unsound — encoder-frozen is the only clean isolation."
- **Joint claim:** "The MTL win is interactional **architecture × substrate** — neither alone explains the lift; both are necessary."

Code changes landed: `--freeze-cat-stream` flag in `scripts/train.py` + `freeze_cat_stream: bool` in `ExperimentConfig` + freeze hook in `mtl_cv.py` + mandatory `requires_grad=False` filter in `setup_per_head_optimizer` (else AdamW weight_decay decays frozen weights silently) + 4 regression tests passing (gradient-flow + no-decay-after-step + structural).

Result JSONs: `results/check2hgi/{alabama,arizona}/mtlnet_lr1.0e-04_bs2048_ep50_2026042{7_1008,7_1019,7_1029,7_1049}/` for F49 AL+AZ; `results/check2hgi/florida/f49c_{lossside,frozen}_5f_2026042715/2026042718/` for F49c FL n=5; `results/check2hgi/alabama/mtlnet_lr1.0e-04_bs2048_ep50_20260427_1452/` for F49b reproduction gate.

New trackers added today (post-stash, from F49 work): `PAPER_PREP_TRACKER.md` (paper-deliverable tracker — committable claims, headline-blockers, doc-rewrites, risk register, submission checklist) and `review/2026-04-27_study_overview_and_forgotten_items.md` (P0–P7 audit).

### 0.4 · Phase-1 substrate findings (continued)
