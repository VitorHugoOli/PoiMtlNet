...
| 2026-04-29 22:10 | **#70 C7 closed.** `storage.py` stamps `aggregation_basis ∈ {joint_best, per_task_f1_best, per_metric_best}` on every aggregate; new `_collect_per_metric_best_performance` helper aggregates the third basis. 127/127 tracking + 36/36 integration tests green. Audit C-series fully closed. Commit `83cbf68`. |
| 2026-04-29 22:14 | **#39 D5 LAUNCHED.** Code: `mtl_cv.train_model` now logs `reg_encoder_l2norm`, `reg_encoder_drift_from_init`, `reg_encoder_step_drift` and `cat_encoder_*` per epoch (no-op when encoders absent). Runs: H3-alt + B9 paired 1f×50ep with per-fold log_T (`scripts/run_f50_d5_encoder_traj.sh`). Plot: `scripts/analysis/f50_d5_encoder_traj_plot.py`. Tests: 2 new in `test_mtl_cv_improvements.py`. |
| 2026-04-30 03:30 | **F51 multi-seed Tier 1 launched** — B9 + H3-alt at seeds {0, 1, 7, 100} (5f×50ep each, ~2.3 h on 4090). Existing seed=42 reused as reference. Full doc: `F51_MULTI_SEED_FINDINGS.md`. |
| 2026-04-30 05:25 | **🚨 Per-seed log_T BUG FOUND.** v1 sweep used `region_transition_log_fold{N}.pt` built at seed=42 against trainers running at seeds 0/1/7/100. ~80% of val users at non-42 seeds live in seed=42's fold-N TRAIN set → ~80% of val transitions leaked into the prior, inflating absolute reg by ~9 pp at every non-42 seed. Caught by env-B 1-fold smoke + 5-fold seed=42 reproduction (the 5-fold reproduction matched the handover bit-exactly; the smoke didn't, exposing a fold-split mismatch). v1 paired Δs (~+3.4 pp) survive due to uniform-leak property; absolutes are wrong. |
| 2026-04-30 05:40 | **Fix landed (3 files).** `scripts/compute_region_transition.py`: writes `region_transition_log_seed{S}_fold{N}.pt` (seed in filename). `src/training/runners/mtl_cv.py:854-905`: reads seed-tagged file using trainer's own seed; hard-fails on legacy/missing. `scripts/run_f51_multiseed_fl.sh`: builds per-seed log_T before each seed's runs, idempotent. Legacy seed=42 file migrated in-place to `region_transition_log_seed42_fold{N}.pt`. |
| 2026-04-30 08:18 | **🎉 F51 v2 (post-fix) DONE — paper claim DECISIVELY ROBUST.** 5/5 seeds give Δreg ≥ +3.34 pp at p=0.0312 each (5-of-5 folds positive within every seed). **Across-seed Δreg = +3.48 ± 0.12 pp.** Pooled paired Wilcoxon (5 seeds × 5 folds = 25): **p_reg = 2.98×10⁻⁸ (25/25 positive); p_cat = 1.33×10⁻⁵ (19/25 positive).** Cat reaches paper-grade significance once seeds pool. Absolute B9 reg σ_across_seeds = 0.11 pp — recipe is essentially deterministic in the partition-difficulty axis. Full doc: `F51_MULTI_SEED_FINDINGS.md`; structured JSON: `F51_multi_seed_results.json`. |
| 2026-04-30 16:33 | **F51 Tier 2 capacity sweep DONE.** 21 smokes (5f×30ep) across 7 capacity knobs: encoder_layer_size, num_encoder_layers, encoder_dropout, shared_layer_size, num_crossattn_blocks, num_crossattn_heads, crossattn_ffn_dim. **Result: B9 is locally optimal in 5/7 dimensions; no paper-grade promotion candidate.** Single Pareto-trade (`num_crossattn_blocks=3`: +0.75 pp Δreg / -2.62 pp Δcat) refines F52's "mixing is dead" claim to "mixing is dead at depth=2 (B9), small contribution at depth=3, breaks cat at depth=4". Three width-knobs catastrophically break cat without affecting reg (shared_layer_size=384/512, num_crossattn_blocks=4, crossattn_ffn_dim=1024) — adds a "cat width-stability cliff" mechanism to the F50 D5 encoder-saturation narrative. Full doc: `F51_TIER2_CAPACITY_FINDINGS.md`; structured JSON: `F51_tier2_results.json`. |
