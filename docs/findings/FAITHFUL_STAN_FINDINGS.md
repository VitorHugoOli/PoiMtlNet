...
- `train.py` — StratifiedGroupKFold(5, shuffle=True, seed=42) on (target_category, userid). AdamW(lr=1e-4, wd=0.01) + OneCycleLR(max_lr=3e-3) + grad-clip 1.0. 50 epochs, batch 2048. Best-epoch selection on val Acc@10. Output JSON to `docs/results/baselines/`.
