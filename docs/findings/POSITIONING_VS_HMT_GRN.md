...
**Borrowed baseline choice.** Per `docs/CONCERNS.md §C06`, `next_gru` (our region-head champion) is the literature-aligned choice because HMT-GRN's GRN is a GRU + graph message passing. We adopt the GRU family deliberately to keep comparability; the graph signal HMT-GRN injects per step is already baked into check2HGI's input embeddings at encode time (the check2HGI encoder is a hierarchical-graph-contrastive learner — see `docs/check2hgi_overview.tex`).
...
- `docs/CONCERNS.md §C06` — rationale for `next_gru` as region champion.
