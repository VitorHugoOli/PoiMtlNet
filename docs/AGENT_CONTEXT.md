...
> - See `CONCERNS.md` C21 (full diagnosis) and the **active follow-on study at [`docs/studies/mtl-protocol-fix/`](studies/mtl-protocol-fix/) (launched 2026-05-20)** for the closure path. The earlier `docs/studies/mtl-exploration/FUTUREWORK_substrate_aware_mtl_balancing.md` memo (F1/F2/F3) is superseded by the active study. Re-evaluation of existing Tier-1..6 candidates AND of the shipping baseline under a substrate-aware selector is a **zero-retraining** job via `scripts/canonical_improvement/analyze_t64_selectors.py` (reads per-epoch val CSVs already on disk).
