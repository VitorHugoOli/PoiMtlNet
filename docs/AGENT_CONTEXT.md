...
> - **`mtl_improvement` SUCCEEDED 2026-06-06** (branch `mtl-improve`) — champion **"G"**: a single MTL model BEATS both STL ceilings (reg+cat) at all 4 states, 4-seed (tradeoff INVERTED), via the C25 unweighting fix + dual-tower/`aux`/prior-OFF. Experimental work complete; only the BRACIS paper-doc restatement remains. See [`studies/mtl_improvement/CHAMPION.md`](studies/mtl_improvement/CHAMPION.md) + `CONCERNS.md §C25` + `CANONICAL_VERSIONS.md §v16`.
