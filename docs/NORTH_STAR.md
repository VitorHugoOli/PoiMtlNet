...
> 🔬 **2026-06-30 — CANDIDATE NEXT PIN (n=20-confirmed, promotion gated):** **bs=8192 + per-head cat-lr 1e-3**
> via `MTL_ONECYCLE_PER_HEAD_LR=1` beats the board champion at every tested state (AL +1.0 / AZ +2.3 cat; FL
> +0.17 cat / +0.20 reg AND ~7% faster). It was unlocked by fixing a latent bug: **the champion's documented
> per-head LRs (`cat 1e-3 / reg 3e-3 / shared 1e-3`) are INERT under `onecycle`** — a scalar `max_lr` broadcasts
> 3e-3 to every head, so the §0.1 champion effectively ran uniform 3e-3 (the ratios are decorative on the onecycle
> path; they DO apply on the older H3-alt constant/cosine path below). Full record:
> [`studies/closing_data/perhead_lr_n20.md`](studies/closing_data/perhead_lr_n20.md) +
> [`future_works/per_head_lr_onecycle_fix.md`](future_works/per_head_lr_onecycle_fix.md). The current board champion
> trains on the **`check2hgi_dk_ovl`** overlap engine ([`studies/closing_data/RESULTS_BOARD.md §1`](studies/closing_data/RESULTS_BOARD.md)).
> Promotion to §0.1 deferred (needs CA/TX + the flag-OFF eager-parity test).
