...
| Florida / California / Texas | — | — | — | FL = 2/5 partial (PR #47), CA/TX dropped — see `../../studies/closing_data/RESULTS_BOARD.md` §4 |
...
- **`e2e` note:** native CTLE-E2E (A40, seed 0 × 5f, leak-clean per-fold; `scripts/baselines/ctle_e2e.py`, JSONs `docs/results/closing_data/baseline_compare/{alabama,florida}_ctle_e2e_seed0.json`): **AL cat 21.14 (final-epoch) / 23.94 (best-epoch); FL cat 29.69 (final) / 33.45 (best)**. We report BOTH the final-epoch number (the original CTLE-E2E rule — AL 21.14 reproduces the prior ~21.2; FL 29.69 reproduces the prior unbacked 29.65 to ±0.04) AND the best-epoch number (matches the board cat-ceiling scorer `score_stl_cat_ceiling.py`). Even at its *best* epoch, CTLE-E2E (AL 23.9 / FL 33.4) sits **far below** Check2HGI-cat (AL ≈ 55.6 / FL ≈ 73–75): E2E > frozen CTLE-SC 17.75 is a *within-CTLE* frozen-vs-fine-tuned effect (CTLE never ingests our vectors in either setup), so it only shows "frozen-SC undersells deep models" and justifies running E2E. It is **not** evidence that a baseline benefits from our substrate (do not claim that). Frame: "even in its best E2E form CTLE is well below ours" — never "we crushed CTLE".
