...
- **Seed-matched ceilings (the #1 gate):** the (c)/(d) STL ceilings G beats were re-run at G's own seeds {0,1,7,100} (they were seed-42 only). The ceilings are stable (σ≤0.7) and **G still beats both at all 4 states** (AL +1.80reg/+2.56cat, AZ +0.95/+4.09, GE +0.93/+3.93, FL +0.30/+3.20) — the feared "lucky seed-42 ceiling" did not materialize.
...
**Numbers to trust:** `studies/mtl_improvement/{INDEX.html #tier2v, log.md 2026-06-06/07, CRITIQUE_TIER2_C25_2026-06-06.md §7, CHAMPION.md}`. **G remains a STUDY champion, NOT yet paper §0 canon** — the BRACIS restatement is the lone open author item.
...
The study launched 2026-05-28 (entry below) is now closed. Closure synthesis: [`studies/substrate-protocol-cleanup/CLOSURE.md`](studies/substrate-protocol-cleanup/CLOSURE.md). Five Tiers landed; **one promotion** (Tier A1), the rest nulls/closures. ~10–11 GPU-h total (well under the ~40–45 GPU-h budget). Small-states only (AL/AZ) for paper-grade; FL/CA/TX as pilots only.
...
- **Tier D1 — window/causal-mask audit CLEAN** (no leak; shared artefact with `mtl_improvement` T0.2). Surfaced C24 (STAN bidirectional watch-item) + the Tier C4 mtime guard recommendation. Source: [`studies/substrate-protocol-cleanup/window_mask_audit.md`](studies/substrate-protocol-cleanup/window_mask_audit.md).
