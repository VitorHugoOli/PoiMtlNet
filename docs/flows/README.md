...
| `MTL_TRAIN_DIAGNOSTICS` | **auto: `1` on `--profile`/`MTL_PROFILE=1`, else `0`** | P4 (pipeline_audit 2026-07-01, default flipped same day): the batch-0 grad-cosine diagnostic (2 extra backwards/epoch + host syncs; forces `donated_buffer=False` under `--compile`) is now OFF for bare runs (~9% wall at AL) and rides with the profiler. Explicit `=1`/`=0` wins (science runs can enable it without profiling; profiled runs can disable it for production-exact timing). **Byte-identical training either way** (parity-verified); `grad_cosine_shared` logs NaN when off. Legacy `MTL_NO_TRAIN_DIAGNOSTICS=1` still forces off. |
