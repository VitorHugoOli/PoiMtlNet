...
- The **other 5-6 head sweep candidates** (next_lstm, next_transformer_pf, next_getnext, next_stan_baseline, next_gru-as-reg) are absorbed into [`docs/studies/mtl_improvement/`](../studies/mtl_improvement/) (corrected 2026-06-03 — the old "T7" pointer is stale; the plan numbers heads as T5): they run leak-free in **Tier S / S.1** (STL-alone deep search) and feed **T5.1** (head re-ablation under the consolidated MTL backbone); the α-prior + tail-loss tuning lands in **T1.4** (bounded STL ceiling-hardening). This memo is the narrower precursor; once those land it can be archived.
