...
| **C25** | **⭐ MTL reg trained on CLASS-WEIGHTED CE vs UNWEIGHTED STL ceiling → MTL→STL reg gap is largely an objective-mismatch confound (`default_mtl use_class_weights=True`)** | **re-validated + CLOSED 2026-06-05** | AL/GE/FL re-baseline + per-task fix + regime re-test + all follow-ups (v15 pin, Acc@10 monitor, §0.1 FL A/B +3.15reg/+3.52cat, MoE/SwiGLU stretch); only paper-doc restatement remains |
...
**The bug.** `ExperimentConfig.default_mtl` silently sets `use_class_weights=True` (`src/configs/experiment.py:364`; dataclass default `:235` also `True`). This flows to `src/training/runners/mtl_cv.py:1283-1291`, where the **MTL reg criterion** becomes `CrossEntropyLoss(weight=alpha_next)`. The STL reg **ceiling (c)** is set by `scripts/p1_region_head_ablation.py`, which uses **unweighted** `CrossEntropyLoss` (`build_calibrated_loss` with no calibration, `:549`). **NB (re-audit correction 2026-06-05):** `default_next` (`:444`) is ALSO `use_class_weights=True` (only `default_category` `:403` is `False`) — so an STL reg run via `train.py --task next` would ALSO class-weight, a separate latent inconsistency; but the (c) ceiling itself was p1/unweighted, so the MTL-vs-(c) comparison is the weighted-vs-unweighted mismatch. The MTL reg criterion is with `alpha_next = compute_class_weights(...)` (`:1276`). So the ~1109/4702-region MTL reg head trains on **class-BALANCED CE**, whereas the STL reg ceiling (p1 / `default_next`) and every clean reconstruction train on **UNWEIGHTED CE**.
...
**Pragmatic resolution (in flight, user-approved 2026-06-05).** (1) per-task class-weighting code fix — **DONE**: `default_mtl` now sets **BOTH heads UNWEIGHTED** (`use_class_weights_{reg,cat}=False`). reg-unweighted matches Acc@10 + the STL ceiling; cat-unweighted was EMPIRICALLY validated (AL real-joint: +5.14pp cat macro-F1 48.37→53.51; the "balancing helps macro-F1" assumption was TESTED and FALSE — user-requested). CLI `--[no-]reg-class-weights` / `--[no-]cat-class-weights` decouple them; `--reg-class-weights --cat-class-weights` recovers pre-C25; (2) re-test the regime finding + a real joint run (`category-weight 0.75`) + §0.1/composite re-baseline under unweighted reg CE at **AL/GE/FL** (CA/TX deferred). **Do NOT cite any absolute MTL-reg number until re-baselined.** Frozen (c)/(d) STL ceilings are UNAFFECTED (unweighted).

**Second, smaller artifact (logged here, separate) — FIXED 2026-06-05:** the MTL reg deployment SNAPSHOT was selected by `MultiTaskBestTracker.reg_best` monitor = `accuracy` (Acc@1), whose best epoch ≠ the Acc@10-best epoch → understated the *deployable* reg ~2-3pp. Fixed: `PrimaryMetric.TOP10` added + `CHECK2HGI_NEXT_REGION.task_b.primary_metric` → TOP10 (`src/tasks/{registry,presets}.py`). Independent of the disjoint metric (`per_metric_best.top10_acc_indist`, oracle-Acc@10, selector-independent), which was always correct.

**§0.1 continuity A/B (2026-06-05, FL).** A same-harness control (exact §0.1 B9 recipe, canon GCN substrate, seeds {0,1,7,100}, **only the class-weight flag toggled**) quantifies the confound directly on the paper-canon row: MTL reg **63.91 → 67.06 (+3.15)**, MTL cat **70.34 → 73.86 (+3.52)**; §0.1 architectural Δ_reg **−6.71 → −3.56** (~halved). A separate **harness-drift offset of +0.64 reg / +1.78 cat** vs the *published* §0.1 (63.27/68.56) means re-reporting must use a freshly-measured weighted baseline, not the published numbers. See [`results/RESULTS_TABLE.md §0.1`](results/RESULTS_TABLE.md) continuity annotation. AL/AZ/CA/TX not yet B9-canon A/B'd (effect scales with class count → FL is a lower bound for CA/TX).

**Status:** `re-validated + CLOSED 2026-06-05` (fix committed; multi-seed AL/GE/FL done; all follow-ups landed). The per-task-weighting fix is committed; the AL/GE/FL re-baseline {0,1,7,100} under unweighted reg CE is DONE; the regime finding is re-tested. **Outcome: the confound's downstream claims are OVERTURNED/DISSOLVED** — regime finding (CH28) overturned (substrate transfers, Δreg(v14−canon) +1.92/+1.49/+0.81), composite (CH25) dissolved (single MTL ≥ composite), Tier-2 architecture-negative overturned (dual-tower closes the gap), §0.1 MTL reg +10-13pp, MTL cat above ceiling. **Follow-ups ALL CLOSED:** NORTH_STAR + CANONICAL_VERSIONS v15/v16 pinned; Acc@1→Acc@10 reg-monitor fixed (above); FL-B9 §0.1 continuity A/B done (above); Tier-2 stretch (MoE + SwiGLU) confirms the residual is the α·log_T prior / shared-pathway, not architecture; champion **G** (v16) found + multi-state confirmed. **HARDENED 2026-06-07 (Tier 2V — `studies/mtl_improvement/CRITIQUE_TIER2_C25_2026-06-06.md` §7):** the unweighting fix + champion survived a skeptical critique close-out — seed-matched ceilings (G beats 4/4), FAIR alt-arch re-rank (all lose 1.6–2.1pp, un-confounded), no tail regression, no hypertuning lever beats G (logit-adjust HURTS the MTL cat). Full numbers: `docs/studies/mtl_improvement/{log.md,HANDOFF.md,PAPER_UPDATE.md,INDEX.html #tier2v}` 2026-06-05/06/07. **Only paper-doc restatement remains (author decision).**
...
## C26 — An FL-only result (G′) was over-propagated as a multi-state champion before the scale axis was covered (PROCESS LESSON 2026-06-07)

**What happened.** The B-A3 cat-private ablation produced a variant **G′** (`mtlnet_crossattn_dualtower_catpriv`) that showed a robust **+1.61 cat gain at FL** (4-seed, σ 0.04). On that single-state evidence it was propagated as the **"new FL champion"** to 5 claim docs (CHAMPION/NORTH_STAR/CANONICAL_VERSIONS/CLAIMS/HANDOFF) **before any multi-state confirm**. The confirm (AL/AZ/GE × {0,1,7,100}) then **FALSIFIED** it — cat craters −3.6 to −15.3 pp at the small states (AL/AZ even fall below the STL cat ceiling) — and the propagation had to be REVERTED.

**Two lessons.**
1. **A single-state Pareto gain is NOT a champion until the scale axis is covered.** Promotions deserve the same multi-state discipline as falsifications: confirm at ≥1 small + the development-large state (and ideally the GE middle band) *before* touching the claim docs. The "don't ship an FL-only result" guard applies to *promotions*, not only to negative results.
2. **The first mechanism read can be wrong — cite the evidence before naming the mechanism.** The collapse was initially diagnosed "overfits at small data scale" (plausible-sounding, written without looking). The train-F1 trajectory showed the opposite — train-F1 caps at 0.45 (vs the GRU head's 0.98) = **underfit**; a rescue screen then refined it further to a **head↔task-cardinality mismatch** (the off-label STAN flow/attention head can't learn a 7-class target at small data, and the regularization that yields the FL gain is exactly what starves small states). Name a mechanism only after reading the curve/ablation that distinguishes it.

**Resolution / status:** `CLOSED 2026-06-07`. G′ DEMOTED to an FL-only experimental dead-end; **G (cat-SHARED `next_gru`) remains the multi-state champion**; the rescue screen found no recoverable config (the B-A3/G′ line is closed). No paper exposure (G′ was never paper canon; a do-not-cite guard was added to `PAPER_UPDATE.md`). Trail: `studies/mtl_improvement/{log.md 2026-06-07, CHAMPION.md §G′, INDEX.html #T2V-5}`; `CHANGELOG.md` 2026-06-07; drivers `gprime_{multistate,rescue_screen}.sh`.

---

## C27 — MTL gradient-balancers are mis-wired / mis-defaulted under the dual-tower; a latent preflight bug omitted two of them (DISCOVERED + FIXED 2026-06-08, `mtl_improvement` Tier-4 audit)

**Context.** The Tier-4 balancer sweep (full `src/losses` registry under champion G) showed every advanced balancer clustered at the equal-weight point and none beat tuned `static_weight`. That clustering was flagged as suspicious and audited (2 sub-agents + live probes). Two real issues were found; neither changes the Tier-4 verdict (see "Why it doesn't move the verdict"), but both are recorded for correctness.

**Issue 1 — gradient-surgery balancers only reweight the SHARED params; under G's dual-tower the private reg tower (which carries ~all of reg) trains at UNIT weight.** `cagrad`/`pcgrad`/`aligned_mtl` compute their surgical/combined gradient over `shared_parameters()` and set `p.grad` there, but task-specific params get `autograd.grad(losses.sum(), ts_params)` = unit weight on every task. G's reg signal lives in the *private* STAN tower (∈ `reg_specific`, OUTSIDE shared), so the surgery never touches it → these methods silently collapse to ≈equal-weighting (live probe: cagrad logs weights `[1,0]` yet its private-tower gradient == `equal_weight`'s). **→ cagrad/pcgrad/aligned_mtl cells do NOT count as balancer tests under a dual-tower head.** (gradnorm/dwa/fairgrad are additionally *misconfigured at defaults* — gradnorm lr=1e-3 + L1-renorm-to-sum=2 centers it on equal; dwa uses per-batch loss history; fairgrad step_size too small.)

**Issue 2 (code bug, FIXED) — `_BACKWARD_ONLY_LOSSES` preflight in `scripts/train.py` omitted `cagrad` + `aligned_mtl`.** These backprop internally / return `loss=None`, so under `gradient_accumulation_steps>1` they would hit a `TypeError` at `src/training/runners/mtl_cv.py` instead of a clean exit. Safe in practice only because `default_mtl` pins `grad_accum=1`. **Fixed 2026-06-08** (`scripts/train.py` `_BACKWARD_ONLY_LOSSES` now includes both; dated comment in place).

**Why it doesn't move the Tier-4 verdict.** The mechanism is decisive independently: **gradient cosine(cat,reg)≈0** at both states (FL +0.0007 / AL +0.0026) → there is NO task conflict for any balancer to resolve, so even correctly-wired surgery cannot help. Corrected re-runs (gradnorm @lr=0.05, nash @max_norm=2.2) + the static cw-sweep confirm no Pareto win, matching the literature (Kurin/Xin NeurIPS'22).

**Resolution / status:** `Issue 2 FIXED; Issue 1 documented (a real limitation, moot here by cos≈0).** If a future study uses a *conflicting* task pair (negative cosine), the surgery methods' task-specific-gradient handling MUST be fixed first (apply per-task weights to task-specific params too) before their results count under a dual-tower head. Trail: `studies/mtl_improvement/{log.md 2026-06-08, INDEX.html #tier4}`; `results/mtl_improvement/T4_audit_and_verdict.md`; `findings/MTL_FLAWS_AND_FIXES.md §5.6`.

## C28 — Aux-gate dead codepath on the dual-tower + a rundir-capture race contaminated a multi-seed cell (DISCOVERED + FIXED 2026-06-12, `mtl_improvement` HANDOFF_AUDIT)

**Context.** A pre-closure deep code-audit (`studies/mtl_improvement/CODE_AUDIT_2026-06-12.md`) of the champion-G MTL path surfaced two C25/C27-class silent flaws, both resolved during the audit punch-list (`HANDOFF_AUDIT.md`).

**Issue 1 — `next_stan_flow_dualtower` was missing from `_HEADS_REQUIRING_AUX_MTL` (`src/data/folds.py:933-937`) → the α·log_T prior AND log_T-KD were structurally DEAD on the dual-tower.** `get_current_aux()` returned `None` in every forward, so `_apply_prior` always took the defensive `logits + α·0.0` branch and the trainer's KD branch (requires `_aux is not None`) was a no-op. Consequences: every "prior-ON" dual-tower arm ever run was actually prior-OFF, and the `c25_gv2.sh` `g_kd0.1/0.2` arms were no-ops → CHAMPION §5's old "KD adds nothing on the dual-tower" was a **dead-codepath artifact, not a real test.** **G's own numbers are unaffected** (G pins prior-OFF + KD 0.0 → numerically identical either way). **FIXED 2026-06-12:** head added to both gate sets (`folds.py` + `scripts/p1_region_head_ablation.py`); the now-real KD-on-G test (HANDOFF_AUDIT X2) is **NULL** (FL reg +0.05 / AL −0.13 ≪ 0.3pp) → the "KD adds nothing" verdict now stands on a real test.

**Issue 2 — a `ls -dt | head -1` rundir-capture race mis-mapped a multi-seed cell** (the same trap `HANDOFF_TIER5.md §4` + memory `ref_concurrent_rundir_race` warn about). The cat-transfer manifest's FL `s1/s7/s100` rows all pointed to ONE rundir — which was not even a cat-transfer run but the FL *fully-shared* intrinsic-test run captured concurrently. FL cat-transfer at {1,7,100} had never run; the published FL decomposition (72.09 / arch +2.13 / transfer +1.08) counted one wrong run ×3. **FIXED 2026-06-12:** re-ran genuine reg-OFF at {1,7,100} with **PID-suffix rundir capture** (`cat_transfer_fl_reseed.sh`); true FL cat+trunk **72.24 ± 0.03**, transfer **+0.93** (sign held). Lesson re-affirmed: never capture rundirs via `ls -dt|head` under any concurrency — always `..._${pid}`.
...
**Resolution / status:** `Both issues FIXED 2026-06-12; champion G unchanged; study CLOSED.` Guards: `tests/test_regression/test_mtl_param_partition.py` (dualtower family + PCGrad coverage), `test_cli_param_coercion.py`. Trail: `studies/mtl_improvement/{CODE_AUDIT_2026-06-12.md, HANDOFF_AUDIT.md, log.md 2026-06-12, INDEX.html #tier7}`; `results/mtl_improvement/X_SERIES_FINDINGS.md`.
