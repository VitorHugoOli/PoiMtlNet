...
**Status:** `re-validated 2026-06-05` (fix committed; multi-seed AL/GE/FL done). The per-task-weighting fix is committed; the AL/GE/FL re-baseline {0,1,7,100} under unweighted reg CE is DONE; the regime finding is re-tested. **Outcome: the confound's downstream claims are OVERTURNED/DISSOLVED** — regime finding (CH28) overturned (substrate transfers, Δreg(v14−canon) +1.92/+1.49/+0.81), composite (CH25) dissolved (single MTL ≥ composite), Tier-2 architecture-negative overturned (dual-tower closes the gap), §0.1 MTL reg +10-13pp, MTL cat above ceiling. Remaining to fully close: NORTH_STAR/CANONICAL pin of the unweighted recipe + the Acc@1→Acc@10 reg-monitor code fix + FL-B9 §0.1-continuity follow-up. Full re-validated numbers: `docs/studies/mtl_improvement/{log.md,HANDOFF.md,PAPER_UPDATE.md}` 2026-06-05.
