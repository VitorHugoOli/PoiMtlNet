...
**Reference:** `docs/studies/archive/substrate-protocol-cleanup/window_mask_audit.md` + log.md 2026-05-28 advisor entry.
...
**Status:** `re-validated + CLOSED 2026-06-05` (fix committed; multi-seed AL/GE/FL done; all follow-ups landed). The per-task-weighting fix is committed; the AL/GE/FL re-baseline {0,1,7,100} under unweighted reg CE is DONE; the regime finding is re-tested. **Outcome: the confound's downstream claims are OVERTURNED/DISSOLVED** — regime finding (CH28) overturned (substrate transfers, Δreg(v14−canon) +1.92/+1.49/+0.81), composite (CH25) dissolved (single MTL ≥ composite), Tier-2 architecture-negative overturned (dual-tower closes the gap), §0.1 MTL reg +10-13pp, MTL cat above ceiling. **Follow-ups ALL CLOSED:** NORTH_STAR + CANONICAL_VERSIONS v15/v16 pinned; Acc@1→Acc@10 reg-monitor fixed (above); FL-B9 §0.1 continuity A/B done (above); Tier-2 stretch (MoE + SwiGLU) confirms the residual is the α·log_T prior / shared-pathway, not architecture; champion **G** (v16) found + multi-state confirmed. **HARDENED 2026-06-07 (Tier 2V — `studies/archive/mtl_improvement/CRITIQUE_TIER2_C25_2026-06-06.md` §7):** the unweighting fix + champion survived a skeptical critique close-out — seed-matched ceilings (G beats 4/4), FAIR alt-arch re-rank (all lose 1.6–2.1pp, un-confounded), no tail regression, no hypertuning lever beats G (logit-adjust HURTS the MTL cat). Full numbers: `docs/studies/archive/mtl_improvement/{log.md,HANDOFF.md,PAPER_UPDATE.md,INDEX.html #tier2v}` 2026-06-05/06/07. **Only paper-doc restatement remains (author decision).**

**Reference:** `docs/studies/archive/mtl_improvement/log.md` 2026-06-05 ROOT-CAUSE entry; `HANDOFF.md` §top; `PAPER_UPDATE.md` superseding banner. Scripts: `scripts/mtl_improvement/t2p0_*`.
...
**Resolution / status:** `CLOSED 2026-06-07`. G′ DEMOTED to an FL-only experimental dead-end; **G (cat-SHARED `next_gru`) remains the multi-state champion**; the rescue screen found no recoverable config (the B-A3/G′ line is closed). No paper exposure (G′ was never paper canon; a do-not-cite guard was added to `PAPER_UPDATE.md`). Trail: `studies/archive/mtl_improvement/{log.md 2026-06-07, CHAMPION.md §G′, INDEX.html #T2V-5}`; `CHANGELOG.md` 2026-06-07; drivers `gprime_{multistate,rescue_screen}.sh`.
...
**Resolution / status:** `Issue 2 FIXED; Issue 1 documented (a real limitation, moot here by cos≈0).** If a future study uses a *conflicting* task pair (negative cosine), the surgery methods' task-specific-gradient handling MUST be fixed first (apply per-task weights to task-specific params too) before their results count under a dual-tower head. Trail: `studies/archive/mtl_improvement/{log.md 2026-06-08, INDEX.html #tier4}`; `results/mtl_improvement/T4_audit_and_verdict.md`; `findings/MTL_FLAWS_AND_FIXES.md §5.6`.
...
**Context.** A pre-closure deep code-audit (`studies/archive/mtl_improvement/CODE_AUDIT_2026-06-12.md`) of the champion-G MTL path surfaced two C25/C27-class silent flaws, both resolved during the audit punch-list (`HANDOFF_AUDIT.md`).
