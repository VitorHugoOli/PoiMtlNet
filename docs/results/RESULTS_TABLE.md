...
> - [`docs/studies/archive/mtl_improvement/`](../studies/archive/mtl_improvement/) (LAUNCHED 2026-05-16, branch `mtl-improve`) — full T0-T8 architectural chain (backbones, loss, batch, LR, α, heads, multi-seed champion).
> - [`docs/studies/archive/substrate-protocol-cleanup/`](../studies/archive/substrate-protocol-cleanup/) (LAUNCHED 2026-05-28, main) — Tier A-D substrate + protocol cleanup.
>
...
> ⚠ **2026-06-04 — `onecycle` small-state recipe finding (mtl_improvement Tier 2; ANNOTATION, table above NOT rewritten — author sign-off needed).** The §0.1 table reports **MTL B9** at all states, but the *shipped* small-state recipe is **H3-alt** (§0.4) — and B9's alt-SGD **tanks small-state cat**, so the AL/AZ B9 cat numbers above understate the deployable cat. The mtl_improvement track found **`onecycle` (aggressive schedule, NO alt-opt) dominates H3-alt and beats B9 at small states** (v14 multi-seed +6–9pp reg / +1–2pp cat vs H3-alt). **Paper-substrate (v11, AL/AZ {0,1,7,100}, diagnostic-best) under onecycle:** AL reg 53.15 (+2.98 vs B9) / cat 47.93 (+7.36); AZ reg 41.54 (+0.76) / cat 49.79 (+4.69). **Implied small-state arch-Δ under the deployable recipe:** AL reg **−8.06** (was −11.04) / cat **+6.58** (was −0.78); AZ reg **−11.52** (was −12.28) / cat **+5.89** (was +1.20). **Nuance:** the reg shrink is *modest* on v11 (the big +6-9pp is a v14-base effect); the large cat "flip" is *mostly* a B9→deployable-recipe fix (vs H3-alt, onecycle cat is only ~+1pp). **Large states keep B9** (onecycle doesn't dominate at FL/CA). Adopt at AL/AZ; re-state the small-state arch-Δ under the deployable recipe so cat isn't understated — but this reshapes a central §0.1 small-state claim → **author decision required.** Provenance: [`../studies/archive/mtl_improvement/PAPER_UPDATE.md`](../studies/archive/mtl_improvement/PAPER_UPDATE.md) + [`mtl_improvement/T21_recipe_matrix.md`](mtl_improvement/T21_recipe_matrix.md).

> ⚠⚠ **2026-06-05 — C25 class-weighting fix → §0.1 FL continuity (mtl_improvement; ANNOTATION, table above NOT rewritten — author sign-off needed).** The §0.1 MTL B9 numbers were measured with the reg+cat heads on **class-WEIGHTED CrossEntropy**, while the reported metrics (reg `top10_acc_indist` = Acc@10; cat macro-F1) and the STL ceiling are unweighted — an objective-vs-metric mismatch (`default_mtl use_class_weights=True`; root cause [`../CONCERNS.md §C25`](../CONCERNS.md)). Fixed to **per-task UNWEIGHTED by default**. **Same-harness A/B at FL** (exact §0.1 B9 recipe, canon GCN substrate, seeds {0,1,7,100}, **only the class-weight flag toggled**): MTL reg **63.91 → 67.06 (+3.15)**, MTL cat **70.34 → 73.86 (+3.52)**. The §0.1 architectural gap shrinks: **Δ_reg −6.71 → −3.56 (vs STL 70.62, ~halved)**, **Δ_cat +3.18 → +6.70 (~doubled)**. The published §0.1 FL row (reg 63.27 / cat 68.56) carries an additional **harness-drift offset of +0.64 reg / +1.78 cat** vs the same-harness weighted baseline (63.91 / 70.34) — so re-reporting §0.1 should use a freshly-measured weighted baseline, not subtract from the published numbers. Other states (AL/AZ/CA/TX) NOT yet A/B'd on the B9-canon recipe — the unweighting effect scales with class count, so the FL magnitude is a lower bound for CA/TX. **Reproduce the pre-fix (weighted) numbers:** `--reg-class-weights --cat-class-weights`. Provenance: [`../studies/archive/mtl_improvement/log.md`](../studies/archive/mtl_improvement/log.md) (2026-06-05 "STRETCH CLOSED") + [`../studies/archive/mtl_improvement/HANDOFF.md`](../studies/archive/mtl_improvement/HANDOFF.md) (C25 finding) + drivers `scripts/mtl_improvement/c25_fl_b9_{continuity,weighted}.sh`.
