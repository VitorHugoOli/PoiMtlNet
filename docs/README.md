...
> 🔬 **2026-05-30 — Default flip v11→v12 (read before citing v11 numbers).** [`studies/archive/substrate-protocol-cleanup/`](studies/archive/substrate-protocol-cleanup/) is **CLOSED 2026-05-29** and produced the new **v12 default** = v11 recipe + **log_T-KD W=0.2** (deployable reg lift, paper-grade AL/AZ / pilot FL/CA/TX) + **ResLN encoder** (STL-best; no MTL benefit). A bare `train.py` run now produces v12; the **BRACIS paper cites v11**, reproducible with `--log-t-kd-weight 0.0 --encoder gcn` (the frozen `output/check2hgi/` substrate). Full version registry: [`results/CANONICAL_VERSIONS.md`](results/CANONICAL_VERSIONS.md). [`studies/archive/mtl_improvement/`](studies/archive/mtl_improvement/) (architectural axis, branch `mtl-improve`) is **CLOSED 2026-06-12** — champion **G = v16** is now the `train.py --task mtl` default; read [`studies/archive/mtl_improvement/FINAL_SYNTHESIS.md`](studies/archive/mtl_improvement/FINAL_SYNTHESIS.md). Other v11 caveats: F1 selector fix +5.6 pp FL multi-seed (C21 RESOLVED); composite STL ceiling +7-12 pp (Phase 3 §4.2). See [`results/RESULTS_TABLE.md`](results/RESULTS_TABLE.md) top banner.
...
- [`studies/`](studies/) — research tracks layered on check2hgi (active + closed). Cross-study outcomes log at [`studies/log.md`](studies/log.md).
  - **⭐ CLOSED 2026-06-12** [`studies/archive/mtl_improvement/`](studies/archive/mtl_improvement/) — architectural axis, branch `mtl-improve`. **One-stop closure doc (read FIRST, incl. the corrections-and-retractions registry): [`FINAL_SYNTHESIS.md`](studies/archive/mtl_improvement/FINAL_SYNTHESIS.md)**; final matched-metric bar = R0 (Δreg −0.09…−0.31 "matches", Δcat +2.56…+4.08); survived 3 audit passes + the X-series (all null); CA/TX + the aligned-pairing pre-freeze gate → `closing_data/`. **Outcome: champion "G" — a SINGLE MTL model that MATCHES the STL reg ceiling (Pareto-non-inferior) AND substantially BEATS the cat ceiling (+3pp) at all 4 available states, 4-seed** (the MTL tradeoff is Pareto-POSITIVE). ⚠ reg verb CORRECTED 2026-06-07 — B-A2 (the critique's independent re-eval) found the earlier "beats reg" was an indist-vs-full metric mismatch; on a matched metric G is ~0.35pp BELOW the reg ceiling (FL 72.93 vs 73.31) → reg = "matches". Enabled by the **C25 class-weighting fix** (`CONCERNS.md §C25`) + the reg-private **dual-tower + `aux` fusion + prior-OFF**. Architecture capacity is NOT the lever (falsified 5 ways; the STAN private tower is load-bearing). Config + runnable command: [`studies/archive/mtl_improvement/CHAMPION.md`](studies/archive/mtl_improvement/CHAMPION.md); pin: [`results/CANONICAL_VERSIONS.md §v16`](results/CANONICAL_VERSIONS.md). **Study champion, NOT yet paper §0 canon** — the BRACIS restatement is the lone open (author) item. (A cat-private variant **G′** was tried 2026-06-07 and **CLOSED as an FL-only dead-end** — cat craters −3.6…−15.3pp at small states, no rescue config; the champion cat head stays the **shared `next_gru`**. See `CHAMPION.md §G′` / `CONCERNS §C26`.)
  - **SCAFFOLDED (not launched) 2026-06-12** [`studies/closing_data/`](studies/closing_data/) — the experimental engine for the NEW paper: cross-study re-eval + BRACIS-suite inventory (RUN_MATRIX) → pre-freeze gates (G0.1 aligned-pairing) → recipe+substrate FREEZE → full-base regeneration ONCE (STL baselines re-run + champion + suite cells, ALL states × 4 seeds × 5 folds) → hand-off to the story effort. Read [`AGENT_PROMPT.md`](studies/closing_data/AGENT_PROMPT.md) + [`PLAN.md`](studies/closing_data/PLAN.md); launch pending user sign-off.
  - **CLOSED (Part-1) 2026-06-02** [`studies/archive/embedding_eval/`](studies/archive/embedding_eval/) — leak-aware 4-level (L0 geometry → L3 MTL) substrate evaluation ladder + re-screen of dropped improvements. Outcome: **v14 = `check2hgi_design_k_resln_mae_l0_1`** (ResLN+mae cat ⊕ Delaunay-POI-GCN reg, orthogonal stack) is the dual-axis champion base (supersedes v13, opt-in; canonical `check2hgi` untouched). design_k re-validated at FL (overturned prior AL/AZ-only K≡J); leak-free multi-seed reg +0.9-1.1pp over canonical (closes 54-78% of HGI gap, HGI keeps a small edge); cat ≈ frozen-canon ≫ HGI. **NO MTL benefit** (v14 or dual-substrate routing pilots) → MTL cross-attn regime is the wall (Part-2). Synthesis: [`FINAL_SYNTHESIS.md`](studies/archive/embedding_eval/FINAL_SYNTHESIS.md); version: [`results/CANONICAL_VERSIONS.md §v14`](results/CANONICAL_VERSIONS.md).
  - **CLOSED 2026-05-29** [`studies/archive/substrate-protocol-cleanup/`](studies/archive/substrate-protocol-cleanup/) — substrate + protocol cleanup. Outcomes: **log_T-KD PROMOTED** (deployable MTL reg lift, now v12 default); substrate axis (Designs B/J/Lever 4/5) **NULL in MTL** at AL/AZ/FL — the cross-attn joint-training regime washes out substrate/encoder gains on both axes (even HGI ≈ canonical in MTL); §4.4 + P4 closed (architectural → `mtl_improvement`); ResLN encoder = STL-best (v12 default, no MTL benefit). Closure: [`CLOSURE.md`](studies/archive/substrate-protocol-cleanup/CLOSURE.md); synthesis: [`findings/F_SUBSTRATE_PROTOCOL_CLEANUP_SYNTHESIS.md`](findings/F_SUBSTRATE_PROTOCOL_CLEANUP_SYNTHESIS.md); defaults: [`results/CANONICAL_VERSIONS.md`](results/CANONICAL_VERSIONS.md).
  - **CLOSED 2026-05-24 v6 final** [`studies/archive/mtl-protocol-fix/`](studies/archive/mtl-protocol-fix/) — F1 selector fix (+5.6 pp FL multi-seed deployable; C21 RESOLVED); P4 frozen-cat identifies residual MTL-vs-STL reg gap as architectural; Phase 3 outcomes: §4.5 log_T-KD PROMOTED, §4.6 sampler FALSIFIED, §4.2 composite ESTABLISHED (+7-12 pp project headline on reg). Closure verdict: [`results/mtl_protocol_fix/phase1_phase2_verdict_v6_final.md`](results/mtl_protocol_fix/phase1_phase2_verdict_v6_final.md). Deferred-work map: [`studies/archive/mtl-protocol-fix/DEFERRED_WORK.md`](studies/archive/mtl-protocol-fix/DEFERRED_WORK.md).
  - **CLOSED 2026-05-19** [`studies/archive/canonical_improvement/`](studies/archive/canonical_improvement/) — 6 tiers, 26 mechanism families, substrate axis exhausted ±0.8 pp; surfaced C21. ⚠ Tier 6 FL-MTL artefacts carry stale-log_T caveat — see [`CONCERNS.md` C22](CONCERNS.md#c22).
  ...
  - [`studies/archive/hgi_category_injection/`](studies/archive/hgi_category_injection/) — HGI POI2Vec category-injection on AZ (CLOSED 2026-05-04, falsified, kept pending FL/CA/TX revisit).
