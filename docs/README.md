...
- [`studies/`](studies/) — research tracks layered on check2hgi (active + closed). Cross-study outcomes log at [`studies/log.md`](studies/log.md).
  ...
  - **SCAFFOLDED (not launched) 2026-06-12** [`studies/closing_data/`](studies/closing_data/) — the final study: cross-study re-eval → pre-freeze gates (G0.1 aligned-pairing) → recipe FREEZE → CA/TX majors (once) → final 6-state tables. Read [`AGENT_PROMPT.md`](studies/closing_data/AGENT_PROMPT.md) + [`PLAN.md`](studies/closing_data/PLAN.md); launch pending user sign-off.
