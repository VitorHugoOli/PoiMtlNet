...
> ⛔ **2026-06-05 — CLASS-WEIGHTING CONFOUND flagged (CONCERNS C25) — affects every absolute MTL-reg claim.** The MTL reg head was trained on **class-weighted CE** (`default_mtl use_class_weights=True`) while the STL reg ceiling is **unweighted** — class-balancing optimizes macro accuracy *away from* the reported Acc@10 metric, depressing MTL reg ~10-14pp (verified: `--no-class-weights` recovers MTL reg to ≥ the STL ceiling). **`CH25` (composite STL-cat ⊕ STL-HGI-reg = +7-12pp vs MTL@disjoint) is UNDER RE-VALIDATION** — that advantage was measured against class-weight-depressed MTL reg and may shrink/dissolve. Any §0.1 absolute MTL-reg figure is likewise pending re-baseline (AL/GE/FL under unweighted reg CE). *Relative* within-MTL claims (common-mode) are likely unaffected. Do NOT promote MTL-reg-dependent claims to the whitelist until the re-baseline lands. Source: `CONCERNS.md §C25` + `docs/studies/mtl_improvement/log.md` 2026-06-05 ROOT-CAUSE.
