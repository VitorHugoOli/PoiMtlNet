...
> ⭐⭐⭐ **2026-06-06 — `CH30` MTL IS PARETO-POSITIVE (the `mtl_improvement` champion "G").** A SINGLE jointly-trained MTL model **MATCHES the STL next-region ceiling (Pareto-non-inferior) AND substantially beats the next-category ceiling (+3pp macro-F1) at ALL 4 available states, 4 seeds each**: AL cat 52.91 (+2.94); AZ 54.48 (+3.47); GE 61.43 (+3.31); FL 73.16 (+3.19) [reg AL 64.47/AZ 55.75/GE 59.37/FL 73.57]. ⚠ **REG verb CORRECTED 2026-06-07 (B-A2 / `studies/archive/mtl_improvement/CRITIQUE_TIER2_C25` §8):** the earlier "beats reg ceiling (+0.26..+1.59)" compared G's *in-distribution* Acc@10 to the (c) ceiling's *full* `top10_acc` (p1 has no indist split); on a MATCHED metric G is ~0.35pp BELOW the (c) reg ceiling (FL 72.93 vs 73.31) → reg = "matches" (Pareto-non-inferior, within ~0.4pp), NOT "beats". The cat beat is exact. The Pareto-positive / inverted-tradeoff headline (matches reg + beats cat dissolves the −7..−17pp 'MTL sacrifices reg' tension) STANDS. ⚠ **G′ (cat-private, FL-ONLY — DEMOTED 2026-06-07):** giving the CAT head a private tower too (both-private dual-tower, `mtlnet_crossattn_dualtower_catpriv`) gained cat at FL only (74.77, +1.61); the multi-state confirm (AL/AZ/GE × 4 seeds) **FALSIFIED** it — cat CRATERS at small states (AL 37.66 = −15.25 vs G, AZ −12.45, GE −3.59; reg flat). The cat-private tower **UNDERFITS** small-state cat (NOT overfit — the off-label STAN-flow head is over-regularized: AL train-F1 caps ~0.45 vs the GRU head's 0.98; a rescue screen of lower dropout / softer LR / smaller tower **CLOSED 2026-06-07 with NO rescue** — best AL lever still −14.5pp vs G, and the FL gain survives ONLY at the original `priv_dropout=0.3` (lowering it erases the gain, 74.74→73.17); the STAN flow/attention head is architecturally mismatched for a 7-class target at small data) → G′ is a **CLOSED FL-only experimental dead-end**, **NOT a champion**; **G (cat-SHARED) remains the multi-state champion**. See CHAMPION.md / INDEX `#T2V-5`. FL also TIES the (d) 2-model composite reg (73.62) while winning cat → the composite is strictly dominated. **The classic MTL tradeoff is INVERTED, not just dissolved.** G = `mtlnet_crossattn_dualtower` + `next_stan_flow_dualtower` (`fusion_mode=aux freeze_alpha=True alpha_init=0.0`), v14 substrate, unweighted onecycle KD-OFF. Mechanism: private reg tower (carries reg) + `aux` fusion (adds shared WITHOUT diluting private) + prior-OFF (removes the biased α·log_T logit term). Architecture CAPACITY is NOT the lever — falsified 5 ways (MoE, SwiGLU, MulT, crossstitch→crossattn, more-blocks). **STUDY champion, NOT yet paper §0 canon** (paper still v11; restatement is the open author decision). Full config + reproduction: [`studies/archive/mtl_improvement/CHAMPION.md`](studies/archive/mtl_improvement/CHAMPION.md). This SUPERSEDES the "≥ composite" framing in the 2026-06-05 banner below (G now BEATS the ceilings, not just matches). **VALIDATED 2026-06-07 (Tier 2V / `CRITIQUE_TIER2_C25` §7):** the headline held under a skeptical re-test — the (c)/(d) ceilings re-run at G's own seeds {0,1,7,100} are stable (σ≤0.7) and G still beats both at 4/4 states; the alt-arch falsifications are now FAIR (standalone, post-C25, per-arch `category-weight` → all lose by 1.6–2.1pp); no tail regression; no hypertuning lever beats G (logit-adjust HURTS the MTL cat → plain CE is the MTL cat optimum); "½ params" corrected to +4.9%. **The reg "beats both ceilings" claim is paper-safe; CA/TX (T2V.9) deferred to future-work.** ⚠ **CAT-GAIN DECOMPOSITION (2026-06-08, cat-transfer ablation — refines the "+3pp cat" claim):** the cat beat is **architecture-dominated**, NOT region transfer. Running G with reg-gradient OFF (`--category-weight 1.0`) isolates the cross-attn shared trunk: it alone gives **+2.27 (FL) / +3.22 (AL)** over the STL cat ceiling (4-seed {0,1,7,100}); genuine region→category transfer adds only **+0.93 (FL) / −0.67 (AL)** (multi-seed, tight σ; supersedes the seed0 +0.89/−0.71). **(FL re-run 2026-06-12 under HANDOFF_AUDIT P0** — was +2.13/+1.08; the FL `s1/s7/s100` cat-transfer manifest rows had mis-pointed to the fully-shared intrinsic-test run (`ls -dt` capture race), so FL cat-transfer at {1,7,100} had never run; re-ran genuine reg-OFF → FL cat+trunk 72.24 ± 0.03; transfer sign unchanged, now closer to the seed0 +0.89.) So the honest framing is "the joint cross-attn architecture is a better category *encoder* (+ small region transfer at scale)," NOT "the region task teaches category." Consistent with the gradient-orthogonality finding (CH31). Source: [`studies/archive/mtl_improvement/{cat_transfer_and_T53.md, WHY_ORTHOGONAL_AND_NO_MODERN_OPTIMIZERS.md}`].

> ⛔ **2026-06-05 — CLASS-WEIGHTING CONFOUND (CONCERNS C25) — FIXED + RE-VALIDATED multi-seed; central reg claims OVERTURNED.** The MTL reg head was trained on **class-weighted CE** (`default_mtl use_class_weights=True`) while the STL ceiling + the reported **Acc@10** metric are **unweighted** — class-balancing optimises macro accuracy *away from* top-K, depressing MTL reg ~10-14pp. FIXED (per-task, both heads unweighted) + re-validated {0,1,7,100} at AL/GE/FL. **Verdicts (do NOT cite the OLD versions):**
> - **`CH28` (regime: "STL substrate gain washes out in MTL") — OVERTURNED.** Unweighted Δreg(v14−canon) = +1.92/+1.49/+0.81 (AL/GE/FL, σ~0.1) → the substrate gain **TRANSFERS** to MTL.
> - **`CH25` (composite = +7-12pp over MTL@disjoint; ship 2 models) — DISSOLVED.** A single MTL model **≥ composite** (AL +0.94, GE −0.92, FL −2.07). The 2-model deploy is no longer justified on reg.
> - **The Tier-2 "architecture-negative / dual-tower loses" line — OVERTURNED** (dual-tower CLOSES the FL gap, orderings flipped under the fix).
> - **§0.1 absolute MTL reg — re-stated +10-13pp** (canon v11 GCN: AL 62.60/GE 56.34/FL 70.74 vs old ~50/42/61); **MTL cat re-stated UP** (exceeds STL cat ceiling +2-3.5pp).
> *Relative* within-MTL recipe orderings (R2 onecycle) survive (common-mode). Source: `CONCERNS.md §C25` + `docs/studies/archive/mtl_improvement/{log.md,HANDOFF.md,PAPER_UPDATE.md}` 2026-06-05.
...
> - **CH26 (PROMOTED multi-seed n=20 at small states; large-state pilot; NOW v12 CODE DEFAULT 2026-05-30)** — log_T-KD supervisory signal at the reg head (prior pathway). **Small states paper-grade**: disjoint reg Acc@10 +2.27 pp (AL) / +4.91 pp (AZ) @ W=0.2, seeds {0,1,7,100}, 20/20 folds, paired Wilcoxon p=9.54e-07 each; cat untouched at disjoint. Leak-audited clean ([`findings/F_TIER_A1_LEAK_AUDIT.md`](findings/F_TIER_A1_LEAK_AUDIT.md)). **Large states are a seed=42 pilot, NOT paper-grade**: FL +2.40 (5-fold p=0.031), CA +1.42 / TX +1.71 (1-fold) — sign-and-magnitude only; paper-grade large-state needs {0,1,7,100}. Frame as the **isolated single-MTL-artefact** reg lift, distinct from and smaller than the CH25 composite headline (+7–12 pp). **Defaulted in v12 (`--log-t-kd-weight 0.2`, scoped to MTL `check2hgi_next_region`); v11 paper §0.1 is no-KD — pass `--log-t-kd-weight 0.0`.** Still a **study-section claim**, NOT on the v10/v11 paper whitelist — whitelist promotion waits on the §0 re-run. Source: [`results/substrate_protocol_cleanup/tier_a1/phase_a1_verdict.md`](results/substrate_protocol_cleanup/tier_a1/phase_a1_verdict.md) + [`tier_a1_largestate/phase_a1_largestate_addendum.md`](results/substrate_protocol_cleanup/tier_a1_largestate/phase_a1_largestate_addendum.md); finding [`findings/F_TIER_A1_PROMOTION.md`](findings/F_TIER_A1_PROMOTION.md); version map [`results/CANONICAL_VERSIONS.md`](results/CANONICAL_VERSIONS.md). (Single-seed origin: [`results/mtl_protocol_fix/phase3_rank1_findings.md`](results/mtl_protocol_fix/phase3_rank1_findings.md).)
...
> - **CH28 (mechanism, the regime bottleneck — triply confirmed)** — the cross-attn MTL joint-training regime washes out encoder/substrate improvements on BOTH axes; only the prior pathway (log_T-KD, CH26) moves MTL reg. Three converging lines: (a) four design substrates (B/J/L/M) close **0 %** of any MTL gap (disjoint \|Δ\|≤0.16 pp FL, all NS); (b) **HGI ceiling** — even the STL reg winner (+2.12 pp STL) gives **no MTL reg advantage** (Δ+0.51, p=0.41 NS) and its STL win vanishes under B9 joint training; (c) **isolation cell** — identical head/config, STL α=0 LEARNS region ~73 % Acc@10 while MTL α=0 FLOORS at ~0.03 % (α=0 is OOD → regime-scoped). Verdict: the MTL reg bottleneck is the joint-training regime / shared-backbone architecture, not the substrate → `mtl_improvement`. **Study-section mechanism claim, not paper-whitelisted.** Source: [`studies/archive/substrate-protocol-cleanup/CLOSURE.md`](studies/archive/substrate-protocol-cleanup/CLOSURE.md), [`results/RESULTS_TABLE.md §0.9`](results/RESULTS_TABLE.md), [`findings/F_SUBSTRATE_PROTOCOL_CLEANUP_SYNTHESIS.md`](findings/F_SUBSTRATE_PROTOCOL_CLEANUP_SYNTHESIS.md).
> - **CH29 (ResLN STL dual-axis — STL-ONLY, NOW v12 ENCODER DEFAULT)** — `ResidualLNEncoder` is the best STL cat encoder: cat F1 **+0.86 FL / +1.48 AL / +1.70 AZ** (5/5 seeds, p=0.03125); reg ≈0 small states / +0.71 FL. Dual-axis STL champions: **ResLN+design_b** (general) / **ResLN+design_j** (AL-specific) — opt-in research variants. **CRITICAL: NO MTL benefit** (per CH28 regime finding); the value is STL/representation-quality/generality only. Defaulted in v12 for future builds; the frozen v11 GCN paper substrate is unchanged. **Study-section claim, NOT a paper MTL claim.** Source: [`results/substrate_protocol_cleanup/tier_resln/phase_resln_verdict.md`](results/substrate_protocol_cleanup/tier_resln/phase_resln_verdict.md), `canonical_improvement` T3.2.
> - **CH31 (the unifying mechanism + the modern-optimizer NEGATIVE — 2026-06-08, `mtl_improvement` Tiers 3/4/5 close).** The next-category and next-region **task gradients are ORTHOGONAL** on the shared trunk (cos≈0: FL +0.0007 / AL +0.0026 over all 50 epochs). **TESTED intrinsic, not architecture-induced (2026-06-10):** the obvious confound — that G's dual-tower (β=0.1 aux gating) starves reg's shared channel and so *induces* cos≈0 — is ruled out; in a fully-shared model where reg's shared gradient is LARGER than cat's (ratio 1.26 AL / 1.78 FL), the cosine is still ≈0 (+0.0024 AL / +0.0017 FL). So the dual-tower *exploits* a pre-existing orthogonality, it doesn't manufacture it (`results/mtl_improvement/orthogonality_intrinsic_test.md`). This single fact unifies the study: **(a) NO modern MTL optimizer helps** — the full `src/losses` balancer registry (RLW/GradNorm/PCGrad/CAGrad/Nash/Aligned-MTL/DWA/DB-MTL/FAMO/UW/… — per-method-tuned, arch-wired) + loss-scale-norm all FAIL to Pareto-beat tuned `static_weight cw=0.75` (gradient-surgery only helps under *negative* cosine; loss-scale-norm starves the high-card reg head; static 0.75 is on the Pareto front). Expected at k=2 with a tuned baseline per **Kurin et al. NeurIPS'22** + **Xin et al. NeurIPS'22**. **(b)** It explains why *more sharing* failed (Tier 2 MoE/cross-stitch/hard-share lose reg — forcing orthogonal tasks together induces conflict) and **(c)** why the **dual-tower wins** (protect reg in a private tower; let cat harvest the jointly-trained cross-attn encoder). The MTL win is **representational/architectural, not gradient-cooperation**. **Study-section claim** (the negative is paper-grade for a limitations/analysis section). Reg-input levers (overlap data-scale R1, HGI substrate R2) likewise *transfer but don't beat the moving ceiling* (rising tides). Source: [`studies/archive/mtl_improvement/WHY_ORTHOGONAL_AND_NO_MODERN_OPTIMIZERS.md`](studies/archive/mtl_improvement/WHY_ORTHOGONAL_AND_NO_MODERN_OPTIMIZERS.md), [`results/mtl_improvement/T4_audit_and_verdict.md`](results/mtl_improvement/T4_audit_and_verdict.md); figures `studies/archive/mtl_improvement/figs/{grad_cosine_tasks,t4_balancer_scatter_FL,t4_loss_weight_trajectories_FL}.png`; CONCERNS §C27.
...
>
> **Tier-6 final closure verdict (2026-05-19, matched-protocol analysis across ALL four pre-registered mechanism families):**
>
> **CH23-A (Tier-6 POI-internal-supervision hypothesis — FALSIFIED across four orthogonal mechanism families).** Under matched protocol (FL, single-seed=42, n=5 folds, ep=50), no Tier-6 substrate intervention delivers a deployable single-checkpoint improvement under `joint_geom_simple`. Per-task-disjoint reg-top10 ceiling at FL clusters at ~76.1-76.9 pp across 11 cells with σ ~0.3 pp. The four mechanism families:
>
> | Mechanism family | Cells | Per-task disjoint Δ_reg | Disposition |
> |---|---|---:|---|
> | T6.4 (loss-shape reform: InfoNCE @ p2r + two-pass corruption) | 2 | +0.08 to +0.17 | FALSIFIED (within σ) |
> | T6.1 (POI↔POI co-visit InfoNCE 4th boundary; original 4 cells + 1 robust) | 5 | +0.05 to +0.20 | FALSIFIED (within σ; cat regresses on deployable selector) |
> | T6.2 (HGI-inspired composite edge weights: Delaunay multiplier × cross-region penalty) | 4 | +0.23 to +0.76 | §Discussion (cat -1.3 to -3.6 pp Pareto trade; reproduces T4.4 trade shape at more favourable point) |
> | T6.3 (capacity-restricted per-POI bias at Checkin2POI attention-logit; rank-r structural cousin of T5.1 DEAD) | 2 of 4 (stage-1 halted) | halted | FALSIFIED at G3 hi/lo ratio gate (AZ r=8: 3.58× → 3.09×) |
>
> **None of the 11 measured T6 cells delivered both reg ≥ +0.5 pp AND cat non-inferior at the deployable `joint_geom_simple` selector.** T6.4 code paths land as opt-in infrastructure (default-off, byte-identical) for future studies that pair them with other interventions. T6.1 code paths same. T6.3 implementation in `research/embeddings/check2hgi/model/Checkin2POI.py` is opt-in default-off (rank-r per-POI bias at attention-logit, zero-init v ⇒ step-0 bit-identical to canonical).
>
...
> - **Allowed paper claim (CH23-A, summarising the Tier-6 falsification)**: "Across four orthogonal mechanism families (loss-shape reform, contrastive 4th boundary, geometric edge-weighting, capacity-restricted POI bias), no Tier-6 substrate intervention raises the per-task-disjoint reg-top10 ceiling at FL by more than +0.76 pp at single-seed n=5, and that cell trades it for a -3.55 pp cat regression. The substrate ceiling under canonical static_weight w_cat=0.75 MTL balancing is ~76.1 pp; the gap to HGI is a protocol-side artefact, not a substrate-side artefact."
> - **Disallowed paper claims**:
>   - "T6.4 / T6.1 / T6.2 / T6.3 ships a substrate improvement." (All FALSIFIED at matched protocol — T6.3 halted at pre-registered G3 hi/lo ratio gate at AZ r=8.)
>   - "T6.4 / T6.1 enables cat +2 / reg +13." (Cross-selector comparison artefact; the lift exists in **shipping itself** under per-task disjoint, not in any T6.* variant specifically.)
>   - "T6.2 α=2.0 w_r=0.3 is the first non-null Tier-6 substrate effect." (Per-task disjoint reg lift +0.76 pp at single-seed n=5 is partially separable from σ=0.82 but does not survive Bonferroni at m_Tier6=10 cells; cat regression -3.55 pp at per-task disjoint makes it non-shippable; the cell reproduces the T4.4 cat-reg Pareto trade at a more favourable point, not a paper-grade substrate win.)
>   - "T6.3 r=4 passes the pre-registered gate." (Locked criterion was ALL state-rank cells pass; AZ r=8 hi/lo ratio compression 3.58× → 3.09× tripped the gate per pre-registration. The 3-of-4-cells-pass observation is §Discussion-only.)
...
> - **AL G3 gate violation at T6.4 + leak-class trip at T6.3 r=8** are §Discussion-only and are moot because no T6.* candidate has a shipping promotion path under any selector.
> - **Multi-seed deferred** for ALL Tier-6 candidates (no signal at the deployable selector to tighten) and for shipping (matched-protocol single-seed adequately documents the selector bug). The shipping FL/CA/TX numbers should be **re-reported under the mtl-exploration F1 selector** once that lands — zero retraining required via `scripts/canonical_improvement/analyze_t64_selectors.py`.
> - **Source-of-truth**: `docs/results/canonical_improvement/T6_{1,2,4}_dual_selector*.{json,md}` + `T6_1_robustness_dual_selector.{json,md}` (matched single-seed=42 n=5 ep=50) + `G3_{alabama,arizona}_T6_3_r{4,8}.json` (T6.3 stage-1 only — FL stage halted by pre-registered gate). Upstream framing: **`CONCERNS.md` C21**. Closure path: **`docs/studies/mtl-exploration/FUTUREWORK_substrate_aware_mtl_balancing.md`** F1/F2/F3.
>
...
**Source:** `docs/results/P1_5b/next_category_alabama_{check2hgi,hgi}_5f_50ep_fair.json`.
...
**Source:** `docs/results/probe/{alabama,arizona}_{check2hgi,hgi}_last.json` (Leg I) + `docs/results/phase1_perfold/{AL,AZ}_{check2hgi,hgi}_cat_{gru,single,lstm}_5f50ep.json` (Leg II + C2) + `docs/results/paired_tests/*_cat_*.json` (statistical tests).
...
**Source:** `results/probe/alabama_check2hgi_pooled_last.json` (linear probe) + `results/check2hgi_pooled/alabama/next_lr1.0e-04_bs1024_ep50_20260427_*` (AL matched-head STL) + `results/{check2hgi,check2hgi_pooled,hgi}/arizona/next_lr1.0e-04_bs1024_ep50_20260503_*` (AZ matched-head STL). Per-fold JSONs in `docs/results/phase1_perfold/AZ_*_cat_gru_5f50ep_20260503.json`. Code: `scripts/probe/build_check2hgi_pooled.py`, launcher `scripts/run_AZ_pervisit_counterfactual.sh`.
