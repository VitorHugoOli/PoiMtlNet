...
| A5 | **Temporal-split bridge**: per-user chronological 80/10/10 at FL + AL; run champion G, STL ceilings, Markov-1, (CTLE if A3 done) | Connects to the field's universal protocol; tests forward-time generalization the CV protocol never touches | One split implementation + ~10 runs | standalone (pre-paper) |
...
| B9 | **Second dataset validation phase**: Massive-STEPS NYC (champion G + STL ceilings + Markov floor, 4 seeds; shipped temporal splits) — see [`future_work.md §8`](future_work.md) | Breaks Gowalla-specificity AND delivers the A5 temporal-split bridge on a public, license-clean benchmark in one move (A5 then shrinks to the Gowalla-side chronological re-split only) |
...
| **4 — External validation** (may overlap P3 tail) | champion + ceilings + Markov on the 2nd corpus = temporal bridge | `second_dataset` Phase V | CUDA box | yes |
...
| A5 (temporal bridge), B9 (2nd-dataset validation) | `second_dataset` | 0 (ETL) / 4 (validation) | B9 *is* A5 on Massive-STEPS; A5 then shrinks to a Gowalla chronological re-split |
