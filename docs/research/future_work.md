...
**Recommendation: Massive-STEPS NYC as the primary second dataset.** It is the only option that simultaneously (a) is a different LBSN source than Gowalla, (b) ships per-check-in fine-grained categories *and* coordinates (so both the 7-class rollup and TIGER-tract regions reproduce), (c) carries per-check-in **timestamps** from which we can build our **own chronological per-user split** — so it can deliver the temporal-split protocol bridge (roadmap A5) as a scoped re-split addendum (see below), and (d) is modern (2017–18), pre-empting the "2012-era data" critique that Massive-STEPS itself was built to make. Add **FSQ-TKY (TSMC-2014)** as an optional third corpus only if a non-US robustness point is wanted (regions = H3 cells there, which also tests tract-choice sensitivity).

> ⚠ **Correction (F1, ETL 2026-06-15):** the original (c) claimed Massive-STEPS *ships* temporal train/val/test splits, so running it would deliver A5 "for free." **That is false** — the shipped split is a user-stratified **RANDOM** split over short trails (median ~2 check-ins; users deliberately in all three splits), not chronological, and the trails do not fit the window-9 protocol (see [`docs/studies/second_dataset/PHASE_E_REPORT.md §F1–F3`](../studies/second_dataset/PHASE_E_REPORT.md)). The bridge is **NOT free from the shipped split**, but it **CAN** be built on Massive-STEPS by constructing our own **chronological per-user split** from the per-check-in timestamps (local civil time is fine for per-user ordering). This is now the **recommended** way to deliver A5 — better than a Gowalla-side re-split because it lands the bridge on a modern non-US+US corpus — scoped as a `second_dataset` **Phase E2** chronological-re-split addendum (Mac-track, no CUDA, no freeze dependency). The Gowalla-side chronological re-split remains the fallback.

Two design notes: (i) build a documented Foursquare→7-root category mapping so the category task is commensurable with the Gowalla states (or, better, report both 7-class macro-F1 and fine-grained Acc@k); (ii) scope the second dataset as a **validation phase, not a full matrix** — champion G + STL ceilings + Markov floor + (CTLE if available), one city, 4 seeds — not the full closing_data regeneration. A third (F2) note on **region granularity**: NYC uses TIGER census tracts (faithful to the Gowalla definition); Istanbul uses OSM `mahalle` (520 regions — no government tract source exists) plus an already-built H3 sensitivity variant (~2,585 cells, NYC-granularity-matched). Because region cardinalities differ across corpora, Phase V MUST report **gap-to-ceiling / lift-over-floor, NOT absolute Acc@k** — the second-dataset comparison is apples-to-apples only in the gap-to-ceiling framing.
