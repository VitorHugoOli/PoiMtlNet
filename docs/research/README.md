...
| [`pairing_science.md`](pairing_science.md) | ⭐ **Cross-task pairing finding (article candidate, 2026-07-02).** Why sample-aligned cross-task pairing HURTS (an instance-memorization shortcut: train +10.1 / val −7.2, shortcut-first trajectory, absent at 13× data) while the champion's random pairing wins (implicit input-noise regularization) — so the orthogonal transfer is a *regime* property, not a pairing artifact. Paper-ready paragraph + verified citations; evidence base in `../studies/pipeline_audit/`. |
