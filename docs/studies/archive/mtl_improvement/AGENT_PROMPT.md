...
> 1. **The "regime finding" (substrate gains wash out in MTL; gap is irreducibly architectural) is OVERTURNED.** It was an artifact of a **class-weighting confound** (`CONCERNS.md §C25`): the MTL reg head trained on class-WEIGHTED CE while the Acc@10 metric + STL ceiling are unweighted. Fixed (both heads unweighted, now the `default_mtl` default).
> 2. **A champion config "G" now BEATS BOTH single-task STL ceilings (reg AND cat) at all 4 available states, 4-seed** — the MTL tradeoff is INVERTED, not just closed. G = `mtlnet_crossattn_dualtower` + `next_stan_flow_dualtower` (`fusion_mode=aux freeze_alpha=True alpha_init=0.0`), unweighted onecycle. **Full config + runnable command + closed dead-ends: [`CHAMPION.md`](CHAMPION.md).**
> The experimental track is essentially COMPLETE; the only open item is the BRACIS paper-doc restatement (author decision). The tier-chain narrative below is HISTORICAL CONTEXT — do not start new architecture-capacity experiments (falsified 5 ways), and **do not retry the cat-private tower (G′ — CLOSED 2026-06-07 as an FL-only dead-end; the champion cat head is the shared `next_gru`; see `CHAMPION.md §G′`).** If you must onboard, the order is: `HANDOFF.md` (state) → `CHAMPION.md` (the result + how to run it) → `log.md` (chronology) → this prompt (historical design rationale).

> ⚠ **2026-06-02 — this track was REBASED.** The base substrate is now **v14** (`check2hgi_design_k_resln_mae_l0_1`), compared vs canonical paper **v11**. ~~The binding constraint is the regime finding~~ (OVERTURNED — see the 2026-06-06 banner above). Read `INDEX.html` §"What changed (v1→v2)" + §"Metrics & comparand framework" for the framework. The 6-tier v2 chain superseded the old 8-tier v1 chain.
...
| # | File | Purpose |
|---|------|---------|
| 0 | `docs/studies/archive/mtl_improvement/HANDOFF.md` | ⭐ **READ FIRST** — current-state snapshot (frozen ceilings, what's done, traps, the next step). Avoids reconstructing from the full log. |
| 1 | `docs/studies/archive/mtl_improvement/log.md` | Most recent progress — read the **2026-06-02 reframe entry** first |
| 2 | `docs/studies/archive/mtl_improvement/INDEX.html` | Full v2 6-tier design. **Read §What changed + §The regime finding + §Metrics & comparand framework + §Execution guidelines + §Falsified history + §Chain cover-to-cover before any experiment.** |
...
| 4 | `docs/studies/archive/embedding_eval/FINAL_SYNTHESIS.md` | The v14 verdict + **the regime finding** (substrate washes out in MTL). |
...
16. **Tier-S candidate hygiene.** (a) Sweep the **already-coded** `next_*` heads first (Prong A — real registry names; SASRec already exists as `next_transformer_relpos`, don't rebuild to test it). (b) New-arch builds (Prong B — faithful SASRec/Mamba/SimGCL) are in scope (user: keep exploring) but must clear the unit-test gate. (c) Unit-test EVERY head incl. coded ones — "coded" ≠ "working" (bit-rot + the registry silently drops unknown kwargs, so a log_T-aware head can train while ignoring the prior). (d) Do NOT import full next-POI *pipelines* (GETNext/STHGCN/Graph-Flashback/MobGT/Diff-POI) — their contribution is the α·log_T prior we own + they need raw GPS/time/category we lack. (e) Do NOT use the `category_*` family — flat `[B,D]` input + the category-input builder rejects check-in-level engines (`the next-input builder in `src/data/inputs/builders.py``); wrong modality+substrate. The 9k-class softmax is NOT a blocker for an STL head.
...
2. **Tier 1 — consolidate ceilings + encoder probe + ceiling-hardening**: T1.1/T1.2 = confirm-and-pin the STL-on-v14 (c) + composite (d) ceilings already produced by the T0.2 board (no fresh runs) — **DONE**. **T1.3 encoder-isolation probe (≈5 GPU-h) gates the expensive Tier 2** (if the upstream encoder owns the residual, do the cheap encoder-bypass before the dual-tower) — **DONE (dual-tower justified)**. **T1.4 tuned current-head ceiling = the FLOOR (BLOCKS T2)** is the remaining Tier-1 gate: the STL heads were never HP-tuned leak-free, so do the **full per-task HP tune** of the current heads — reg (`next_stan_flow`): α re-tune per-state (incl. α→0 at large per T1.3) + tail-loss + ranked HP axes (depth/dim/dropout/LR/warmup); cat (`next_gru`): loss calibration (logit-adj+focal+label-smoothing, train-only prior) + ranked HP axes (pooling/dropout/LR/warmup). Two-phase: search at FL+AL, validate at FL+AL+AZ 5f + GE confirm. **The tuned best SETS + FREEZES (c)/(d).** It must be the FULL tune, not a bounded subset (advisor 2026-06-03): T2's gate is "fraction of (d) recovered" — an under-tuned ceiling sandbags T2's go/no-go, corrupting the chain branch, not just a paper number. Positive-EV regardless. Framing = ceiling/baseline integrity, NOT MTL transfer.

2b. **Tier S — STL head search (STL-alone) — runs PARALLEL with Tiers 2-4, not in the linear chain.** Clean split: T1.4 tunes the canonical heads' HYPERPARAMETERS (the floor); Tier S SWAPS to other heads, scored against that frozen floor. **Two prongs (both tasks):** Prong A = sweep the **already-coded but never-run** `next_*` heads (the user's point — test what we have): general encoders `next_lstm`/`next_temporal_cnn`/`next_tcn_residual`/`next_conv_attn`/`next_hybrid`, SASRec-style `next_transformer_relpos`/`next_transformer_optimized`/`next_single`, reg-prior ablations `next_getnext`/`next_stan`/`next_tgstan`/`next_stahyper`/`next_stan_flow_hsm`. Prong B = build + test genuinely-NEW archs (faithful SASRec, Mamba/SSM, SimGCL aux) — user: keep exploring even where a similar coded head exists. **S.1** reg, **S.2** cat (encoders only — cat *losses* live in T1.4; `category_*` family EXCLUDED — flat input + builder rejects check-in engines), **S.3** = analyze + compose (gated, results-triggered: a per-task analyst agent + advisor name 2-3 orthogonal compositions — encoder ⊕ aux ⊕ prior, never two encoders — run through the EXISTING gate; "name before max" is the only added rule; STL composite = a candidate-generator for T5, not a destination), **S.4** = no-run synthesis "huge picture" (default / tuned-current / best-coded / best-new / best-composite), labelled a promotion test (unequal HP budget), lose-within-noise → second-round-tune. **Unit-test gate on EVERY head (coded too — bit-rot + silently-dropped kwargs)** + screen-then-promote funnel (AL×1-seed screen → top-2-3 to FL+AL+AZ 5f + GE confirm). Collocate under MPS. **Feeds the T5 candidate set + future-work ONLY; NEVER re-opens the T1.4-frozen (c)/(d).** **Open sandbox (user 2026-06-03):** Tier S may grow — add S.5/S.6 cards as ideas surface, no re-scoping checkpoint needed, PROVIDED each clears the unit-test + promotion gates, obeys the moving-baseline guard + scope boundaries, and is logged. It runs parallel to Tier 2-4 and must not starve the regime work (the headline).
...
- Plots → `docs/studies/archive/mtl_improvement/figs/T{tier}-{id}_*.png`
...
```
docs/studies/archive/mtl_improvement/
