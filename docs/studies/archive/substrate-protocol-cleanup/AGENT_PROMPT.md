...
> **Paste this prompt at the start of every fresh Claude Code session that picks up this track.** This study is intentionally narrow and small-state-only. Its purpose is to close the cheap, non-architectural items left over from `mtl-protocol-fix` while [`docs/studies/archive/mtl_improvement/`](../mtl_improvement/) (branch `mtl-improve`) handles the architectural axis in parallel.
...
| # | File | Purpose |
|---|------|---------|
| 1 | `docs/studies/archive/substrate-protocol-cleanup/log.md` | Most recent progress and decisions |
| 2 | `docs/studies/archive/substrate-protocol-cleanup/INDEX.md` | Full Tier A-D design + decision gates |
| 3 | `docs/studies/archive/substrate-protocol-cleanup/considerations.md` | Why each Tier is here and what was *not* included |
| 4 | `docs/studies/archive/mtl-protocol-fix/DEFERRED_WORK.md` | Authoritative deferred-work map; identifies every item this study owns |
