...
- Folder `docs/studies/archive/substrate-protocol-cleanup/` created alongside `mtl-protocol-fix/` and `mtl_improvement/`.
...
1. Implementing agent should read in order: this log, `INDEX.md` (Tier by Tier), `considerations.md`, `docs/studies/archive/mtl-protocol-fix/DEFERRED_WORK.md`, `docs/results/mtl_protocol_fix/phase3_summary.md`, `docs/CONCERNS.md` C15/C21/C22/C23.
...
- Landed audit doc at [`docs/studies/archive/substrate-protocol-cleanup/window_mask_audit.md`](window_mask_audit.md) with file_path:line_number citations for every claim.
...
**Verdict: overall CLEAN — no leak found. Tier B / C unblocked. `mtl_improvement` Tier 1 unblocked. Per the handoff protocol the mirror entry has been posted to `docs/studies/archive/mtl_improvement/log.md`.**
...
**B10 Git — VERIFIED clean.** `git status`: no tracked modifications; HEAD `c88b043` == `origin/main`. Untracked = result artifacts under `docs/results/substrate_protocol_cleanup/`, `scripts/substrate_protocol_cleanup/{build_v13_catx.sh,c1_rescore/}`, `docs/studies/archive/fusion/` (none gitignored — `git check-ignore` exit 1). These are run-output/scratch, not reproduction- or doc-critical; flag only as housekeeping (commit-or-gitignore if they should be tracked).
