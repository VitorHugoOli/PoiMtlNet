...
| [`train_perf_multifold/`](train_perf_multifold/) | **TOOLING — 2026-06-26 (`study/train-perf-multifold`)** — engineering, not science: (1) ephemeral run **profiler/audit** (`--profile`, bottleneck/speed/quality flags); (2) **multi-fold fan-out** (`--only-folds`/`--run-id`/`--per-fold-seed` + `aggregate_folds.py` + `run_folds_fanout.sh`) — N fold-processes → ONE rundir, **proven byte-identical under 5-way concurrency**; (3) quality-neutral perf (STAN `.any()` graph-breaks 10→2, OOD cache, pin CPU-resident). AL champ-G reproduces the board within fold-std. | A40 | [`train_perf_multifold/PLAN.md`](train_perf_multifold/PLAN.md) |
