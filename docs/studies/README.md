...
| [`closing_data/`](closing_data/) | **SCAFFOLDED (not launched)** — the final study: cross-study re-eval → pre-freeze gates → recipe FREEZE → CA/TX majors (once) → final 6-state tables. Launch pending user sign-off on the plan | (TBD) | [`closing_data/AGENT_PROMPT.md`](closing_data/AGENT_PROMPT.md) + [`closing_data/PLAN.md`](closing_data/PLAN.md) |
