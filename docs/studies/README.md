...
|---|---|---|---|
| [`closing_data/`](closing_data/) | **BOARD COMPLETE (2026-07-13, PR #63) — paper close-out (MobiWac).** The study that produced the paper board, switched to the **v17** headline; **the v17 board is n=20 at all six datasets** (A1 CA/TX done 2026-07-11; M1-full 6-dataset Holm ALL-REJECT; T6 joint-best ≈ diag-best). Residual non-verdict work: A2 ReHDM CA/TX (running) + optional coverage/doc chores. Origin plans/verdicts/manifests + the spent per-machine handoffs live in [`closing_data/archive/`](closing_data/archive/README.md). | (main) | [`closing_data/RESULTS_BOARD.md`](closing_data/RESULTS_BOARD.md) (board SSOT) · [`closing_data/v17_completion/`](closing_data/v17_completion/README.md) (completion record + residual queue) · [`closing_data/log.md`](closing_data/log.md) |
| [`merge_design/`](merge_design/) | **CLOSED** — Designs A-M and Levers 1-6 all falsified or saturated; all structural RQs closed; Lever 5 rescued+falsified by `substrate-protocol-cleanup` Tier B4; design_k graduated into the v14 substrate. No surviving open item. Per [`closing_data/archive/verdicts/PHASE1_VERDICT.md`](closing_data/archive/verdicts/PHASE1_VERDICT.md) §1 (cross-study table) + §2 (lever dispositions). | (no dedicated branch) | [`merge_design/STATE.md`](merge_design/STATE.md) |
...
| [`pipeline_audit/`](pipeline_audit/) | **CLOSED 2026-07-01 (branch `audit/pipeline-correctness-perf`)** — full input-builder + MTL-train correctness/perf audit (10-reader map + 14 adversarial verifiers + parity-gated fixes). **Champion v17 path verified leak-clean** (2 fresh-eyes hunts: no unreported val→train flow, metrics exact). **Pairing question answered**: train batches ARE randomly paired across tasks (val aligned) but forcing alignment is null@FL / harmful@AL (G0.1) — not the transfer wall; `--aligned-pairing` CLI crash FIXED (binding G0.1 unblocked). Fixed: latent ga>1 zero_grad bug (+content test), canon-guard bypass, eager fold-subset OOM path, auto-fp32 train/eval asymmetry (+`p3_board.sh` precision env), 5 guard gaps, freeze-mode dropout re-assert, 11 doc contradictions. Perf: **P4 `MTL_NO_TRAIN_DIAGNOSTICS` adopted (−9% AL wall, byte-identical eager)**. Read [`README.md`](pipeline_audit/README.md). | A40 | [`pipeline_audit/README.md`](pipeline_audit/README.md) |
| [`train_perf_multifold/`](train_perf_multifold/) | **CLOSED 2026-06-30 (`study/train-perf-multifold`, PR #56)** — started as tooling, ended with a board-wide quality win. (1) run **profiler/audit** (`--profile`); (2) **multi-fold fan-out** (`--only-folds`/`--run-id`/`--per-fold-seed`, byte-identical under 5-way concurrency); (3) quality-neutral perf; (4) **batch-size + per-head-LR study**: `--cat-lr` etc. were INERT under onecycle → fix `MTL_ONECYCLE_PER_HEAD_LR` → **bs=8192 + cat-lr 1e-3 beats champion board-wide (n=20): AL +1.0 / AZ +2.3 cat; FL +0.17cat/+0.20reg & +7% faster**. FL regression = cat-LR overshoot (not reg-capture). Read [`CLOSURE.md`](train_perf_multifold/CLOSURE.md) first. | A40 | [`train_perf_multifold/CLOSURE.md`](train_perf_multifold/CLOSURE.md) |
