...
**As of 2026-06-05 (⭐ ROOT CAUSE FOUND: the MTL reg "wall" was a CLASS-WEIGHTED-LOSS confound).** Branch `mtl-improve`, pushed.
This is a single "you are here" snapshot. Full chronology: `log.md` (the 2026-06-05 ROOT-CAUSE entry first; then the bisect/breakthrough/joint-loop thread incl. a RETRACTED input-artifact false alarm and a RETRACTED "joint-loop" framing).
Design + per-tier results: `INDEX.html`. **Tier-2 close-out (now SUPERSEDED — see below): `PAPER_UPDATE.md`.**

> ⭐⭐⭐ **2026-06-05 — ROOT CAUSE of the MTL→STL reg gap: the MTL reg head trains on CLASS-WEIGHTED CE; the STL ceiling uses UNWEIGHTED CE. It is an OBJECTIVE MISMATCH, not architecture/substrate/joint-loop.** `default_mtl` silently sets `use_class_weights=True` (`src/configs/experiment.py:364`; the STL reg **ceiling (c) was set by `p1_region_head_ablation.py`** which uses **unweighted** `CrossEntropyLoss` via `build_calibrated_loss` — NB `default_next` is ALSO `True` (`:444`; line 403=False is `default_category`), so an STL reg run via `train.py --task next` would ALSO be class-weighted — a separate latent inconsistency, but the (c) ceiling itself is p1/unweighted) → `mtl_cv.py:1283-1291` trains the ~1109-region reg head on class-BALANCED CE, which optimizes macro accuracy AWAY from the top-K (Acc@10) metric the study reports. **Verified:** T2P.0 AL + `--no-class-weights` → reg disjoint **64.81** ≥ STL ceiling 62.88 (vs the buggy default 52.90); explains all symptoms incl. state-scaling (more regions → more imbalance → bigger distortion). **The entire Tier-2/2P "MTL sacrifices reg / irreducible architectural wall / ship-the-composite" line — AND possibly the central REGIME FINDING — is substantially a class-weighting artifact** and is UNDER RE-VALIDATION (do NOT cite any absolute MTL-reg number until re-baselined under unweighted reg CE).
>
> **Provenance of the find (3 retractions on the way — read so the next agent doesn't re-walk them):** T2P.0 isolated the gap to "the joint loop" (single-task reconstruction hit the ceiling) → I chased a wrong "input-artifact" hypothesis (RETRACTED: MTL reg input byte-identical to (c)) → an independent agent showed production `train_model` called directly = 68 but via the wrapper = 55 (localized to a "wrapper preamble" — also a RED HERRING) → a deletion-bisect agent + my verification pinned it to the class-weighted reg CE. **My own earlier "loss-weighting ruled out" was WRONG (I checked the `getattr(config,'use_class_weights',False)` fallback, not the `default_mtl` factory value True).**
>
> **In-flight plan (user-approved 2026-06-05):** (1) document (this); (2) RE-AUDIT the other "ruled-out" loose points for another silent-default bug of this class; (3) implement PER-TASK class-weighting (reg OFF for Acc@10; cat decided by macro-F1) — the current `--no-class-weights` flag couples both heads (`mtl_cv.py:1284-1290`); (4) RE-TEST the regime finding + necessary sweeps under unweighted reg CE at **AL/GE/FL** (defer CA/TX). Scripts from the hunt: `t2p0_{linchpin,fp32_control,input_artifact,singletask_isolation,mechanism_probe,ckpt_reeval}.py/.sh`. Diagnostic env-gate `MTL_DISABLE_AMP=1` (forces fp32; default unset = canonical fp16) is committed but NON-causal.
