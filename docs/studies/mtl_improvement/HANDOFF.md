...
>
> **STILL OPEN (doc promotions — the live work):** re-state CH25/CH28/§0.1 + the Tier-2 verdict (positive) in the paper docs; promote per-task class-weighting (a real recipe WIN) to `NORTH_STAR.md` + pin a new `CANONICAL_VERSIONS` version; the Acc@1→Acc@10 reg-monitor code fix; FL-B9 follow-up for exact §0.1-table recipe continuity (re-validation used onecycle). T2P.1/2/3 = UNNEEDED (no gap to stage around); T2.3/T2.4 = optional stretch (axis live but gap closed).
>
