...
**As of 2026-06-05 (⭐⭐⭐ THE UNWEIGHTING FINDING + RE-VALIDATION COMPLETE — the study's reg narrative is reframed).** Branch `mtl-improve`, pushed.
...
>
> **The narrative flip:** OLD = *"MTL sacrifices reg; the gap is irreducibly architectural; ship the 2-model composite."* → NEW = **"a single MTL model matches/beats the STL ceiling AND the composite once the loss matches the metric; the substrate gain transfers to MTL; cat improves too."** A much stronger, positive paper.
>
> **Residual:** the only quantitative gap left was FL −1.8pp; the Tier-2 re-run shows it is the **α·log_T prior** (prior-OFF closes ~80%) + closed by the dual-tower (−0.25) — NOT a fundamental MTL limit. A dual-tower + prior-OFF combo (FL multi-seed) is confirming full closure.
>
...
> - ✅ **T2.3 MoE (mmoe/cgc) + T2.4 SwiGLU (`mtlnet_crossattn_swiglu`, pre-norm+SwiGLU, gate GREEN) = both NULL on reg** (+0.13..+0.22, within σ). With dual_gated 73.06 / prior_off 72.94 as the only movers, this **double-confirms the residual gap is the α·log_T prior + shared-pathway dilution, NOT architecture capacity.** SwiGLU's only effect: best cat of any arm (72.13). MulT-faithful + crossstitch→crossattn hybrids DROPPED (3rd test of a twice-falsified hypothesis). dual_gated remains FL best (−0.25 from ceiling).
>
> **STILL OPEN (paper-doc restatement only — author decisions, no more experiments):** re-state CH25/CH28/§0.1 + the Tier-2 verdict (positive) in the BRACIS paper docs. T2P.1/2/3 = UNNEEDED (no gap to stage around). Study is at a clean stopping point — no open GPU work; CA/TX deferred (large-state compute).
>
