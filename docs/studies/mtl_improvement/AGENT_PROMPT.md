...
15. **Frozen ceiling after T1.4.** Once T1.4 closes, anchors (c) STL-ceiling and (d) composite-ceiling are **immutable** for the rest of the track. Tier S and T5 may find better STL heads, but those are T5 candidates + future-work notes — they NEVER retroactively change (c)/(d) or re-score completed T2+ deltas (the moving-baseline trap). All STL-improvement work is framed as **ceiling/baseline integrity, NOT MTL transfer** (the regime finding predicts STL gains wash out jointly — do not re-litigate this).
