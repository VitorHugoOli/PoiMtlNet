...
2b. **Tier S — STL Frontier (deep NEW-architecture search, STL-alone) — runs PARALLEL with Tiers 2-4, not in the linear chain.** Clean split: T1.4 = tuned current heads (the floor); Tier S = NEW archs scored against that frozen floor. S.1 reg new-archs (SASRec, Mamba block, SimGCL aux, leak-free reg-head registry candidates); S.2 cat NEW encoder only (attention-pooling/TCN — cat *losses* live in T1.4, not here); S.3 = no-run synthesis assembling the per-task "huge picture" (default / tuned-current / best-new-arch), labelled a promotion test (unequal HP budget), with the lose-within-noise → second-round-tune safeguard. FL+AL+AZ 5f + GE confirm. Collocate under MPS alongside the MTL runs (STL-only → near-zero added wall-clock). **Feeds the T5 candidate set + future-work ONLY; NEVER re-opens the T1.4-frozen (c)/(d)** (the moving-baseline trap). De-scoping: do NOT import next-POI models — see Hard rules.
