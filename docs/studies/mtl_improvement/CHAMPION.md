...
> ⚠ **CAT-GAIN is ARCHITECTURE-dominated, NOT region transfer (2026-06-08 cat-transfer ablation).** The "+3 pp cat" is mostly the cross-attn shared *encoder*, not the region task teaching category. Decomposition (run G with `--category-weight 1.0` → reg gradient OFF → trunk trains cat-only): the cross-attn trunk alone gives **+2.27 (FL) / +3.22 (AL)** over the STL cat ceiling (4-seed); genuine region→category transfer adds only **+0.93 (FL) / −0.67 (AL)**. (FL re-run 2026-06-12 under HANDOFF_AUDIT P0 — was +2.13/+1.08; the FL multi-seed manifest had mis-pointed to the fully-shared intrinsic-test run; transfer sign unchanged.) Consistent with the gradient-orthogonality finding, which is **tested-intrinsic** (cos(∇cat,∇reg)≈0 persists in a fully-shared model where reg dominates the shared gradient — `orthogonality_intrinsic_test.md`). Honest paper framing: "the joint cross-attn architecture is a better category encoder, with a small region transfer at scale." Detail: `docs/results/mtl_improvement/cat_transfer_and_T53.md`, `WHY_ORTHOGONAL_AND_NO_MODERN_OPTIMIZERS.md`.
...
| `fusion_mode=aux` | **the key lever** | `aux` fuses as `feat = priv + β·aux_proj(shared)` (β learnable, init 0.1) — it ADDS the shared pathway as a non-attenuating residual. The default `gated` makes private/shared *compete* (a convex gate) and thereby DILUTES the private reg signal → `gated` lands at 73.06 vs `aux` 73.57. **Do not switch to `gated` or any multiplicative/competitive fusion — it regresses the ceiling-break.** ⚠ **2026-06-12 (HANDOFF_AUDIT X3) nuance:** β is logged to **decay to ≈0 by ~epoch 20 — by gradient, NOT weight decay** (re-run with β in a zero-WD group still drives β→0, mean final −0.0001; FL reg/cat null Δ +0.015/+0.139). So the model *elects to gate the shared pathway off* — `aux` is the right fusion precisely because it LETS reg do that (protect the private signal), whereas `gated` forces dilution. The "key lever" is the non-competitive form, not a positive shared contribution. See `docs/results/mtl_improvement/X_SERIES_FINDINGS.md` §X3. |
| `freeze_alpha=True alpha_init=0.0` | **prior-OFF** | the in-head additive α·log_T prior is a biased logit term that HURTS top-K. Turning it off (hard 0, frozen) gains ~+1.4 pp reg. Keep it off. (Soft log_T-KD distillation is a *different* mechanism; after the aux-gate fix it was genuinely tested on the dual-tower and adds nothing — see §5.) |
...
| **KD** | `--log-t-kd-weight 0.0` | KD off (the v12 default is 0.2). The earlier "KD doesn't help the dual-tower" evidence was a DEAD CODEPATH (the dualtower head was missing from the aux gate → the `g_kd0.1/0.2` arms were no-ops). **Aux gate FIXED + KD-on-G genuinely tested 2026-06-12 (HANDOFF_AUDIT X2): NULL** — KD W=0.2 gives FL reg +0.05 / AL reg −0.13 (both ≪ the 0.3pp promote gate) and FL cat −0.57. KD off stays correct. See `CODE_AUDIT_2026-06-12.md` P0-B + `docs/results/mtl_improvement/x_series_results.json`. |
...
| **Re-introduce the α·log_T prior (additive)** | DON'T | it is a biased logit term that hurts top-K on the single-pathway heads; G freezes it off. ⚠ **2026-06-12 (code audit P0-B + X2):** the old "soft log_T-KD `0.1/0.2` tested on the dual-tower — IDENTICAL to G" line was a **dead-codepath artifact** — `next_stan_flow_dualtower` was missing from `_HEADS_REQUIRING_AUX_MTL` (`folds.py:933-937`), so `get_current_aux()` was None and the KD branch never executed. **FIXED + re-tested (HANDOFF_AUDIT X2): KD-on-G is genuinely NULL** (FL reg +0.05 / AL reg −0.13 ≪ 0.3pp gate, FL cat −0.57) — so the "adds nothing" verdict now stands, but on a *real* test rather than a no-op. |
