...
| **F48-H3** | **P1** | **Per-head LR (cat=1e-3 constant, reg=3e-3 constant) via multi-param-group AdamW** | 2b | m4\_pro | ~50 LOC dev + 30min train AL+AZ | Teste mais limpo da hipótese "regimes disjuntos". Se funciona, paper tem receita: "per-head OneCycleLR com `next_gru` em peak gentle e `next_getnext_hard` em sustained max". `research/F44_F48_LR_REGIME_FINDINGS.md §Next-step`. |
