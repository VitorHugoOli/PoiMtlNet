...
| **F48-H3-alt** | **🎯 done 2026-04-25 — CONFIRMED 🎯** | **Per-head LR INVERSO (cat=1e-3, reg=3e-3, shared=1e-3 — todos constant)** | 2b | m4\_pro | ~30min train AL+AZ | **PAPER RECIPE.** AL cat F1=**42.22 ± 1.00** (≈ B3 42.71), reg Acc@10=**74.62 ± 3.11** (EXCEDE STL ceiling 68.37 por **+6.25 pp**). AZ cat F1=**45.11 ± 0.32** (≈ B3 45.81), reg Acc@10=**63.45 ± 2.49** (fecha **75% do gap** STL-B3 [53.82 → 66.74]). **Mecanismo confirmado:** α (em `next_getnext_hard.head`, grupo reg) cresce com sustained 3e-3 → reg lift; shared cross-attn a 1e-3 protege cat path; cat encoder a 1e-3 mantém estável. Single-line additive recipe: `--scheduler constant --cat-lr 1e-3 --reg-lr 3e-3 --shared-lr 1e-3`. Launcher: `scripts/run_f48_h3alt_per_head_lr.sh`. |
