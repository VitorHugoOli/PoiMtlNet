...
**Status (2026-04-28, post-F37 closing):** **Three tracks at paper-grade — and the headline narrative is now scale-conditional.**
...
2. **F49 architecture attribution** (AL+AZ+FL n=5): **The H3-alt reg lift on AL is architecture-dominant** (+6.48 ± 2.4 pp from architecture alone, ~2.7σ). **Cat-supervision transfer is null on all 3 states** (≤|0.75| pp). Refutes legacy "+14.2 pp transfer" claim by ≥9σ on FL n=5 alone. Layer 2 methodological contribution: loss-side `task_weight=0` ablation is unsound under cross-attention MTL. See [`research/F49_LAMBDA0_DECOMPOSITION_RESULTS.md`](research/F49_LAMBDA0_DECOMPOSITION_RESULTS.md).

3. **F37 FL closure (2026-04-28) — narrative-changing.** STL `next_gru` cat 5f on FL = 66.98 ± 0.61 (MTL > STL by **+0.94 pp** ✓ — cat-side claim survives at scale). STL `next_getnext_hard` reg 5f on FL = **82.44 ± 0.38**, beating MTL H3-alt FL by **−8.78 pp paired Wilcoxon p=0.0312, 5/5 folds negative**. F49 Layer 3 closes with architectural Δ FL = **−16.16 pp p=0.0312**. The architectural lift on AL **does not generalise to FL scale**. CH18 reframed scale-conditional; CH21 reframed per-state; C15 re-opened with FL caveat. See [`research/F37_FL_RESULTS.md`](research/F37_FL_RESULTS.md).

The three findings give a per-state pattern: **AL** (architecture-dominant joint MTL lift on both heads) → **AZ** (classical: architecture costs reg, partial recovery) → **FL** (substrate-only joint lift; architecture costs reg heavily; matched-head STL ceiling above MTL on reg). The cat-side MTL > STL relation generalises across scale (+0.94 to +3.64 pp). The architecture-side reg lift is **AL-only** at our 3 states. Champion config (F48-H3-alt per-head LR) is unchanged in [`NORTH_STAR.md`](NORTH_STAR.md); end-to-end derivation in [`MTL_ARCHITECTURE_JOURNEY.md`](MTL_ARCHITECTURE_JOURNEY.md); paper deliverables in [`PAPER_PREP_TRACKER.md`](PAPER_PREP_TRACKER.md). Most recent operational state: [`SESSION_HANDOFF_2026-04-27.md`](SESSION_HANDOFF_2026-04-27.md) + [`research/F37_FL_RESULTS.md`](research/F37_FL_RESULTS.md).
