...
- **`run_northstar_category_variants.py`** — north-star eval driver (next-CAT via `next_gru --target category`, next-REG via `next_getnext_hard --input-type region`). Writes `docs/studies/check2hgi/results/P1/region_head_arizona_cat*_*.json`.
...
- **North-star fold-level metrics:** `docs/studies/check2hgi/results/P1/region_head_arizona_cat<X>_{checkin,region}_5f_30ep_NS_AZ_cat<X>_{nextcat,nextreg}_5f30ep.json`
- **North-star aggregated JSON:** `docs/studies/check2hgi/results/paired_tests/HGI_CATEGORY_NORTHSTAR.json`
