...
**Date:** 2026-04-28. **Tracker:** see `FOLLOWUPS_TRACKER.md` §1 (F33 done; F50 sub-rows). **Status:** **Tier 0 + T1.1 + T1.2 STL + T1.2-MTL n=5 + T1.3 + T1.4 ALL DONE 2026-04-29** (`F50_T1_RESULTS_SYNTHESIS.md`); F33 PASS + all Tier-1 alternatives FAIL the +3 pp acceptance threshold; H3-alt champion confirmed. **NEW Tier 1.5 (cheap probes) launched 2026-04-29** — minimal-edit ablations of the cross-attn mechanism (P1/P2/P3/P4) before Tier 2 dev. Tier 2 (Cross-Stitch + PLE) implementation in parallel. **For pickup state see `F50_HANDOFF_2026-04-28.md` (superseded by `F50_T1_RESULTS_SYNTHESIS.md` for T1 results).** **Predecessor docs:** `F49_LAMBDA0_DECOMPOSITION_RESULTS.md`, `F37_FL_RESULTS.md`, `SUBSTRATE_COMPARISON_FINDINGS.md`. **Cross-refs:** `PAPER_PREP_TRACKER.md` §2, `PAPER_DRAFT.md` §1-2, `CONCERNS.md` §C12 §C13 §C14 §C15, `MTL_FLAWS_AND_FIXES.md` §3 (H1 closed).
...
| **T1.2** | Hierarchical softmax reg head | ✅ **DONE 2026-04-29** (n=5) | dev ~2 h; STL ~30 min MPS; MTL ~30 min CUDA | STL HSM PASS (82.64 ± 0.42 vs flat 82.44, p=0.0312). MTL HSM at full n=5 (CUDA): reg top10 = 70.60 ± 10.78 vs CUDA H3-alt 73.61 ± 0.83 → **Δ = −3.01 pp, FAIL** (W+=10, p_greater=0.3125). Fold-2 ep-3 collapse outlier; even dropping fold 2, mean Δ ≈ +1.66 pp, still below +3 pp acceptance. Head-capacity-mismatch hypothesis refuted at paper grade. See `F50_T1_RESULTS_SYNTHESIS.md`. |
| **T1.3** | FAMO drop-in | ✅ **DONE 2026-04-29** (n=5, CUDA) | ~19 min on RTX 4090 | reg top10 = 74.23 ± 0.81 vs CUDA H3-alt 73.61 ± 0.83 → **Δ = +0.62 pp, FAIL** (W+=11, p_greater=0.2188). Cat F1 tied within σ. Magnitude-balancing does not recover FL. See `F50_T1_RESULTS_SYNTHESIS.md`. |
| **T1.4** | Aligned-MTL drop-in | ✅ **DONE 2026-04-29** (n=5, CUDA) | ~19 min on RTX 4090 | reg top10 = 73.50 ± 0.41 vs CUDA H3-alt 73.61 ± 0.83 → **Δ = −0.11 pp, FAIL** (W+=4, p_greater=0.8438). Cat F1 regressed −0.90 pp. Direction-alignment does not recover FL. See `F50_T1_RESULTS_SYNTHESIS.md`. |
| **T1.5 (NEW)** | Cross-attn mechanism probes (P1 no-cross-attn / P2 detach-K/V / P3 cat-freeze / P4 separate-optimizers) | 🔵 **IN FLIGHT** 2026-04-29 | ~80 min train (4 × 19 min) + ~6 h dev | Minimal-edit ablations of `MTLnetCrossAttn` to test the F49 Layer 2 mechanism under full-MTL conditions. Inserted between Tier 1 (loss/head fixes) and Tier 2 (PLE / Cross-Stitch dev) because they fail-fast at much lower cost. See §5.5 below + `MTL_FLAWS_AND_FIXES.md` §3 H1.5. |
