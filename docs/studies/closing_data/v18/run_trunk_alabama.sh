...
OUT=$BASE/logs/trunk; SIDE=docs/results/closing_data/v18_trunk
mkdir -p "$OUT" "$SIDE"
DRV=$BASE/logs/trunk_DRIVER.log
...
for f in sorted(glob.glob(f"{rd}/metrics/fold*_next_category_val.csv")):
    ...
    pf.append(v[bi])
json.dump({"tag":tag,"state":"alabama","seed":0,"model_param":mp,"rundir":rd,
...
log "===== v18 TRUNK arms at alabama (baseline champion-G: cat 27.3836 / reg 69.6831) ====="
arm T1_A_nosharing  "--model-param disable_cross_attn=True"
arm T2_Aprime_nomix "--model-param identity_cross_attn=True"
