#!/usr/bin/env bash
# v18 SWEEP_PLAN rows 3 + 7 for ONE state:  SWEEP_STATE=<state> bash run_mtl_state.sh
# The cw0.75 / class-weights-OFF cell IS the row-3 anchor and is deliberately not repeated in row 7.
#
...
ENG=check2hgi_v18; V14=check2hgi_design_k_resln_mae_l0_1; ST=${SWEEP_STATE:?set SWEEP_STATE}; SD=0
BASE=docs/studies/closing_data/v18
OUT=$BASE/logs/mtl_$ST; SIDE=docs/results/closing_data/v18_sweep
mkdir -p "$OUT" "$SIDE"
DRV=$BASE/logs/mtl_${ST}_DRIVER.log
...
mtl(){ # tag catlr catweight epochs extra_flags extra_json
  ...
  env PYTHONPATH=src MTL_DISABLE_AMP=1 MTL_CHUNK_VAL_METRIC=1 MTL_STRICT=1 MTL_COMPILE_DYNAMIC=1 \
      ...
      TORCHINDUCTOR_CACHE_DIR="$HOME/.inductor_v18mtl_$ST" \
...
import json,sys,glob,csv,os,statistics as stx
tag,rd,extra,wall,side=sys.argv[1:6]
ST_ENV=os.environ.get("SWEEP_STATE","alabama")
...
for f in sorted(glob.glob(f"{rd}/metrics/fold*_next_category_val.csv")):
    ...
    pf.append(v[bi])
d={"tag":tag,"arm":"mtl","state":ST_ENV,"seed":0,"wall_seconds":int(wall),"rundir":rd,
...
log "===== v18 MTL rows 3+7 at $ST ====="

# --- row 3: category-head LR + schedule length -------------------------------------------------
mtl "mtl${ST}_r3_catlr0.0005_ep50" 0.0005 0.75 50 "--no-cat-class-weights" '{"row":3,"cat_lr":0.0005}'
mtl "mtl${ST}_r3_catlr0.001_ep50"  0.001  0.75 50 "--no-cat-class-weights" '{"row":3,"cat_lr":0.001,"anchor":true}'
mtl "mtl${ST}_r3_catlr0.002_ep50"  0.002  0.75 50 "--no-cat-class-weights" '{"row":3,"cat_lr":0.002}'
mtl "mtl${ST}_r3_catlr0.001_ep25"  0.001  0.75 25 "--no-cat-class-weights" '{"row":3,"cat_lr":0.001,"epochs":25}'
...
mtl "mtl${ST}_r7_cw0.75_catwON"  0.001 0.75 50 "--cat-class-weights"    '{"row":7,"cat_class_weights":true,"category_weight":0.75}'
mtl "mtl${ST}_r7_cw0.50_catwOFF" 0.001 0.50 50 "--no-cat-class-weights" '{"row":7,"cat_class_weights":false,"category_weight":0.50}'
mtl "mtl${ST}_r7_cw0.50_catwON"  0.001 0.50 50 "--cat-class-weights"    '{"row":7,"cat_class_weights":true,"category_weight":0.50}'

log "===== MTL rows 3+7 at $ST COMPLETE ====="
