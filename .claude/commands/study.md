...
claim-driven ablation described in `docs/studies/`.
...
5. Never edit `docs/studies/state.json` by hand — always go through the CLI so writes stay atomic.

## Subcommands

| Form | What it does |
|------|---|
| `/study init [--force]` | Write initial `docs/studies/state.json` skeleton (P0 running, P1–P5 planned). Refuses to overwrite without `--force`. |
| `/study status` | Summarize current phase, per-phase test counts, and open issues. |
| `/study validate --state <s> --engine <e> [--cross <e2> <e3>...]` | Run parquet integrity checks; writes JSON to `docs/studies/results/P0/integrity/`. |
...
| `/study import --run-dir <path> --phase <P> --test-id <id> [--claims C01 C02] [--overwrite]` | Archive a training run (from this or another machine) into `docs/studies/results/<phase>/<test_id>/` and update state. |
...
- Claim IDs come from `docs/studies/CLAIMS_AND_HYPOTHESES.md` (C01..Cnn).
