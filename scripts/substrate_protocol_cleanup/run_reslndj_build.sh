#!/usr/bin/env bash
# tier_resln (Design J branch) Stage 2+3 — build 3 ResLN+Design J substrates +
# postbuild glue. Mirrors run_resln_build.sh exactly.
#   ResLN+Design J (check2hgi_resln_design_j) @ AL/AZ/FL
# 500 epochs, --device cuda, seed=42, anchor-lambda 0.1. AL/AZ ~6 min, FL ~30-40 min.
# Postbuild: next.parquet + next_region.parquet + cp canonical seed42 log_T
# (touch after parquet, C22 mtime guard).
...
build_design_j() {  # state
  local state="$1"
  echo "[$(date -u +%H:%M:%S)] BUILD resln-design_j $state START" >> "$LOG/build.log"
  $PY scripts/probe/build_design_j_anchor.py \
      --state "$state" --epochs 500 --num-layers 2 --device cuda \
      --anchor-lambda 0.1 --seed 42 \
      --encoder resln --out-engine check2hgi_resln_design_j \
      >> "$LOG/build_reslndj_${state}.log" 2>&1
  echo "[$(date -u +%H:%M:%S)] BUILD resln-design_j $state EXIT rc=$?" >> "$LOG/build.log"
}
...
# ---- AL/AZ in parallel (small, fast) ----
for state in alabama arizona; do
  guard; build_design_j "$state" &
done
wait
postbuild check2hgi_resln_design_j alabama
postbuild check2hgi_resln_design_j arizona

# ---- FL serially (large) ----
guard; build_design_j florida
postbuild check2hgi_resln_design_j florida
