#!/usr/bin/env bash
# tier_resln Stage 4 — MTL runs (seed=42, 5-fold, --no-checkpoints).
# Cells (engine|state|tag):
#   ResLN-canonical, ResLN+design_b, canonical(no-ResLN) baseline  ×  AL/AZ/FL
# Recipe: AL/AZ = H3-alt (constant sched, no alternating/alpha-no-wd/min-best);
#         FL = B9 (alternating, alpha-no-wd, min-best 5, cosine max-lr 3e-3).
# Common: static_weight cat-weight 0.75, mtlnet_crossattn, cat-head next_gru,
#         reg-head next_getnext_hard, task_a=checkin task_b=region,
#         cat/reg/shared-lr 1e-3/3e-3/1e-3, batch 2048.
# Output: tier_resln/<tag>/<state>/seed42/ (+ metrics snapshot copied in).
# Detached; GPU>=10GB + disk>=4GB guard; concurrency cap 1 (GPU contended).
set -u
...
CELLS=(
  "check2hgi_resln|alabama|resln_canonical"
  "check2hgi_resln|arizona|resln_canonical"
  "check2hgi_resln|florida|resln_canonical"
  "check2hgi_resln_design_b|alabama|resln_design_b"
  "check2hgi_resln_design_b|arizona|resln_design_b"
  "check2hgi_resln_design_b|florida|resln_design_b"
  "check2hgi|alabama|canonical_noresln"
  "check2hgi|arizona|canonical_noresln"
  "check2hgi|florida|canonical_noresln"
)

for spec in "${CELLS[@]}"; do
  IFS='|' read -r engine state tag <<< "$spec"
  # Skip if substrate inputs not yet built (postbuild incomplete).
