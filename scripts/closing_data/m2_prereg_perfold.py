...
Pre-registration: analysis_protocol/STATISTICAL_PROTOCOL.md
...
(analysis_protocol/EXECUTED_ANALYSIS.md LIMITS #2). Those four sidecars were recovered from the A40 on 2026-07-25 and
are shipped under analysis_protocol/istanbul_cat_ceiling_perfold/, so the COMPLETE six-dataset
family runs for the first time. The paper reports the seed-level paired t (analysis_protocol/DEVIATION_LOG.md, D-1 and D-2);
this script is the registered test reported alongside it as corroboration.
...
Reads the per-fold score files produced by the training recipes of README section 5 (not shipped;
see "What is NOT included"). The Istanbul dedicated-category per-fold arrays ARE shipped, at
analysis_protocol/istanbul_cat_ceiling_perfold/, because the registered test depends on them.
"""
import json
import os
...
# Not shipped (README, "What is NOT included"): regenerated by the recipes of README section 5,
# then scored with score_joint_best.py. Set RESULTS_ROOT to point at a tree that holds them.
RESULTS_ROOT = Path(os.environ.get("RESULTS_ROOT", REPO))
J1 = RESULTS_ROOT / "docs/studies/closing_data/joint_best/data/j1_results.json"
CATX = RESULTS_ROOT / "docs/results/closing_data/catx_v17_n20/joint_best"
SWEEP = RESULTS_ROOT / "docs/studies/closing_data/v17_completion/cat_ceiling_sweep/sweep_results"
IST_CEIL = REPO / "analysis_protocol/istanbul_cat_ceiling_perfold"
P1 = RESULTS_ROOT / "docs/results/P1"

# Dedicated category ceiling arms (analysis_protocol/CEILINGS_N20_FINAL.md; per-state max over the sweep).
...
# Reported cells to gate against (analysis_protocol/{CEILINGS_N20_FINAL,JOINT_BEST_RESULTS}.md).
BOARD = {
