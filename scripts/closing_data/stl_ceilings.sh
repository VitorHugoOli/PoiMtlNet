#!/bin/bash
# FL STL (single-task) ceilings — the dedicated-model comparison points for the joint model.
...
# Then Δcat / Δreg vs the committed FL joint-model result. Serial on a single GPU.
set -uo pipefail
REPO="$(cd "$(dirname "$0")/../.." && pwd)"; cd "$REPO"; export PYTHONPATH=src
PY=.venv/bin/python
...
L=/tmp/a40_board; mkdir -p "$L" results/closing_data
MTL_JSON=results/closing_data/fl_s0_matched_score.json   # the seed-0 FL MTL matched-score JSON (Section 5.4 scorer output)
...
out={"state":"florida","seed":0,"folds":5,"engine":"check2hgi_dk_ovl",
     ...
     "cat_rundir":cat_rd}
open("results/closing_data/fl_ceilings_s0.json","w").write(json.dumps(out,indent=2))
print("  wrote results/closing_data/fl_ceilings_s0.json")
PY
