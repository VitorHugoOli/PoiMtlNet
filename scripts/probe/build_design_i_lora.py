"""Design I — LoRA-style low-rank correction on top of frozen POI2Vec residual.

Mixes B (frozen Linear(POI2Vec)) with H (learnable per-POI table) at much
lower parameter cost.

    poi_for_reg = poi_canon.detach()
                  + γ · ( Linear(POI2Vec)        # frozen prior, B
                          + U[poi] · V )          # rank-r learnable delta

U: [N_pois, r], V: [r, D]. Total extra params: N·r + r·D + 64·D.
For r=8, D=64, N=11k AL: ~92k (vs H's 758k, B's 4k).

Cat path identical to canonical c2hgi (encoder unchanged, .detach() before
adding residual). Reg path sees the enriched POI embeddings.

Usage::

    python scripts/probe/build_design_i_lora.py --state alabama --epochs 500
"""
...
class Check2HGI_DesignI(Check2HGI):
    def __init__(self, *args, poi2vec_table: torch.Tensor, num_pois: int,
                 lora_rank: int = 8, gamma_init: float = 1.0, **kwargs):
        super().__init__(*args, **kwargs)
        D = self.hidden_channels
        self.poi2vec_proj = nn.Linear(POI2VEC_DIM, D, bias=True)
        self.lora_U = nn.Parameter(torch.zeros(num_pois, lora_rank))
        nn.init.normal_(self.lora_U, std=0.01)
        self.lora_V = nn.Parameter(torch.zeros(lora_rank, D))
    ...
    def forward(self, data):
        ...
        frozen_residual = self.poi2vec_proj(self.poi2vec_table)
        lora_delta = self.lora_U @ self.lora_V
        residual = frozen_residual + lora_delta
...
def load_poi2vec_table(state: str, num_pois: int, placeid_to_idx: dict) -> torch.Tensor:
    ...
    if n_unmapped > 0:
        print(f"  WARN {n_unmapped} POIs missing POI2Vec — left zero")
...
def train(state: str, args):
    ...
    out_dir = REPO / "output" / f"check2hgi_design_i{suffix}" / state_lc
    ...
    num_checkins = d["num_checkins"]; num_pois = d["num_pois"]; num_regions = d["num_regions"]
    print(f"[{state_lc}] checkins={num_checkins} pois={num_pois} regions={num_regions} "
          f"feat={in_channels} (canonical 11-dim)")
    ...
    model = Check2HGI_DesignI(
        ...
        poi2vec_table=poi2vec_table, num_pois=num_pois,
        lora_rank=args.lora_rank, gamma_init=args.gamma_init,
    ).to(device)
    n_lora = args.lora_rank * (num_pois + args.dim)
...
def main():
    ...
    ap.add_argument("--lora-rank", dest="lora_rank", type=int, default=8)
