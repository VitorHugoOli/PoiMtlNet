"""Design J — Design H with anchor regularization to POI2Vec.

Same architecture as Design H (learnable per-POI table at POI-pool, warm
start from POI2Vec, cat path detached). Adds an L2 anchor loss that pulls
the learnable POI table toward POI2Vec each step:

    L_total = L_c2hgi  +  λ · ‖ E_h − POI2Vec ‖²

Forces the table to stay near fclass clusters. Tests whether the contrastive
drift in H is helpful or harmful.

Usage::

    python scripts/probe/build_design_j_anchor.py --state alabama --epochs 500 --anchor-lambda 0.1
"""
...
class Check2HGI_DesignJ(Check2HGI):
...
def train(state: str, args):
    ...
    print(f"[{state_lc}] pois={num_pois} regions={num_regions} feat={in_channels} λ={args.anchor_lambda}")
