...
man = {}  # tag -> {seed: rundir}
for line in (REPO / "scripts/mtl_frontier/ccplus_fl_multiseed_manifest.tsv").read_text().splitlines():
    p = line.split("\t")
    if len(p) >= 4:
...
out = {"comparand": "matched same-batch champion G (cond=none, KD-off), FL", "configs": {}}
...
outp = REPO / "docs/results/mtl_frontier/ccplus_fl_multiseed_results.json"
