"""Next-POI head training pipeline — train next task via scripts/train.py. Usage: python pipelines/train/next_head.pipe.py"""
...
def process_state(name: str, state_cfg: dict) -> bool:
    """Train next-POI head for a single state via scripts/train.py."""
    try:
        state_cfg = dict(state_cfg)
        base = dict(state_cfg.pop('config', CONFIG))
        base.update(state_cfg)

        cmd = [sys.executable, _train, "--state", name, "--engine", base['engine'], "--task", "next"]
...
def run_pipeline():
    ...
    logger.info(f"Next-POI Training Pipeline - {len(STATES)} state(s) | engine={CONFIG['engine']}")
