...
Usage:
    python scripts/train.py --task next --state alabama --engine check2hgi

This wrapper will be removed in a future release.
"""
import warnings
warnings.warn(
    "pipelines/train/next_head.pipe.py is deprecated. Use scripts/train.py directly.",
    DeprecationWarning,
    stacklevel=1,
)
import sys
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(description="Next-POI training pipeline (thin wrapper)")
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    for state in args.state:
        for engine in args.engine:
            cmd = [sys.executable, _train, "--state", state, "--engine", engine, "--task", "next"]
