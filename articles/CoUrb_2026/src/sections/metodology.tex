...
\begin{figure*}[htbp]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1\textwidth]{imagens/arquitetura_modelo.png}}
...
Padrões de mobilidade humana apresentam regularidades cíclicas, como horários de refeição e deslocamentos semanais \cite{cho2011friendship}, e modelos da próxima visita as leem por meio de representações temporais explícitas \cite{sun2020go}. No entanto, o modelo original MTLNet não incorpora representações temporais explícitas para capturar essas informações. 
% [2026-07-30, dissertacao PENDENCIAS 2.15, decisao do autor: caminho A] ALEGACAO ESTREITADA nesta
% fonte publicada, por instrucao do autor de corrigir os artigos originais e registrar cada ponto no
% apendice de errata da dissertacao. O PDF publicado de registro (DOI 10.5753/courb.2026.22960) NAO e
% editado e continua como esta; esta e a fonte de trabalho.
% A frase atribuia a sun2020go que as regularidades ciclicas carregam informacao discriminativa sobre
% a NATUREZA FUNCIONAL dos POIs, ou seja, que o sinal temporal revela a SEMANTICA do lugar. O
% sun2020go e o LSTPM (AAAI 2020, DOI 10.1609/aaai.v34i01.5353), que modela preferencia de USUARIO de
% curto e longo prazo para recomendacao do proximo POI, e nao afirma nada sobre sinal temporal prever
% a funcao do lugar. A metade da regularidade temporal e apoiada diretamente por cho2011friendship
% (KDD 2011, DOI 10.1145/2020408.2020579), chave que JA esta neste .bib (linha 59) e ja e citada em
% sections/intro.tex:1, portanto nenhuma entrada e adicionada. A metade semantica nao tem apoio em
% nenhuma entrada desta bibliografia e por isso foi REMOVIDA, nao recitada. O argumento do paragrafo
% nao muda: o MTLNet carece de representacao temporal explicita, que e o que o ST-MTLNet acrescenta.
...
Sobre esse grafo são executadas caminhadas aleatórias (\textit{random walks}) seguindo a metodologia do Node2Vec \cite{grover2016node2vec}. Cada caminhada é convertida em uma sequência de categorias secundárias, as classes finas, resultando em coocorrências locais e globais entre categorias.
% [2026-07-30, dissertacao PENDENCIAS 2.15, decisao do autor: caminho A] TERMO BANIDO REMOVIDO. O
% GLOSSARY da dissertacao registra o nome voltado ao leitor como "fine class" ("classe fina" em PT) e
% proibe o identificador de codigo `fclass` em prosa.
O modelo aprende os \textit{embeddings} utilizando a estratégia \textit{skip-gram} com \textit{negative sampling} \cite{church2017word2vec}:
...
na qual $\mathbf{e}_i$ é o \textit{embedding} da classe alvo, $\mathbf{e}_j$ o \textit{embedding} positivo de contexto, e $\mathbf{e}_{n_k}$ representam as amostras negativas. A implementação incorpora um termo de regularização hierárquica \cite{Xu2023} entre categoria e classe fina: $\mathcal{L}_{\text{hier}} = \sum_{(c,s) \in \mathcal{H}} \left\| \mathbf{e}_s - \mathbf{e}_c \right\|_2^2$, na qual $\mathcal{H}$ contém os pares (categoria, classe fina).
% [2026-07-30, dissertacao PENDENCIAS 2.15, decisao do autor: caminho A] CITACAO SUBSTITUIDA,
% belkin2003laplacian -> Xu2023, e o `fclass` banido trocado por "classe fina".
% O objeto citado e uma penalidade L2 que aproxima o embedding de uma subcategoria do embedding da
% categoria pai sobre uma arvore de rotulos conhecida. Laplacian eigenmaps (Neural Computation
% 15(6):1373-1396) e reducao de dimensionalidade nao linear, portanto nao sustenta o termo. O Xu2023 e
% o TME (ACM TOIS 41(4), DOI 10.1145/3582553), que "utilizes the predefined category hierarchy to
% regularize the relatedness among categories" (resumo, lido no Crossref em 2026-07-30). Xu2023 ja
% esta neste .bib na linha 281 e ja e citado em sections/related.tex, logo nenhuma entrada e criada.
A perda total é $\mathcal{L} = \mathcal{L}_{\text{SGNS}} + \lambda_{\text{hier}} \, \mathcal{L}_{\text{hier}}$. Ao final, o \textit{embedding} resultante é gerado por categoria e remapeado para cada POI.
