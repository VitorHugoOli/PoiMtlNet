...
the exact POI of the next visit. This is not one of your tasks, but serves as a based for how we attack theses problms.
The literrature for the next vist start with the
recurrent models: ST-RNN uses
...
trajectory-flow map into a transformer~\cite{yang2022getnext}. All of these methods
predict the exact next place, so they provide methodological background rather than
direct baselines for the targets studied here.
...
computation instead of forcing every task through one fixed trunk. The joint
model in Chapter~\ref{ch:mobiwac} follows this principle. It uses cross-attention
between task streams, rather than expert gates, to let each task read from the
other without merging their main pathways.
...
Mobility applications of MTL usually support next-place prediction. MCARNN jointly
...
co-equal end targets of one joint model. MTLnet begins this
dissertation's investigation with hard parameter sharing over a place embedding. It
does not outperform the dedicated single-task models for that configuration, which
motivates the representation-focused studies that follow~\cite{silva2025mtlnet}.
...
The two task metrics are reported separately. Their geometric mean is used only to
select a checkpoint for the joint model. Under this joint-best convention, both task
scores come from the same validation-selected model. The scores are then read against
task-specific reference points. The category floor always predicts the majority
class. A first-order Markov model over the training partition provides a sequential
floor~\cite{gambs2012mmc}. The potential predictability of about 93 percent for a
coarse next-location target~\cite{song2010limits} is not a ceiling for seven-class
macro-F1 or region ranking. The relevant comparison for the research question is the
dedicated single-task model for each output.
...
This section connects the preceding concepts to the sequence of studies. The
connection begins with the targets. Next place, next category, and next region have
different outputs and different measures of success. This dissertation treats next
category and next region as final outputs rather than as intermediate steps toward an
exact-place prediction.

\subsection{Conceptual connection}

The central connection is that task choice, representation, sharing, and evaluation
cannot be interpreted independently.

That choice places more pressure on the input representation. A place embedding can
encode semantic and geographic relationships, but it assigns the same vector to every
visit to a place. Check2HGI adds the visit as a separate level so that the input can
vary with the check-in. MTL then determines how the two prediction tasks use and share
that information. Hard sharing may cause negative transfer, while specialized
balancers may offer little benefit when the task gradients are nearly orthogonal.
The comparison must therefore be made against the dedicated single-task model for each
output.

The evaluation protocol supplies the remaining controls. User-disjoint
cross-validation limits user leakage. Macro-F1 and OOD-discounted Acc@10 describe
different outputs and are compared with explicit reference points. Paired superiority
tests support the verb \emph{outperforms}, while non-inferiority tests within a
two-point margin support \emph{matches}. These rules prevent an observed mean
difference from being reported as a conclusion that its test does not support.

\subsection{Questions carried into the studies}

The combined concepts yield one question for each stage of the research arc.

Together, the fundamentals produce three questions. Chapter~\ref{ch:cbic} asks
whether a place-level representation and hard parameter sharing are sufficient for a
joint model. Chapter~\ref{ch:courb} keeps the model fixed and asks whether a richer,
decomposed input changes the result. Chapter~\ref{ch:mobiwac} asks what a
check-in-level representation and a different sharing topology enable for two
sequential targets. Its joint model outperforms the dedicated model on next category
at all six datasets and on next region at Istanbul, Florida, Texas, and California.
At Alabama and Arizona, it matches the dedicated model by being statistically
non-inferior within a two-point margin. The following chapters provide the evidence
for these answers in that order.
