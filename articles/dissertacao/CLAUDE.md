...
- **v1 lives in [`src/`](src/)** — the single working copy; the draft folders (`storyline/`,
  ...
  files: `make defense` → `build/main.pdf` (**105 pp**), copied to
  [`src/dissertacao.pdf`](src/dissertacao.pdf), the banca PDF; `make final` →
  `build/main_final.pdf` (**102 pp**, the AcademicoPG deposit body); `make ppgc` →
  `build/main_ppgc.pdf` (**106 pp**, the defense document plus the approval sheet, from a two-line
