...
| **paired superiority test** | Paired *t* on the four per-seed means (inferential unit: n = 4, three degrees of freedom), which is the reported convention; the registered per-fold Wilcoxon signed-rank over the 20 fitted models (n = 20) is reported alongside it and agrees. Verdict verb: **"outperforms"**, licensed only where the test survives Holm correction within its task family: **next region at Texas and California, next category at Florida**. | verbs bound to tests (WRITING_LAW §3). At n = 4 the exact one-sided Wilcoxon floors at 0.0625 whatever the effect size, which is why the *t* carries the verdict; both footings and the departure are logged in `docs/studies/closing_data/log.md` D-1 to D-3 |
