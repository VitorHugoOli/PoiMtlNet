...
| **not resolved** | The verdict for a difference that fails superiority where **no equivalence margin is registered for that axis** — the five next-category cells outside Florida. Surface: **"the difference is not resolved"** (PT: **"a diferença não foi resolvida"**). | Never "matches", "empata", "semelhante", "a par", or any bare claim of equality. The magnitude statement available for these cells is the equivalence bound above, which is a claim about size and carries its direction with it |
...
| the difference is not resolved | a diferença não foi resolvida |
