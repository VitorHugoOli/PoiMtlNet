...
| **MTLnet** | The joint architecture of the CBIC paper: task-specific MLP encoders → FiLM modulation → shared residual trunk → task heads (category ensemble; Transformer next-head). | CBIC (Vitor) | Ch.3 (introduced), recapped Ch.4/Ch.5 related work |
...
| **ST-MTLNet** | CoUrb's variant: MTLnet unchanged, input replaced by concatenated spatial + temporal + categorical encoders (64-d each → 192-d). | CoUrb (Tarik 1st author) | Ch.4 |
