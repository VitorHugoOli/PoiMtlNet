...
| **Delaunay triangulation** (PT: *triangulação de Delaunay*) | The spatial graph the three infomax representations are built on: places are the vertices, and the triangulation joins nearby ones. **The construction is shared; the edge weights are not** — Ch. 3 weights by logarithmic decay of the geodesic distance (`3_cbic/method.tex` (a ponderacao por decaimento logaritmico da distancia geodesica)), Ch. 4 by logarithmic decay of the Haversine distance with a county-boundary penalty (`4_courb/methodology.tex` (o decaimento logaritmico da Haversine com penalizacao de fronteira)), and the delivered Ch. 5 prose says only that "nearby places are linked at the place level", the name appearing in Appendix E of the **main volume** (`apx_h_check2hgi_joint_model.tex:80`). Never assert that the three graphs are the same object. | Already live in the delivered prose of Chs. 3 and 4 and in Ch. 2 (`2_fundamentals.tex:442`), and already printed on four deck slides without a registry line. Registered 2026-08-25 by the author, to regularize those and to license one shared line in the fundamentals slide |
