...
topology (Chapter~\ref{ch:mobiwac}). Under this design, one joint model holds its own against two
...
Arizona, and Florida it stays within the registered two-point margin. Three of those four
are small deficits that a test in the reverse direction resolves, the largest being Alabama
at $-0.87$, so they are reported as differences inside the margin rather than as ties. On
...
training protocol fixed. At Alabama and California the wider model scores below the narrower one it
replaces, so the added capacity brings no gain on this task. Capacity is therefore not the variable
next-category prediction responds to at these two datasets, which leaves the representation as the
