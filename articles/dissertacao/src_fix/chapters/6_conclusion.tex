...
topology (Chapter~\ref{ch:mobiwac}). Under this design, one joint model stays within the
registered margin of two dedicated single-task models on both tasks. On the region task it outperforms
...
other five differences are not resolved, because the equivalence margin was registered for
the region axis only, and all six are equivalent to zero within half a point.
