...
\begin{itemize}
  \item A check-in-level representation that extends hierarchical graph infomax from the place to the individual visit, improving next-category prediction over a standard place embedding at every one of the six datasets, by $+0.23$ to $+6.29$ points of macro-F1 (a balanced score that counts every
    category equally), with every fold favoring it at every dataset (Table~\ref{tab:mobiwac:representation}). The direction is
