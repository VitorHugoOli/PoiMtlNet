...
# B4. An abstract noun as the agent of a verb of motion or volition. A motion preposition is
# required after "point(s)" so that "a margin of three points" and "two points of macro-F1", both
# ordinary noun uses in this document, do not match.
_ABSN = (r"(?:decline|increase|decrease|rise|fall|drop|gain|loss|gap|margin|cosine|slope|trend|"
         ...
         r"difference|effect|value|series|curve|distribution|departure|change)")
_AGENTV = (r"(?:points?|pointing|pointed)\s+(?:away|toward|towards|back)|"
           r"(?:moves?|moving|moved|travels?|travelling|traveling|walks?|heads?|heading|drifts?|"
           r"drifting|climbs?|climbing|refuses?|wants?|prefers?|admits?|concedes?|insists?|"
           r"agrees?|disagrees?|knows?|believes?|decides?|chooses?|cares?|worries|hopes?|tells?|"
           r"speaks?|listens?|watches)\b")
SHAPE_ABSTRACT_AGENT = (
    "abstract noun as the agent of a motion or volition verb",
    rf"\b(?:the|a|an|its|their|this|that|both|one|two|each)\s+{_ABSN}s?\s+"
    rf"(?:[a-z]+\s+){{0,2}}?(?:{_AGENTV})",
    'the author\'s instance: "the decline stays inside the margin throughout while moving toward '
    'zero rather than away from it". A number does not move by itself; say what it does: "the '
    'mean falls toward zero and stays inside the margin".',
)
...
OPEN_REGISTER: tuple[tuple[str, str, str, str], ...] = (
    ("chapters/apx_f_cosine.tex", "needs saying plainly",
     "the Appendix F track (PENDENCIAS 2.22 point 8)",
     "the author's own instance of (a); that track rewrites the sentence"),
    ("chapters/apx_f_cosine.tex", "Two departures from that flat picture appear",
     "the Appendix F track (PENDENCIAS 2.22 point 9)",
     "the author's own instance of (b); that track rewrites the sentence"),
    ("chapters/apx_f_cosine.tex", "Both point away from trouble in any case",
     "the Appendix F track (PENDENCIAS 2.22 point 12)",
     "the author's own instance of (b); that track rewrites the sentence"),
    ("chapters/apx_f_cosine.tex",
     "the decline stays inside the margin throughout while moving toward zero",
     "the Appendix F track (PENDENCIAS 2.22 point 12)",
     "the author's own instance of (b); that track rewrites the sentence"),
    ("chapters/3_cbic/conclusion.tex", "biased towards the features required",
     ...
     "in tables/cbic/errata_wording.tex, the same class as the six wording rows already there. "
...
def self_test() -> None:
    ...
    assert re.search(SHAPE_ABSTRACT_AGENT[1], b3, re.I), (
        "self-test: the author's instance of the abstract-agent shape did not fire")
    assert len(SHAPE_CHAINED_QUALS.findall(b3)) >= CHAIN_MIN_QUALS and b3.count(",") >= CHAIN_MIN_COMMAS, (
        "self-test: the author's instance of the chained-qualification shape did not fire")
