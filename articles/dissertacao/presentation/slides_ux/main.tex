...
% --- fontes: Petrona (títulos) + IBM Plex Sans (corpo) + IBM Plex Mono (tabelas/código) ---
% Nomes de arquivo SEMPRE explícitos -- "*" no fontspec substitui pelo nome da família COM
% espaço, que não bate com os .ttf (sem espaço) em fonts/.
...
\newfontfamily\titlefont{Petrona SemiBold}[
    Path=fonts/,
    UprightFont=Petrona-SemiBold.ttf,
    BoldFont=Petrona-Bold.ttf,
]
...
% Só a FAMÍLIA aqui -- o corpo de cada elemento vem do mockup, via \dcfontsize no nesped.sty
% (\setbeamerfont mescla atributos, então tamanho e família convivem). Sem \bfseries: o
% mockup usa Petrona 600, que é justamente o upright de \titlefont (Petrona SemiBold);
% pedir negrito aqui puxaria o Petrona-Bold (700), mais pesado que o desenho.
\setbeamerfont{title}{family=\titlefont}
\setbeamerfont{frametitle}{family=\titlefont}
% Use \codefont dentro de tabelas de números para colunas de dígito alinhadas, ex.:
%   {\codefont \begin{tabular}{lrrr} ... \end{tabular}}

% TODO: título exatamente como no folha de rosto entregue (src/) — fala em PT, slides em EN (§0).
% O título da folha de rosto é uma frase só, com dois pontos; na capa ele se parte nos três
% slots do mockup: rótulo (kicker), título e subtítulo. O texto entregue não muda.
\kicker{Defesa de Dissertação de Mestrado}
\title[Título curto]{Multitask Learning for Point-of-Interest Classification and Prediction Tasks}
