    ...
    \begin{columns}[t,onlytextwidth]
        \begin{column}{0.468\textwidth}
            \begin{block}{Practical --- what it delivers}
                ...
                \begin{itemize}\setlength{\itemsep}{4pt}\setlength{\parskip}{0pt}
                    \item \textbf{one model, one forward pass, two predictions} --- the next
                          category and the next region of a visit;
                    \item \textbf{one artifact} to train, version and deploy, in place of two ---
                          \alert{operational, not computational}: the joint model is the larger
                          artifact;
                    \item \textbf{Check2HGI is the reusable part} --- independent of the
                          prediction heads on top.
                \end{itemize}
            \end{block}
        \end{column}
        \hfill
        \begin{column}{0.468\textwidth}
            \begin{exampleblock}{Scientific --- what it establishes}
                \small
                \begin{itemize}\setlength{\itemsep}{4pt}\setlength{\parskip}{0pt}
                    \item \textbf{the novelty is the combination}: per-visit context inside a
                          hierarchical graph-infomax representation;
                    \item helps \alert{under the final design and evaluation protocol}, not always;
                    \item the \textbf{input representation} is one condition; architecture and
                          scale \emph{suggested, not isolated}.
                \end{itemize}
            \end{exampleblock}
        \end{column}
    \end{columns}
...
\begin{frame}{One lineage, one idea}
    ...
    \medskip
    \begin{columns}[t,onlytextwidth]
        \begin{column}{0.604\textwidth}
            \small
            \renewcommand{\arraystretch}{1.3}\begin{tabular}{@{}lp{5.6cm}@{}}
                \toprule
                ...
                MTLnet & \raggedright first joint model: hard parameter sharing; \tabularnewline
        ...
        \begin{column}{0.326\textwidth}
            ...
            \begin{tikzpicture}[
                ...
                \node[font=\small, anchor=west] at (1.30,0.00) {Ch.~5};
            \end{tikzpicture}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{The architecture: sharing by exchange}
        ...
        \begin{column}{0.550\textwidth}
            ...
            \begin{itemize}\setlength{\itemsep}{4pt}
                \item \textbf{Two inputs, one model} --- the category task reads the window of
                      per-visit vectors (\textbf{semantic stream}); the region task reads the
                      same window, each visit as its region node's vector
                ...
                \item \textbf{the shared trunk} --- a cross-attention stack of two blocks;
                      attention lets each stream read the other's features, each keeping its own
    ...
    \begin{alertblock}{}
        \small
        The tasks share by \alert{exchanging information between per-task streams}, not by
        owning hidden layers in common.
    \end{alertblock}
\end{frame}
...
\begin{frame}{The protocol, in four steps}
    ...
    \begin{itemize}\setlength{\itemsep}{5pt}
        \item \textbf{Category: macro-F1} --- reference point the \textbf{majority-class floor},
              \alert{5.7 to 7.3} depending on the dataset;
...
\begin{frame}[noframenumbering]{Se a pergunta for uma destas, o slide já existe}
    ...
    \begin{column}{0.465\textwidth}
        ...
        \hyperlink{b61}{\beamerbutton{B6-1}}\hyperlink{b62}{\beamerbutton{B6-2}}\hyperlink{b63}{\beamerbutton{B6-3}}\hyperlink{b64}{\beamerbutton{B6-4}}\hyperlink{b65}{\beamerbutton{B6-5}}
        \smallskip
