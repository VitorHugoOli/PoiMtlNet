...
\begin{frame}{The line this work stands on, and the one idea on it}
    ...
    $\rightarrow$ \textbf{GCN $\cdot$ GAT $\cdot$ GraphSAGE} $\rightarrow$
    \textbf{graph infomax: DGI $\cdot$ HGI $\cdot$ Check2HGI}\par
    \smallskip
    \textit{from position, to a geometry that reflects relationships, to relationships learned
    by neighborhood aggregation.}\par}

    \medskip
    \begin{block}{The idea these methods share}
        \small
        The model learns useful vectors by being asked to tell a true pairing of two parts of the
        data from a corrupted one, and it needs no labels to do so.\par
        \smallskip
        \alert{Infomax names what is maximized; the pairing test is how it is estimated}, because
        the quantity is never evaluated in closed form.
    \end{block}

    \vfill
    {\footnotesize
    \textit{In all three, a Delaunay triangulation joins the places.} \textbf{Each chapter adds
    its own mechanism.}\par
    \smallskip
    \textit{In this work the representations are trained with \textbf{no next-category and no
    next-region target}.}\par}
    \vfill
\end{frame}
