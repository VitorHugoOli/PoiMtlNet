...
\textbf{\textcolor{crim}{O relógio.}} Os tempos abaixo são os do plano e somam 48:30.
...
\textbf{ABERTURA --- a pergunta e o escopo} & S1--S7 & 5:00 & \rule{32mm}{0.4pt} \\
\textbf{FUNDAMENTOS --- dito uma vez} & S8--S16 & 11:00 & \rule{32mm}{0.4pt} \\
\textbf{MTLnet --- Cap. 3 (CBIC)} & S17--S25 & 16:30 & \rule{32mm}{0.4pt} \\
\textbf{ST-MTLNet --- Cap. 4 (CoUrb)} & S26--S32 & 22:30 & \rule{32mm}{0.4pt} \\
\textbf{Check2HGI --- Cap. 5 (MobiWac)} & S33--S49 & 43:00 & \rule{32mm}{0.4pt} \\
\textbf{CONCLUSÃO --- a resposta condicional} & S50--S55 & 48:30 & \rule{32mm}{0.4pt} \\
\hline
\end{tabular}
\clearpage
...
{\large\bfseries\color{ink} Movement is regular, and services depend on that}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape O ponto de partida é uma regularidade.}\par\vspace{1.5mm}
...
\vspace{1.5mm}{\footnotesize\color{soft} O ponto de partida é uma regularidade. Os rastros de mobilidade são ruidosos, mas o movimento humano é bastante regular: uma análise de entropia sobre rastros em larga escala estimou a previsibilidade potencial da próxima localização de uma pessoa em cerca de 93 por cento. Faço a ressalva na mesma frase, porque ela importa. Essa estimativa é sobre a próxima localização em resolução grossa, e ela não é teto para nenhuma métrica que eu vou reportar aqui. O que ela mostra é que existe regularidade aprendível. As pessoas voltam a um conjunto pequeno de lugares, e fazem viagens longas com menos frequência. E antecipar o que e o onde da próxima visita é o que sustenta recomendação, navegação, planejamento de transporte e alocação de recursos por área.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S3} $\cdot$ slide \textbf{2} $\cdot$ PDF p.4 $\cdot$ 55 s $\cdot$ fim previsto \textbf{2:00}}\par\vspace{1mm}
{\large\bfseries\color{ink} The ground: check-ins, mobility, and what joint training promises}\par\vspace{2mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S4} $\cdot$ slide \textbf{3} $\cdot$ PDF p.5 $\cdot$ 60 s $\cdot$ fim previsto \textbf{3:00}}\par\vspace{1mm}
{\large\bfseries\color{ink} The question, and the answer in one line}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape A pergunta da dissertação, literalmente: o aprendizado multitarefa ajuda a predição de pontos de interesse, próxima categoria e próxima região, e de que depende a resposta?}\par\vspace{1.5mm}
...
\item equivalentes a zero dentro de meio ponto
\end{itemize}
...
\vspace{1.5mm}{\footnotesize\color{soft} A pergunta da dissertação, literalmente: o aprendizado multitarefa ajuda a predição de pontos de interesse, próxima categoria e próxima região, e de que depende a resposta? Ela vem com uma restrição que vale para tudo o que vem depois: um artefato treinado tem de produzir as duas saídas numa passagem só. E a resposta eu dou agora, no minuto três, e não no fim, porque daqui em diante cada slide é resposta a uma pergunta que eu já fiz. Na previsão da \textbf{próxima região}, o modelo conjunto \textbf{supera} os dedicados no \textbf{Texas} e na \textbf{Califórnia}, e nos outros quatro conjuntos \textbf{permanece dentro da margem de dois pontos}, registrada antes de qualquer resultado ser lido: quatro déficits pequenos, com a direção declarada, nenhum empate. Na \textbf{próxima categoria}, \textbf{supera na Flórida}, e as cinco diferenças restantes são \textbf{equivalentes a zero dentro de meio ponto}. As quatro células dentro da margem eu enumero uma a uma na Seção 5, com intervalo e com teste.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S5} $\cdot$ slide \textbf{4} $\cdot$ PDF p.6 $\cdot$ 40 s $\cdot$ fim previsto \textbf{3:40}}\par\vspace{1mm}
{\large\bfseries\color{ink} What is predicted, and what is not}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape O escopo, no positivo. Eu predigo duas propriedades da próxima visita: a categoria, que é o tipo do lugar, e a região, que é a unidade administrativa em escala de bairro onde a visita acontece.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{COBRE} --- INTRODUZ as três tarefas (Defs. 2.6, 2.7, 2.8) e a exclusão do próximo lugar (Def. 2.9) $\cdot$ RETOMA a restrição de modelo único}\par\vspace{1mm}
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- "prediz o próximo POI". $\cdot$ Nenhum particular do corpus: nem o número de classes, nem setor censitário, nem mahalle, nem nome de estado (isso é de S10).}\par
\vspace{1.5mm}{\footnotesize\color{soft} O escopo, no positivo. Eu predigo duas propriedades da próxima visita: a categoria, que é o tipo do lugar, e a região, que é a unidade administrativa em escala de bairro onde a visita acontece. O próximo lugar exato eu não predigo. Ele está definido no Capítulo 2 justamente para ser excluído, e nenhum capítulo reporta resultado para ele. Existe ainda uma terceira tarefa no trabalho, a classificação de categoria, que é estática: lê a representação de um lugar, e não um histórico. Ela é metade do par nos dois primeiros estudos.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S6} $\cdot$ slide \textbf{5} $\cdot$ PDF p.7 $\cdot$ 40 s $\cdot$ fim previsto \textbf{4:20}}\par\vspace{1mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S7} $\cdot$ slide \textbf{6} $\cdot$ PDF p.8 $\cdot$ 40 s $\cdot$ fim previsto \textbf{5:00}}\par\vspace{1mm}
{\large\bfseries\color{ink} The contribution, in one block}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape A contribuição, em duas metades, e eu volto a esta tela no fim com as mesmas palavras.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{COBRE} --- INTRODUZ a contribuição una (a segunda aparição, com redação idêntica, é o slide de fechamento da Seção 6)}\par\vspace{1mm}
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- "MTL funciona" sem condição. $\cdot$ Nenhum número aqui, e em particular nenhuma contagem de parâmetros: a razão entre o modelo conjunto e os dois dedicados não foi re-medida (PLANO §8 regra 9).}\par
\vspace{1.5mm}{\footnotesize\color{soft} A contribuição, em duas metades, e eu volto a esta tela no fim com as mesmas palavras. A metade prática: um modelo, uma passagem, duas predições. O ganho é operacional, não computacional. O modelo conjunto é o artefato maior, e uma passagem por ele custa mais do que rodar os dois dedicados; o que diminui é o número de modelos para treinar e manter. A metade científica: o que eu entrego são condições, não um sim universal. A representação de entrada e a topologia de compartilhamento decidem se o multitarefa ajuda nestas tarefas. É por isso que um resultado nulo com embedding por lugar e compartilhamento rígido não contradiz um resultado positivo com representação em nível de check-in e outra forma de compartilhar.\par}
\end{tcolorbox}\vspace{2mm}
\vspace{2mm}{\LARGE\bfseries\color{teal} FUNDAMENTOS --- dito uma vez}\par\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S8} $\cdot$ slide \textbf{7} $\cdot$ PDF p.10 $\cdot$ 55 s $\cdot$ fim previsto \textbf{5:55}}\par\vspace{1mm}
{\large\bfseries\color{ink} One lineage, one idea}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Uma ideia só cobre três métodos desta dissertação, então é melhor dizê-la agora do que três vezes.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{COBRE} --- INTRODUZ a ideia infomax $\cdot$ INTRODUZ a linhagem de modelos (Tab. 1) e o diagrama de níveis $\cdot$ *(o nome Check2HGI aparece na Tab. 1 como linha do mapa; o artefato é INTRODUZ em 5.2, e MTLnet em 3.2)*}\par\vspace{1mm}
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- nenhum resultado, nenhum número de capítulo. $\cdot$ Não explicar FiLM aqui. $\cdot$ Nunca Space2Vec nem POI2Vec como componentes deste trabalho.}\par
\vspace{1.5mm}{\footnotesize\color{soft} Uma ideia só cobre três métodos desta dissertação, então é melhor dizê-la agora do que três vezes. A ideia infomax, nas palavras do próprio capítulo: o modelo aprende vetores úteis sendo obrigado a distinguir um pareamento verdadeiro de um pareamento corrompido, e não precisa de rótulo nenhum para isso, porque os próprios dados dizem qual é o verdadeiro. O DGI faz essa comparação entre um nó e um resumo do grafo. O HGI estende o mesmo objetivo por uma hierarquia de lugar, região e cidade. E o Check2HGI, do Capítulo 5, acrescenta um quarto nível abaixo do lugar, que é o check-in. O mecanismo de cada um fica com o capítulo dono dele. A tabela na tela é a Tabela 1 da dissertação, e ela é o mapa da fala inteira: a cada seção eu volto a ela e digo em que linha eu estou.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S9} $\cdot$ slide \textbf{8} $\cdot$ PDF p.11 $\cdot$ 40 s $\cdot$ fim previsto \textbf{6:35}}\par\vspace{1mm}
{\large\bfseries\color{ink} How two tasks share a model, and how that fails}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Duas definições e um critério. Compartilhamento rígido é a topologia em que todas as tarefas atravessam um mesmo tronco e só se separam na saída de cada uma.}\par\vspace{1.5mm}
...
\vspace{1.5mm}{\footnotesize\color{soft} Duas definições e um critério. Compartilhamento rígido é a topologia em que todas as tarefas atravessam um mesmo tronco e só se separam na saída de cada uma. Transferência negativa é o desfecho que se teme: o treino conjunto deixa uma tarefa pior do que o modelo dedicado dela deixaria. O critério está declarado no Capítulo 2, e eu vou cobrá-lo mais adiante: um método de balanceamento só é útil se superar uma ponderação fixa bem ajustada. Guardem essa frase. É ela que decide o que eu posso e o que eu não posso afirmar sobre o balanceador na Seção 3.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S10} $\cdot$ slide \textbf{9} $\cdot$ PDF p.12 $\cdot$ 55 s $\cdot$ fim previsto \textbf{7:30}}\par\vspace{1mm}
{\large\bfseries\color{ink} The evidence base: six datasets, said once}\par\vspace{2mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S14} $\cdot$ slide \textbf{13} $\cdot$ PDF p.16 $\cdot$ 30 s $\cdot$ fim previsto \textbf{10:00}}\par\vspace{1mm}
{\large\bfseries\color{ink} The axis that separates this work}\par\vspace{2mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S15} $\cdot$ slide \textbf{14} $\cdot$ PDF p.17 $\cdot$ 40 s $\cdot$ fim previsto \textbf{10:40}}\par\vspace{1mm}
{\large\bfseries\color{ink} Related work in representation: the line this work stands on}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Esta é a base mais importante da dissertação, e por isso ela tem tela própria.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{COBRE} --- INTRODUZ a linhagem de representações, no nível de o que cada degrau resolve}\par\vspace{1mm}
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- o mecanismo do DGI ou do HGI aqui: eles pertencem a 3.2A e 4.1A. $\cdot$ Nunca Space2Vec nem POI2Vec como componentes deste trabalho. $\cdot$ Nenhum resultado, nenhum número.}\par
\vspace{1.5mm}{\footnotesize\color{soft} Esta é a base mais importante da dissertação, e por isso ela tem tela própria. É uma escada. Começa no identificador one-hot, que marca um lugar por posição e não codifica relação nenhuma. Sobe para as representações distribuídas, skip-gram, DeepWalk, node2vec, em que a geometria do vetor reflete a relação que está nos dados. Sobe de novo para as redes de grafo, GCN, GAT, GraphSAGE, que aprendem essa relação por agregação de vizinhança. Depois vem o DGI, que aplica o objetivo infomax entre um nó e um resumo do grafo, e o HGI, que estende o mesmo objetivo pela hierarquia de lugar, região e cidade. O mecanismo de cada um fica com o capítulo dono: o DGI no Capítulo 3, o HGI no Capítulo 4. Aqui é só o mapa, para a linhagem ficar legível quando eu chegar ao Check2HGI.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S16} $\cdot$ slide \textbf{15} $\cdot$ PDF p.18 $\cdot$ 20 s $\cdot$ fim previsto \textbf{11:00}}\par\vspace{1mm}
{\large\bfseries\color{ink} With the vocabulary fixed, each study says only what it changed}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Com o vocabulário, os dados e a métrica fixados uma única vez, cada estudo agora só precisa dizer o que mudou, e cada um nomeia a sua própria convenção quando chegar a hora.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{COBRE} --- RETOMA vocabulário, dados e métrica}\par\vspace{1mm}
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- que as "regras de decisão" ficaram fixadas aqui. $\cdot$ O protocolo estatístico entra só em 5.4, e prometer o fechamento que a Seção 2 não entrega é o defeito que a reescrita de 2026-08-21 corrigiu.}\par
\vspace{1.5mm}{\footnotesize\color{soft} Com o vocabulário, os dados e a métrica fixados uma única vez, cada estudo agora só precisa dizer o que mudou, e cada um nomeia a sua própria convenção quando chegar a hora. E o par de tarefas dos dois primeiros estudos não é o do terceiro; quando ele mudar, eu aviso. O primeiro usou o que a literatura oferecia: um vetor por lugar e um tronco compartilhado.\par}
\end{tcolorbox}\vspace{2mm}
\vspace{2mm}{\LARGE\bfseries\color{teal} MTLnet --- Cap. 3 (CBIC)}\par\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S17} $\cdot$ slide \textbf{16} $\cdot$ PDF p.20 $\cdot$ 30 s $\cdot$ fim previsto \textbf{11:30}}\par\vspace{1mm}
{\large\bfseries\color{ink} One static task, one sequential task}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape O par do primeiro estudo. Uma tarefa estática: ler a representação de um lugar e prever a categoria dele.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{COBRE} --- INTRODUZ a dicotomia estática × sequencial que o Cap. 3 põe à prova $\cdot$ RETOMA as Defs. 2.6 e 2.7 e a armadilha de nome de S12}\par\vspace{1mm}
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- "prediz o próximo POI" ou "próximo lugar". $\cdot$ Neste capítulo Next-POI Prediction é a próxima categoria, e o carimbo está na tela.}\par
\vspace{1.5mm}{\footnotesize\color{soft} O par do primeiro estudo. Uma tarefa estática: ler a representação de um lugar e prever a categoria dele. E uma tarefa sequencial: ler um histórico de nove visitas e prever a categoria da próxima. Na superfície elas são relacionadas, porque saem dos mesmos dados e do mesmo espaço de sete categorias. Na natureza, não: uma depende das características intrínsecas de um lugar, a outra depende de ordem temporal e de transição. O capítulo entra no experimento com uma hipótese declarada, e ela é negativa: essa diferença é grande o bastante para limitar o que um tronco compartilhado consegue fazer pelas duas.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S18} $\cdot$ slide \textbf{17} $\cdot$ PDF p.21 $\cdot$ 45 s $\cdot$ fim previsto \textbf{12:15}}\par\vspace{1mm}
...
\vspace{1.5mm}{\footnotesize\color{soft} Esta é a arquitetura, o MTLnet. Vale guardar a figura, porque o Capítulo 4 não vai alterar uma linha dela. Cada tarefa entra por um encoder próprio, um MLP. Vem então a modulação FiLM, e ela cabe numa cláusula: um vetor de identidade de tarefa gera uma escala e um deslocamento, aplicados às features antes da parte compartilhada, de modo que as duas tarefas leiam os mesmos parâmetros sob escalas diferentes. Depois vem o tronco de blocos residuais, que é o compartilhamento rígido da Definição 2.10. E no fim duas saídas, uma por tarefa. O rodapé é o detalhe que vai importar daqui a dois slides: o capítulo declara os parâmetros em dois conjuntos disjuntos, os compartilhados e os específicos de tarefa. É sobre o primeiro conjunto que um balanceador de gradientes age.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S19} $\cdot$ slide \textbf{18} $\cdot$ PDF p.22 $\cdot$ 45 s $\cdot$ fim previsto \textbf{13:00}}\par\vspace{1mm}
{\large\bfseries\color{ink} DGI: how it works | why it was used}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape O primeiro mecanismo, e ele responde uma pergunta que costuma vir.}\par\vspace{1.5mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S22} $\cdot$ slide \textbf{21} $\cdot$ PDF p.25 $\cdot$ 40 s $\cdot$ fim previsto \textbf{14:50}}\par\vspace{1mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S23} $\cdot$ slide \textbf{22} $\cdot$ PDF p.26 $\cdot$ 55 s $\cdot$ fim previsto \textbf{15:45}}\par\vspace{1mm}
{\large\bfseries\color{ink} The null result, shown rather than asserted}\par\vspace{2mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S24} $\cdot$ slide \textbf{23} $\cdot$ PDF p.27 $\cdot$ 25 s $\cdot$ fim previsto \textbf{16:10}}\par\vspace{1mm}
{\large\bfseries\color{ink} A null with three suspects}\par\vspace{2mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S25} $\cdot$ slide \textbf{24} $\cdot$ PDF p.28 $\cdot$ 20 s $\cdot$ fim previsto \textbf{16:30}}\par\vspace{1mm}
{\large\bfseries\color{ink} A null with three suspects does not close the investigation}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Um nulo com três suspeitos não encerra a investigação: ele desenha o próximo experimento.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{COBRE} --- RETOMA a bifurcação de 3.5}\par\vspace{1mm}
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- que o próximo capítulo responde os três suspeitos. $\cdot$ Ele condena um.}\par
\vspace{1.5mm}{\footnotesize\color{soft} Um nulo com três suspeitos não encerra a investigação: ele desenha o próximo experimento. Congelar a arquitetura e mover apenas a entrada.\par}
\end{tcolorbox}\vspace{2mm}
\vspace{2mm}{\LARGE\bfseries\color{teal} ST-MTLNet --- Cap. 4 (CoUrb)}\par\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S26} $\cdot$ slide \textbf{25} $\cdot$ PDF p.30 $\cdot$ 40 s $\cdot$ fim previsto \textbf{17:10}}\par\vspace{1mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S27} $\cdot$ slide \textbf{27} $\cdot$ PDF p.32 $\cdot$ 55 s $\cdot$ fim previsto \textbf{18:05}}\par\vspace{1mm}
{\large\bfseries\color{ink} HGI: how it works | why it was used}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape O segundo mecanismo. É o conceito que sustenta o resto da dissertação, então eu vou com calma.}\par\vspace{1.5mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S29} $\cdot$ slide \textbf{29} $\cdot$ PDF p.34 $\cdot$ 45 s $\cdot$ fim previsto \textbf{19:50}}\par\vspace{1mm}
{\large\bfseries\color{ink} The caveat, then the number}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Aqui a ordem importa mais que o número, então eu digo a ressalva primeiro, em uma cláusula, e sigo em frente.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{COBRE} --- INTRODUZ o resultado da tarefa estática do Cap. 4 e a ressalva de rótulo na entrada}\par\vspace{1mm}
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- número antes da ressalva. $\cdot$ "macro-F1 subiu 20 a 22" (são médias de F1 por categoria). $\cdot$ A faixa sem dizer que é melhor-de-dois. $\cdot$ Deixar o ganho estático falar pela tarefa sequencial.}\par
\vspace{1.5mm}{\footnotesize\color{soft} Aqui a ordem importa mais que o número, então eu digo a ressalva primeiro, em uma cláusula, e sigo em frente. Depois da publicação, nós estabelecemos que a entrada da tarefa estática deste capítulo contém o rótulo que ela prevê: a feature de tipo de local mapeia um-para-um nas sete categorias de topo. A acurácia reportada nessa tarefa mede essa consulta, e não inferência semântica aprendida. A consequência é direta e eu prefiro dizê-la eu mesmo: o ganho estático não diz nada sobre a tarefa sequencial. Dito isso, o número. Na tarefa estática a entrada decomposta lidera nas vinte e uma combinações de categoria e estado, com ganhos médios por estado de vinte vírgula dois a vinte e dois pontos percentuais. E eu declaro o que essa faixa é: é o melhor dos dois codificadores espaciais em cada combinação, não é nenhum dos dois sozinho.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S30} $\cdot$ slide \textbf{30} $\cdot$ PDF p.36 $\cdot$ 60 s $\cdot$ fim previsto \textbf{20:50}}\par\vspace{1mm}
{\large\bfseries\color{ink} The diagnostic result is the sequential task}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Agora a tarefa que produz o diagnóstico, que é a sequencial, e a razão é uma só: o alvo dela nunca está na entrada.}\par\vspace{1.5mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S32} $\cdot$ slide \textbf{32} $\cdot$ PDF p.38 $\cdot$ 35 s $\cdot$ fim previsto \textbf{22:30}}\par\vspace{1mm}
{\large\bfseries\color{ink} With the architecture fixed, the input moved the result}\par\vspace{2mm}
...
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S33} $\cdot$ slide \textbf{33} $\cdot$ PDF p.40 $\cdot$ 60 s $\cdot$ fim previsto \textbf{23:30}}\par\vspace{1mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S34} $\cdot$ slide \textbf{34} $\cdot$ PDF p.41 $\cdot$ 35 s $\cdot$ fim previsto \textbf{24:05}}\par\vspace{1mm}
...
\vspace{0.5mm}{\small\textbf{N\'UMEROS} --- classes em Istambul a 8.501 na Califórnia: mais}\par
...
\vspace{1.5mm}{\footnotesize\color{soft} Trabalho relacionado deste estudo, que os dois primeiros não têm, e a primeira metade é a tarefa nova. Próxima região é classificação sobre as regiões candidatas do conjunto, de 520 classes em Istambul a 8.501 na Califórnia: mais grossa que lugar não quer dizer mais fácil. Prever sobre uma partição do mapa é a formulação padrão em mobilidade, com célula de grade como alvo; aqui entra no lugar dela a unidade administrativa de bairro. E onde a área já modela várias granularidades, categoria e região aparecem como sinais auxiliares de um alvo principal de próximo lugar. Eu estudo o par como objeto. O escopo vai junto com a motivação: preparação em nível de bairro, e nenhum serviço construído ou avaliado aqui.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S35} $\cdot$ slide \textbf{35} $\cdot$ PDF p.42 $\cdot$ 25 s $\cdot$ fim previsto \textbf{24:30}}\par\vspace{1mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S36} $\cdot$ slide \textbf{36} $\cdot$ PDF p.43 $\cdot$ 80 s $\cdot$ fim previsto \textbf{25:50}}\par\vspace{1mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S38} $\cdot$ slide \textbf{38} $\cdot$ PDF p.45 $\cdot$ 90 s $\cdot$ fim previsto \textbf{28:30}}\par\vspace{1mm}
{\large\bfseries\color{ink} The geometry of the vectors}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape E este é o resultado da representação sozinha, antes de qualquer modelo.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{DIZER EXATO}}
\begin{itemize}[leftmargin=6mm,itemsep=0.4mm,topsep=0.8mm]
\item 0,57
\item 0,00
...
\item 0,78
\end{itemize}
\vspace{0.5mm}{\small\textbf{N\'UMEROS} --- Ela dá cerca de 0,57 para a representação $\cdot$ check-in contra cerca de 0,00 para o embedding $\cdot$ próximos dá cerca de 0,98 contra 0,78. As}\par
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- nenhum p-valor aqui, e nenhuma afirmação de significância sobre esta figura. $\cdot$ Nunca chamar a diferença de representação de "margem": margem é do TOST, e isto é uma diferença.}\par
\vspace{1.5mm}{\footnotesize\color{soft} E este é o resultado da representação sozinha, antes de qualquer modelo. A pergunta é simples: esses vetores por visita separam as sete categorias? A silhueta por categoria mede quão compactos e quão separados estão os grupos rotulados, numa escala de menos um a um. Ela dá cerca de \textbf{0,57} para a representação em nível de check-in contra cerca de \textbf{0,00} para o embedding por lugar. A pureza de categoria dos dez vizinhos mais próximos dá cerca de \textbf{0,98} contra \textbf{0,78}. As duas médias são sobre os cinco estados americanos. Duas ressalvas, e eu faço as duas antes de alguém pedir. A primeira: a figura caracteriza a \textbf{família} da representação, e não a configuração exata que eu avalio depois. É por isso que ela não precisa de partição, de semente nem de pareamento, e é por isso que ela pode vir antes do protocolo. A segunda: a mesma geometria \textbf{não} separa regiões. O benefício é de categoria, e o fluxo espacial do modelo lê os vetores de região do mesmo grafo, não estes. Nenhum p-valor nesta tela: aqui é geometria, e o teste vem depois, no bloco de resultados.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S39} $\cdot$ slide \textbf{39} $\cdot$ PDF p.46 $\cdot$ 90 s $\cdot$ fim previsto \textbf{30:00}}\par\vspace{1mm}
...
{\small\color{teal}\textbf{DIZER EXATO}}
\begin{itemize}[leftmargin=6mm,itemsep=0.4mm,topsep=0.8mm]
\item por troca de informação entre fluxos por tarefa
\end{itemize}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S41} $\cdot$ slide \textbf{41} $\cdot$ PDF p.48 $\cdot$ 60 s $\cdot$ fim previsto \textbf{32:30}}\par\vspace{1mm}
{\large\bfseries\color{ink} Protocol, step 1 of 4: the unit of data}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape O protocolo, e ele é o degrau que sustenta tudo o que vem depois.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{DIZER EXATO}}
\begin{itemize}[leftmargin=6mm,itemsep=0.4mm,topsep=0.8mm]
\item disjunta por usuário
\item sobrepostas, de passo um
\item estas não são as mesmas janelas
\end{itemize}
...
\vspace{1.5mm}{\footnotesize\color{soft} O protocolo, e ele é o degrau que sustenta tudo o que vem depois. São quatro passos, e cada um responde a uma pergunta que o anterior deixa aberta. Primeiro passo: qual é a unidade de dados. Validação cruzada de cinco partições, \textbf{disjunta por usuário}: todas as janelas de um usuário ficam na mesma partição, então as visitas de um usuário de teste nunca aparecem no treino. Isto é exatamente a reparação da limitação que eu declarei no Capítulo 3. A estratificação é pelo rótulo da próxima categoria, porque as sete classes são desbalanceadas. As janelas são \textbf{sobrepostas, de passo um}: para cada usuário com pelo menos dez visitas, começa uma janela de nove visitas em cada visita, e a visita seguinte é o alvo; janelas curtas duplicadas, que terminam no mesmo alvo, são removidas. Repito o aviso da Seção 2: \textbf{estas não são as mesmas janelas} dos Capítulos 3 e 4, que usaram janelas não sobrepostas. E uma coisa que eu digo agora para não parecer descoberta depois: a partição retida é também a de validação, não há um terceiro corte, e é daí que sai o segundo dos meus limites.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S42} $\cdot$ slide \textbf{42} $\cdot$ PDF p.49 $\cdot$ 65 s $\cdot$ fim previsto \textbf{33:35}}\par\vspace{1mm}
{\large\bfseries\color{ink} Protocol, step 2 of 4: what is measured}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Segundo passo: o que se mede. Na categoria, macro-F1, como eu defini na Seção 2, e o ponto de referência dela é o piso de classe majoritária, que fica entre 5,7 e 7,3 conforme o conjunto.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{DIZER EXATO}}
\begin{itemize}[leftmargin=6mm,itemsep=0.4mm,topsep=0.8mm]
\item 24,7\% das visitas
\end{itemize}
\vspace{0.5mm}{\small\textbf{N\'UMEROS} --- majoritária, que fica entre 5,7 e 7,3 conforme $\cdot$ e ainda assim marca 5,7 de macro-F1, porque}\par
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- Acc@10 sem o desconto OOD. $\cdot$ Nenhum número sem o seu ponto de referência.}\par
\vspace{1.5mm}{\footnotesize\color{soft} Segundo passo: o que se mede. Na categoria, macro-F1, como eu defini na Seção 2, e o ponto de referência dela é o piso de classe majoritária, que fica entre 5,7 e 7,3 conforme o conjunto. Um exemplo concreto mostra por que essa métrica existe e a acurácia simples não: na Flórida esse piso acerta \textbf{24,7\% das visitas} e ainda assim marca \textbf{5,7} de macro-F1, porque as outras seis categorias ele nunca acerta. Na região, acurácia em dez: a fração de visitas de teste cuja região verdadeira está entre as dez predições de maior pontuação. Ela não distingue o primeiro lugar do décimo, e eu declaro isso. E ela vem com um desconto, que é o ponto que mais gera pergunta: uma região que não aparece na partição de treino conta como \textbf{erro}. Então o que eu reporto é a acurácia em dez medida nas visitas dentro da distribuição, multiplicada por um menos a fração fora da distribuição. Os pontos de referência da região são dois. O modelo dedicado, que é a comparação controlada. E um piso de Markov de primeira ordem sobre transições de região, calculado sob as mesmas janelas e as mesmas partições, que alcança de 51 a 72 de acurácia em dez. Esse piso é alto de propósito: janelas de passo um fazem da última região visitada um preditor forte da próxima, e é exatamente esse sinal que uma tabela de transição lê.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S43} $\cdot$ slide \textbf{43} $\cdot$ PDF p.50 $\cdot$ 65 s $\cdot$ fim previsto \textbf{34:40}}\par\vspace{1mm}
{\large\bfseries\color{ink} Protocol, step 3 of 4: what is compared}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Terceiro passo: o que se compara. A comparação é entre o modelo conjunto e os modelos dedicados, lendo a mesma representação, as mesmas janelas e as mesmas partições.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{DIZER EXATO}}
\begin{itemize}[leftmargin=6mm,itemsep=0.4mm,topsep=0.8mm]
\item vinte modelos ajustados por configuração
\item um único modelo salvo por partição
\item Todo veredito que eu vou dar é o da convenção estrita
\end{itemize}
\vspace{0.5mm}{\small\textbf{N\'UMEROS} --- modelo conjunto, em até 0,23 de macro-F1 e $\cdot$ 0,23 de macro-F1 e 0,93 de acurácia em}\par
...
\vspace{1.5mm}{\footnotesize\color{soft} Terceiro passo: o que se compara. A comparação é entre o modelo conjunto e os modelos dedicados, lendo a mesma representação, as mesmas janelas e as mesmas partições. Começo definindo semente, porque a palavra é ambígua na literatura. Aqui uma \textbf{semente} é uma repetição completa do experimento de cinco partições: ela fixa a inicialização aleatória \textbf{e} a divisão dos usuários, então cada semente sorteia a sua própria divisão. Dentro de uma semente, os modelos comparados leem a mesma partição, e é isso que licencia o pareamento. São quatro sementes, zero, um, sete e cem, vezes cinco partições, o que dá \textbf{vinte modelos ajustados por configuração}. Mas a unidade inferencial é \textbf{quatro}, as quatro médias por semente, porque partições dentro de uma semente não são independentes. E a convenção de leitura é a *joint-best*: as duas notas vêm de \textbf{um único modelo salvo por partição}, na época escolhida pela nota conjunta de validação, que é a média geométrica das duas métricas. Eu escolhi a convenção mais estrita de propósito. A alternativa, ler cada tarefa na melhor época dela, não descreve nenhum modelo salvo; ela é mais favorável ao modelo conjunto, em até 0,23 de macro-F1 e 0,93 de acurácia em dez numa semente, e viraria mais quatro resultados de categoria e mais dois de região em melhoras sob a mesma correção. \textbf{Todo veredito que eu vou dar é o da convenção estrita.}\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S44} $\cdot$ slide \textbf{44} $\cdot$ PDF p.51 $\cdot$ 65 s $\cdot$ fim previsto \textbf{35:45}}\par\vspace{1mm}
{\large\bfseries\color{ink} Protocol, step 4 of 4: how it is decided}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Quarto passo: como se decide. A primeira frase é a que organiza tudo: afirmar ganho e afirmar equivalência exigem testes diferentes, e uma diferença não significativa não é evidência de equivalência.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{DIZER EXATO}}
\begin{itemize}[leftmargin=6mm,itemsep=0.4mm,topsep=0.8mm]
\item afirmar ganho e afirmar equivalência exigem testes diferentes
\item reportado ao lado e concorda
\item resultados secundários, fora do plano
\end{itemize}
\vspace{0.5mm}{\small\textbf{N\'UMEROS} --- consegue ficar abaixo de 0,0625, qualquer que seja}\par
...
\vspace{1.5mm}{\footnotesize\color{soft} Quarto passo: como se decide. A primeira frase é a que organiza tudo: \textbf{afirmar ganho e afirmar equivalência exigem testes diferentes}, e uma diferença não significativa não é evidência de equivalência. Havia um plano de análise escrito, fixado durante o desenvolvimento e antes de qualquer resultado ser lido. Ele atribuiu um teste de superioridade à próxima categoria e um teste de não-inferioridade à próxima região, numa margem de dois pontos. A margem tem razão declarada: uma variação desse tamanho na acurácia em dez fica abaixo do nível em que a preparação em escala de bairro se comportaria de outro jeito. O teste primário é um t pareado sobre as quatro médias por semente, com intervalo de confiança de noventa por cento, e correção de Holm sobre os seis conjuntos, separadamente dentro de cada família de tarefa. Aqui eu declaro um desvio, porque ele existe e está no código: o plano registrava um Wilcoxon pareado sobre as vinte diferenças por partição. Ele é \textbf{reportado ao lado e concorda} com o t. O t carrega o veredito porque partições dentro de uma semente não são independentes, e porque, nesse apoio, o Wilcoxon exato unilateral não consegue ficar abaixo de 0,0625, qualquer que seja o efeito. Não é confissão: são dois apoios com o mesmo veredito. E duas consequências do plano, ditas antes dos números: ele não definiu teste de superioridade para região, então os dois ganhos de região que eu vou mostrar são \textbf{resultados secundários, fora do plano}; e ele não registrou margem de equivalência na categoria, então uma diferença de categoria que falha a superioridade é \textbf{não resolvida}, e é reportada pelo limite que o intervalo dela sustenta.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S45} $\cdot$ slide \textbf{45} $\cdot$ PDF p.52 $\cdot$ 110 s $\cdot$ fim previsto \textbf{37:35}}\par\vspace{1mm}
{\large\bfseries\color{ink} Result 1: the representation, at every dataset}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Primeiro resultado, e ele é sobre a representação sozinha, não sobre o multitarefa.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{DIZER EXATO}}
\begin{itemize}[leftmargin=6mm,itemsep=0.4mm,topsep=0.8mm]
\item Só a entrada muda
\item à frente nos seis
\item unânime nas cinco partições em todos eles
\item cinco dos seis
\item direção consistente
\item o que eles não fazem é isolar a hierarquia
\end{itemize}
\vspace{0.5mm}{\small\textbf{N\'UMEROS} --- a p igual a 0,07, onde a direção $\cdot$ salto da tabela, mais 0,23. A faixa vai $\cdot$ faixa vai desse mais 0,23 na Flórida a $\cdot$ na Flórida a mais 6,29 em Istambul. O}\par
...
\vspace{1.5mm}{\footnotesize\color{soft} Primeiro resultado, e ele é sobre a representação sozinha, não sobre o multitarefa. A comparação é controlada: mesmo alvo, mesmo modelo de tarefa única, mesma configuração de treino, mesmas partições, mesmas janelas, mesmo orçamento de épocas, mesmo ajuste de logit. \textbf{Só a entrada muda.} A convenção desta tabela é a semente zero, com cinco partições pareadas, e o desvio é entre partições; guardem isso, porque a próxima tabela tem outra convenção. A leitura é a da própria tabela, não a minha: o nível de check-in está \textbf{à frente nos seis} conjuntos, e é \textbf{unânime nas cinco partições em todos eles}; um teste pareado sobre as cinco partições separa as duas colunas em \textbf{cinco dos seis}, e a Flórida é a exceção, a p igual a 0,07, onde a direção é unânime mas a diferença não alcança significância. E a Flórida ser a exceção não é acaso: ela é o menor salto da tabela, mais 0,23. A faixa vai desse mais 0,23 na Flórida a mais 6,29 em Istambul. O que isso estabelece é uma \textbf{direção consistente}, não um efeito grande. À direita, os dois controles que separam esse ganho de duas explicações mais baratas. O CTLE, que é a contextualização mais próxima, fica cerca de dois pontos abaixo do embedding por lugar na Flórida sob a mesma regra, e repete a ordenação com pesos fixos no Alabama, no Arizona e em Istambul. E a concatenação de features cruas ao embedding por lugar levanta esse embedding em 2,0, 1,7 e 0,8 ponto de macro-F1, no Alabama, no Arizona e na Flórida. Os dois controles limitam explicações mais baratas; \textbf{o que eles não fazem é isolar a hierarquia}. O controle de concatenação foi refeito depois do envio, na escala da Tabela 9, e lá ele fecha a maior parte da diferença --- num conjunto, a ultrapassa. Tenho o slide, se quiserem vê-lo.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S46} $\cdot$ slide \textbf{46} $\cdot$ PDF p.53 $\cdot$ 120 s $\cdot$ fim previsto \textbf{39:35}}\par\vspace{1mm}
{\large\bfseries\color{ink} Result 2: one model, two tasks}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Segundo resultado. E uma frase sobre a coluna do meio, antes de eu ler as duas nossas, porque ela muda o que as outras duas significam: a coluna Dedicated é o sistema mais forte desta tabela.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{DIZER EXATO}}
\begin{itemize}[leftmargin=6mm,itemsep=0.4mm,topsep=0.8mm]
\item pelo menos três vírgula zero seis pontos
\item entre sementes
\end{itemize}
\vspace{0.5mm}{\small\textbf{N\'UMEROS} --- POI-RGNN em pelo menos 3,06 pontos nos seis $\cdot$ na tela, fica entre 5,7 e 7,3. Na $\cdot$ forte em pelo menos 3,55 pontos de acurácia}\par
...
\vspace{1.5mm}{\footnotesize\color{soft} Segundo resultado. E uma frase sobre a coluna do meio, antes de eu ler as duas nossas, porque ela muda o que as outras duas significam: a coluna \textbf{Dedicated} é o sistema mais forte desta tabela. Ela está acima de todo baseline externo reportado, nos seis conjuntos e nas duas tarefas, e acima do piso de Markov também. Então o que vocês vão ver não é o modelo conjunto igualando um espantalho: é ele igualando a régua mais dura que eu tenho. E o capítulo põe número nisso: o conjunto fica \textbf{pelo menos três vírgula zero seis pontos} acima do baseline externo mais forte em todos os conjuntos, enquanto a diferença entre um modelo e dois é \textbf{meio ponto}. É o menor dos três efeitos em jogo aqui, e é nesse sentido que um modelo pode substituir dois. Em cima, a próxima categoria; embaixo, a próxima região; os mesmos seis conjuntos, na mesma ordem, nos dois blocos. Primeiro a convenção, porque ela mudou: aqui são quatro sementes vezes cinco partições, e o desvio é \textbf{entre sementes}, não entre partições como na tabela anterior. A ressalva vem antes da leitura: o modelo dedicado de categoria teve busca de configuração em todos os seis conjuntos, e o conjunto \textbf{não} teve busca no Texas nem na Califórnia, que carregam configuração transferida. Onde a busca do dedicado é a mais ampla, o resíduo favorece o dedicado, o que torna a diferença de categoria que eu reporto conservadora ali. A comparação que sustenta a minha afirmação é entre as colunas \textbf{Dedicated} e \textbf{Joint}, porque essas duas leem a mesma representação, as mesmas janelas e as mesmas partições. As colunas externas estão aqui como comparação com desenhos publicados, e elas rodam com as representações delas, então trazem junto a vantagem de representação do slide anterior. Na categoria, o conjunto excede o POI-RGNN em pelo menos 3,06 pontos nos seis, e o piso de classe majoritária, que não está na tela, fica entre 5,7 e 7,3. Na região, o conjunto excede a referência externa mais forte em pelo menos 3,55 pontos de acurácia em dez. O STAN e o ReHDM não estão na tela, e a razão é a ressalva de protocolo: o STAN roda nas nossas partições mas constrói as próprias representações e as próprias sequências, e o ReHDM roda sob o protocolo publicado dele. E eu vou dizer uma coisa contra mim mesmo, porque ela está no capítulo: o piso de Markov, que é um método não aprendido, fica \textbf{acima} desses três sistemas externos na maioria dos conjuntos. É por isso que eu trato o piso, e não os externos, como a referência que a tarefa de região tem de exceder. O conjunto e o dedicado estão acima do piso nos seis.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S47} $\cdot$ slide \textbf{47} $\cdot$ PDF p.54 $\cdot$ 100 s $\cdot$ fim previsto \textbf{41:15}}\par\vspace{1mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S48} $\cdot$ slide \textbf{48} $\cdot$ PDF p.55 $\cdot$ 75 s $\cdot$ fim previsto \textbf{42:30}}\par\vspace{1mm}
{\large\bfseries\color{ink} The measured trade, and four declared limits}\par\vspace{2mm}
...
\end{itemize}
\vspace{0.5mm}{\small\textbf{N\'UMEROS} --- capítulo reporta cerca de 4,2 milhões de parâmetros $\cdot$ parâmetros no Alabama contra 1,1 milhão dos dois $\cdot$ dois dedicados somados, e 5,2 contra 2,0 na $\cdot$ pequenos, o maior deles 0,87 no Alabama: é}\par
...
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S50} $\cdot$ slide \textbf{50} $\cdot$ PDF p.58 $\cdot$ 55 s $\cdot$ fim previsto \textbf{43:55}}\par\vspace{1mm}
...
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S51} $\cdot$ slide \textbf{51} $\cdot$ PDF p.59 $\cdot$ 45 s $\cdot$ fim previsto \textbf{44:40}}\par\vspace{1mm}
{\large\bfseries\color{ink} The contribution, in one block}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Esta é a mesma tela que eu mostrei no começo, com as mesmas palavras, e agora ela tem a evidência atrás.}\par\vspace{1.5mm}
...
\vspace{1.5mm}{\footnotesize\color{soft} Esta é a mesma tela que eu mostrei no começo, com as mesmas palavras, e agora ela tem a evidência atrás. A metade prática: um modelo, uma passagem, duas predições. O ganho é operacional, não computacional, um artefato para treinar, versionar e implantar. E o preço vai junto: o modelo conjunto é maior que os dois dedicados que ele substitui. A metade científica: as condições, não um sim universal. A representação de entrada e a topologia de compartilhamento decidem se o multitarefa ajuda nestas tarefas. É por isso que o nulo do Capítulo 3 e o resultado positivo do Capítulo 5 não se contradizem.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S52} $\cdot$ slide \textbf{52} $\cdot$ PDF p.60 $\cdot$ 65 s $\cdot$ fim previsto \textbf{45:45}}\par\vspace{1mm}
{\large\bfseries\color{ink} Six limitations, six next steps (1 of 2)}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape As limitações, cada uma amarrada ao passo que ela pede.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{COBRE} --- INTRODUZ as limitações 1, 2 e 3 e os três trabalhos futuros correspondentes}\par\vspace{1mm}
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- apresentar uma limitação sem o passo que ela pede. $\cdot$ ablation na tela ou na fala: o termo não está no GLOSSARY, e a superfície usada é "teste controlado". $\cdot$ As datas não são resultado, e nenhum número de resultado entra aqui.}\par
\vspace{1.5mm}{\footnotesize\color{soft} As limitações, cada uma amarrada ao passo que ela pede. Eu prefiro dizê-las antes de serem perguntadas. Não são desculpas: são seis experimentos que alguém pode rodar. Primeira, a idade dos dados. Os cinco estados vão de janeiro de 2009 a agosto de 2011, e Istambul tem check-ins em dois blocos, 2012 a 2013 e 2017 a 2018, com cerca de sete em cada dez no bloco mais antigo. O passo é direto: rastros mais novos e mais densos. Segunda, a taxonomia é grossa. Sete classes de topo, e uma divisão mais fina pode mudar o efeito do treino conjunto. Terceira, e esta é a que eu mais gostaria de ver feita: a representação é transdutiva. Treinada sobre o grafo de check-ins de cada conjunto, ela não representa lugar nem usuário novo sem retreinar. O passo é uma variante indutiva, que sustentaria uso numa cidade que cresce. E, na mesma representação, dois testes controlados: variar o acoplamento com a tabela de vetores de lugar pré-treinada, e uma formulação em hipergrafo, em que uma aresta junta as várias visitas de uma sessão.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S53} $\cdot$ slide \textbf{53} $\cdot$ PDF p.61 $\cdot$ 65 s $\cdot$ fim previsto \textbf{46:50}}\par\vspace{1mm}
{\large\bfseries\color{ink} Six limitations, six next steps (2 of 2)}\par\vspace{2mm}
{\color{teal}\textbf{\textrightarrow}}\ \textbf{\itshape Quarta: eu não predigo o próximo lugar exato, então as conclusões valem para próxima categoria e próxima região.}\par\vspace{1.5mm}
{\small\color{teal}\textbf{COBRE} --- INTRODUZ as limitações 4, 5 e 6 e os três trabalhos futuros correspondentes}\par\vspace{1mm}
\vspace{0.5mm}{\small\color{crim}\textbf{NUNCA} --- que o confundimento de par de tarefas foi removido; $\cdot$ ele é limitado pelo controle de par fixo. $\cdot$ pipeline: a superfície é "como as entradas são construídas". $\cdot$ ablation.}\par
\vspace{1.5mm}{\footnotesize\color{soft} Quarta: eu não predigo o próximo lugar exato, então as conclusões valem para próxima categoria e próxima região. Acrescentar o próximo lugar como terceiro alvo reusa a representação que já existe: muda a construção da entrada e acrescenta uma saída. Quinta: fora dos Estados Unidos, a evidência se apoia numa cidade só. Mais cidades ampliam a base. Sexta, e é a mais honesta das seis. O par de tarefas mudou junto com a representação e com a topologia, e nenhuma comparação controlada isolada separa as duas mudanças no resultado final. O que eu tenho é o Capítulo 4 como controle de par fixo para o diagnóstico. E eu sei por que a comparação óbvia não serve: rodar a tarefa estática sob a representação em nível de check-in não é limpa, porque a categoria do lugar visitado é atributo de entrada do nó de check-in, e o alvo ficaria parcialmente legível da própria entrada. Isso decorre do desenho, e não foi medido. O confundimento fica limitado pelo controle de par fixo, não removido. O passo que resolveria é um alvo estático que a representação não carregue já como entrada.\par}
\end{tcolorbox}\vspace{2mm}
\begin{tcolorbox}[breakable,colback=white,colframe=teal!55,boxrule=0.6pt,arc=2pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
{\small\color{soft}\textbf{S54} $\cdot$ slide \textbf{54} $\cdot$ PDF p.62 $\cdot$ 70 s $\cdot$ fim previsto \textbf{48:00}}\par\vspace{1mm}
...
\vspace{1.5mm}{\footnotesize\color{soft} os agradecimentos, terminando na banca: *"E, por fim, aos senhores da banca: obrigado por lerem o trabalho e por estarem aqui. Fico à disposição para as perguntas."*\par}
\end{tcolorbox}\vspace{2mm}
\end{document}
