...
na tela, nos dois slides.
  ...
  > **Practical — what it delivers.** One model, one forward pass, two predictions — the next
  > category and the next region of a visit; one artifact to train, version and deploy, in place
  > of two — **operational, not computational**: the joint model is the larger artifact;
  > **Check2HGI is the reusable part**, independent of the prediction heads on top.
  >
  > **Scientific — what it establishes.** **The novelty is the combination**: per-visit context
  > inside a hierarchical graph-infomax representation; helps **under the final design and
  > evaluation protocol**, not always; the **input representation** is one condition, with
  > architecture and scale *suggested, not isolated*.
...
- **Na tela:** dois blocos, sem manchete. **Reproduz o `[BLOCO-CONTRIBUIÇÃO]` do cabeçalho palavra por palavra.** *(v2 · terceira reescrita, 2026-08-24 — ver a nota abaixo.)*
  - Bloco **Practical — what it delivers**:
    - **one model, one forward pass, two predictions** — the next category and the next region of a visit;
    - **one artifact** to train, version and deploy, in place of two — **operational, not computational**: the joint model is the larger artifact;
    - **Check2HGI is the reusable part** — independent of the prediction heads on top.
  - Bloco **Scientific — what it establishes**:
    - **the novelty is the combination**: per-visit context inside a hierarchical graph-infomax representation;
    - helps **under the final design and evaluation protocol**, not always;
    - the **input representation** is one condition; architecture and scale *suggested, not isolated*.
- **v2 · a segunda reescrita, e por que a primeira estava errada:** o autor apontou duas coisas. (1) **A primeira versão v2 derrubou as outras contribuições** e deixou só o Check2HGI. (2) **O resultado negativo não é contribuição direta** — a contribuição é a conclusão tirada dele, de que o benefício do MTL é **condicional**. As duas procedem, e a segunda derruba a proposta que eu tinha feito a partir do **§6.5 (*Final remarks*)**, que enquadra a contribuição metodológica como *"the sequence of evidence"* e chega a dizer *"the negative result... was its first half"*. ⚠ **Registro que o texto entregue carrega essa moldura**; a decisão de não usá-la na tela é do autor.
  **A metade científica agora vem do §6.2 (*The consolidated answer*)**, verbatim: *"multitask learning helps next-category and next-region prediction under the final design and evaluation protocol developed in this dissertation"* e *"the controlled comparisons identify the input representation as one condition, while the results suggest that the architecture and dataset scale may matter, although their individual effects were not fully isolated"* → `src/chapters/6_conclusion.tex:162-200`.
  **A novidade do Check2HGI é dita como combinação, não como prioridade** (decisão do autor): *"The novelty is this specific combination: per-visit context inside a hierarchical graph-infomax representation"* → `src/chapters/5_mobiwac/02_related.tex:71`. ⚠ **O texto entregue NÃO afirma que o Check2HGI é o primeiro a estender HGI ao nível de check-in.** A única cláusula de prioridade do Cap. 5 (`01_introduction.tex:36`) é sobre o **par de tarefas** — *"to our knowledge, the first work to treat fine-grained region as an end target of equal standing"* —, e o `PLANO §3` bane *"o primeiro"* na Seção 1, onde o S7 vive. É RETOMA do S35, não INTRODUZ novo.
  **A metade prática** vem de Intro §Contributions/Practical e /Software (`1_introduction.tex:432-441`); *"one artifact to train, version and deploy"* de `2_fundamentals.tex:1901-1906`.
  ⚠ **Uma frase foi REMOVIDA por não ter fonte:** *"naming which conditions matter is the contribution"* — **zero ocorrências no `src/` entregue** (medido). Estava na tela desde o deck construído.
  ⚠ **R13 — três cópias:** este texto é literalmente o mesmo em **S7, S51 e no `[BLOCO-CONTRIBUIÇÃO]` do cabeçalho**. Qualquer edição move as três.
  **Terceira revisão, 2026-08-24 (decisão do autor):** a manchete emoldurada *"One model, one forward pass, two predictions"* **saiu**. Razão dele, e procede: ela **já era o conteúdo da metade prática**, então a tela dizia a mesma coisa duas vezes, uma delas em destaque. Ela virou o **primeiro marcador** do bloco prático. E os antigos marcadores 1 e 3 **fundiram-se** — *"one artifact... in place of two"* e *"operational, not computational"* são a mesma ideia (o que se ganha e o que se paga), e separá-los obrigava a lê-la duas vezes. ⚠ **A ressalva que a R13 obriga continua na tela**, agora dentro do marcador fundido. Sem a manchete o slide ganhou altura suficiente para os dois blocos voltarem a `\small` e fecharem com folga — o estouro vertical que custou quatro tentativas desapareceu junto com a redundância.
...
- **Na tela:** *(v2 · 156 → ~88 palavras. A frase do bloco é a única frase completa e É o conteúdo — o slide se chama "in one sentence".)*
  ...
  - Tabela, células em fragmento:

  | Model | What it added |
  |---|---|
  | DGI | patches against a global graph summary; |
  | HGI | the place–region–city hierarchy; |
  | MTLnet | first joint model: hard parameter sharing; |
  | Check2HGI | infomax down to the check-in — one vector per visit. |

  - Diagrama tikz dos quatro níveis (city / region / place / **check-in** tracejado, Ch. 5), agora em `\small`.
- **v2 · o que saiu da tela, e onde ficou:** *"because the data itself says which pairing is the true one"* (a cauda da frase do bloco) → fala do próprio S8. As glosas das células — *"by maximizing mutual information between local patches and a global graph summary"*, *"extends graph infomax over the POI, region and city hierarchy to yield region-aware place embeddings"*, *"place embedding and hard parameter sharing"*, *"graph infomax extended to the check-in, so each visit carries its own vector"* → fala do S8, onde já estavam por extenso. A tabela **continua sendo o artefato do slide** (regra 4): o que encolheu foi a prosa dentro das células, não a estrutura.
...
- **Na tela:** *(v2 · 2ª varredura, 2026-08-24 — este campo foi **derivado mecanicamente do frame vigente em `slides/main.tex`**, não transcrito à mão, porque nesta passada o `.tex` foi editado primeiro. A fonte da fala continua sendo este arquivo.)*
  - Bloco **Spatial, 64 d**:
    - **Why:** space entered only through graph topology, never as a continuous coordinate. **SIREN** — sinusoidal activations over normalized coordinates; **Sphere2Vec-M** — multi-scale, spherical, geodesic distance preserved; one contrastive loss for both: positive under **10 km**, negative over **70 km**;
  - Bloco **Temporal, 64 d**:
...
- **Na tela:** *(v2 · 2ª varredura, 2026-08-24 — este campo foi **derivado mecanicamente do frame vigente em `slides/main.tex`**, não transcrito à mão, porque nesta passada o `.tex` foi editado primeiro. A fonte da fala continua sendo este arquivo.)*
  - [figura: fig2_model] **Two inputs, one model** — the category task reads the window of per-visit vectors (**semantic stream**); the region task reads the same window, each visit as its region node's vector (**spatial stream**); **private per-task encoders** — no shared weights; **the shared trunk** — a cross-attention stack of two blocks; attention lets each stream read the other's features, each keeping its own feed-forward weights;
  - Bloco **(sem título)**:
    - The tasks share by **exchanging information between per-task streams**, not by owning hidden layers in common;
...
- **Na tela:** *(v2 · 2ª varredura, 2026-08-24 — este campo foi **derivado mecanicamente do frame vigente em `slides/main.tex`**, não transcrito à mão, porque nesta passada o `.tex` foi editado primeiro. A fonte da fala continua sendo este arquivo.)*
  - **Category: macro-F1** — reference point the **majority-class floor**, **5.7 to 7.3** depending on the dataset; **Region: Acc@10** — the share of test visits whose true region is among the ten highest-scoring predictions; *it does not separate first place from tenth*; **OOD-discounted Acc@10** — a region absent from the training fold counts as an error; **Reference points for region** — the **dedicated single-task model**, and the **Markov-1 floor** over region transitions, **51 to 72** Acc@10 under the same windows and folds;
