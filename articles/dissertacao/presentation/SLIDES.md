...
- **Seção/subseção:** 2.4   **Tempo:** 40 s
- **LEDGER:** INTRODUZ macro-F1 · INTRODUZ o piso de classe majoritária
- **Na tela:** *(v3 · 2026-08-25 — este campo foi **derivado mecanicamente do frame vigente em `slides/main.tex`**, não transcrito à mão, porque nesta passada o `.tex` foi editado primeiro. A fonte da fala continua sendo este arquivo. Mudança desta passada: o ponto de referência ganhou o par concreto Flórida, a pedido do autor — ver Proveniência.)*
  - **macro-F1** — the mean of the per-category F1 scores; every category counts equally, so rare ones matter. Out of 100; **why** — the class distribution is imbalanced (**Food is roughly one third** of the check-ins in a representative state), so plain accuracy hides poor performance on rare classes; **what it does not do** — it does not show *which* classes improve, and it can be low while overall accuracy is high; **the loss is not reweighted** — unweighted cross-entropy; reference point — the **majority-class floor**, a predictor that always answers the most common category: at Florida, **24.7% accuracy** but only **5.7 macro-F1** — the gap this metric is chosen to expose;
- **Fala (PT):** "A métrica de categoria dos três estudos é a macro-F1: a média das F1 por categoria, com cada categoria pesando igual. A razão é a distribuição. Food é cerca de um terço dos check-ins num estado representativo, e uma acurácia simples esconderia o desempenho nas classes menores. Ela também tem um custo, e eu digo qual: a macro-F1 não mostra que classe melhorou, e pode ficar baixa mesmo com acurácia alta. Duas coisas que costumam ser perguntadas, e eu já respondo. A perda não é reponderada, é entropia cruzada sem peso. E toda macro-F1 que eu disser vem com o ponto de referência dela, que é o piso de classe majoritária. Um exemplo concreto mostra por quê: na Flórida, esse piso acerta 24,7% por acurácia simples, mas cai pra 5,7 de macro-F1 quando cada categoria pesa igual — é exatamente a lacuna que essa métrica existe pra expor."
- **Proveniência:** definição, equação e a leitura de fronteira → `chapters/2_fundamentals.tex:1626-1641` (Eq. 2.x de macro-F1; "Food accounts for roughly one third"; "The models use unweighted cross-entropy rather than a reweighted loss"). Piso de classe majoritária → `:1686-1690` ("a majority-class predictor for category classification"). Par Flórida 24,7% acurácia / 5,7 macro-F1 → coluna *Majority* da Tab. 8, linha FL (`tables/mobiwac/datasets.tex:33`) e o mínimo do piso, nomeado como Flórida, em `chapters/5_mobiwac/06_results.tex:54-56` ("between 5.7 and 7.3 macro-F1... lowest at Florida"). A fórmula que liga os dois números (macro-F1 de um preditor constante = `2p/(7·(1+p))`, `p` = acurácia) e a extensão aos outros cinco estados (AL é o máximo, 7.3, não nomeado no texto entregue) estão documentadas em `docs/studies/closing_data/v18/MAJORITY_CLASS_FLOOR_PER_STATE.md`.
- **Nunca dizer:** chamar de "macro-F1" os valores impressos dos Caps. 3 e 4 (são uma F1 por categoria). Fora do par Flórida acima — registrado aqui de propósito, uma vez, pra ilustrar por que se usa macro-F1 — nunca ler a coluna *Majority* da Tab. 8 de nenhum outro estado como se já fosse a macro-F1 do preditor de classe majoritária: uma é a fração de rótulos na classe mais comum (o que a tabela imprime), a outra é o F1 desse preditor (o que precisa ser calculado a partir dela). O par da Flórida só pôde entrar aqui porque os dois números — 24,7% e 5,7 — já estão impressos no texto entregue (Tab. 8 e p. 80); para qualquer outro estado, o macro-F1 do piso não está impresso em lugar nenhum e precisa vir de `docs/studies/closing_data/v18/MAJORITY_CLASS_FLOOR_PER_STATE.md` antes de ser dito em voz alta.
