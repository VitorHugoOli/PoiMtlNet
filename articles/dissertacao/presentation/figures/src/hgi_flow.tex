...
\tikzset{
  gpic/.pic={
    \draw[line width=0.8pt, opacity=0.45]
      ...
      (-0.34,0.15)--(0.28,0.06);
    \fill (-0.34,0.15) circle (1.3pt) (-0.06,0.28) circle (1.3pt)
          (0.28,0.06) circle (1.3pt) (0.06,-0.24) circle (1.3pt)
          (-0.28,-0.17) circle (1.3pt);
  }
}
\begin{tikzpicture}[
    font=\small,
    ...
    tall/.style={box, minimum height=1.35cm},
    dboxA/.style={box, draw=discA, line width=1.1pt, minimum height=0.75cm},
    dboxB/.style={box, draw=discB, line width=1.1pt, minimum height=0.75cm},
    dboxL/.style={box, draw=black!55, line width=1.1pt, minimum height=0.75cm},
    vcell/.style={draw=secondary!70, line width=0.8pt, minimum width=3.4mm,
                  minimum height=1.8mm, inner sep=0pt, fill=white},
    ...
    conn/.style={circle, draw=secondary, line width=1pt, fill=white,
                 minimum size=5mm, inner sep=0pt, font=\small, text=secondary},
    ...
    cmpA/.style={->, line width=0.95pt, draw=discA, dashed, shorten >=2pt},
    cmpB/.style={->, line width=0.95pt, draw=discB, dashed, shorten >=2pt}
]
\def\yA{2.30}\def\yB{1.25}\def\yC{0.20}\def\yD{-0.85}

% ======================= LINHA 1 -- nivel POI, do encoder ao region raw
\node[tall, text width=1.6cm] (cat) at (1.10,1.775)
...
\draw[flow]  ([yshift=5.25mm]cat.east)  -- (2.85,\yA);
\draw[nflow] ([yshift=-5.25mm]cat.east) -- (2.85,\yB);

\node[tall, text width=1.1cm] (pgcn) at (5.10,1.775) {\textbf{POI}\\\textbf{GCN}};
\draw[flow]  (3.75,\yA) -- ([yshift=5.25mm]pgcn.west);
\draw[nflow] (3.75,\yB) -- ([yshift=-5.25mm]pgcn.west);
...
\draw[flow]  ([yshift=5.25mm]pgcn.east)  -- (pva.west);
\draw[nflow] ([yshift=-5.25mm]pgcn.east) -- (pvb.west);

\node[tall, text width=1.55cm] (mha) at (8.95,1.775)
      {\textbf{Multi-Head}\\\textbf{Attention}};
\draw[flow]  (pva.east) -- ([yshift=5.25mm]mha.west);
\draw[nflow] (pvb.east) -- ([yshift=-5.25mm]mha.west);
...
\draw[flow]  ([yshift=5.25mm]mha.east)  -- (rra.west);
\draw[nflow] ([yshift=-5.25mm]mha.east) -- (rrb.west);
...
\node[tall, text width=1.25cm] (rgcn) at (4.60,-0.325) {\textbf{Region}\\\textbf{GCN}};
\draw[flow]  (2.40,\yC) -- ([yshift=5.25mm]rgcn.west);
\draw[nflow] (2.40,\yD) -- ([yshift=-5.25mm]rgcn.west);
...
\draw[flow]  ([yshift=5.25mm]rgcn.east)  -- (rea.west);
\draw[nflow] ([yshift=-5.25mm]rgcn.east) -- (reb.west);

% so' o ramo ORIGINAL sobe: a raia vermelha simplesmente nao continua daqui.
\node[tall, text width=1.85cm, minimum height=1.0cm] (awa) at (8.10,\yC)
...
% leitura da faixa.
\node[dboxB, text width=2.15cm] (drc) at (5.00,-2.675)
      {Discrimination\\Region--City};
\node[dboxL, text width=1.0cm] (loss) at (9.00,-2.675) {Total\\loss};
\node[dboxA, text width=2.15cm] (dpr) at (12.00,-2.675)
