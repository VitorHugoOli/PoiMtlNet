...
\addlinespace
% [2026-09-02, B-13] ELEVENTH ROW. Content defect, not wording: the published claim's SCOPE is
% falsified by the published table. Twenty-one cells; HMRM is the highest in exactly one, Recall /
% Nightlife (42.13 against 36.77 and 25.80).
% [2026-09-02, CORRECTED BEFORE DEPOSIT] An earlier draft of this row and of its comment claimed the
% gap was "wider than the three standard deviations involved". THAT WAS FALSE and it was mine.
% HMRM - Single = 5.36 against the single-task model's own sd of 5.86, so the margin sits INSIDE it
% (only HMRM - MTL = 16.33 clears its sd of 3.54). Worse, it was an inferential claim in a chapter
% that runs no inferential test -- which is exactly what the neighbouring Table B.2 row says about
% the word "significantly". The clause is gone. The defect this row records does not need it: the
% published prose says HMRM is never the highest value, and the published table prints HMRM as the
% highest value once. That is a statement about what the table shows, not about significance.
% Verified four ways before this row was written: this chapter's tables/cbic/category.tex:30, the
% published article's own tables/category_result.tex:25 (same column order, same values), the
% delivered banca.pdf as rendered on p.48, and a mechanical parse of all twenty-one cells.
% THE NUMBER WAS ALREADY KNOWN AND THE CONSEQUENCE WAS NOT. src_utils/adaptation_ledgers/
% 3_cbic_ADAPTATION_LEDGER.md B7 (round 4) records this exact cell -- "HMRM 42.13 > Single 36.77
% bold" -- while fixing how the table's bolding is explained in the caption. Nobody connected it to
% the prose claim fifteen lines earlier. Found again 2026-09-02 because a banca reader highlighted
% the sentence on the delivered PDF (wrapup/REVISAO_BANCA_PDF.md, B-13); author-confirmed same day.
% SECOND EDIT TO ONE PUBLISHED SENTENCE. Table B.2 already carries this sentence, for removing
% "significantly" and replacing "across all" with "in every". That row's right column was re-quoted
% in the same commit so the appendix does not describe chapter text that no longer exists. The two
% rows are deliberate: B.2 is why the significance verb went, this one is why the scope narrowed.
The comparison against HMRM is stated over every POI category and all three per-category
measures. The table the sentence cites reports twenty-one cells, and HMRM holds the
highest value in one of them: recall in the Nightlife category, where HMRM reaches 42.13
against 36.77 for the single-task model and 25.80 for the joint one. &
Scope reconciled to the table: the claim now holds for every category in F1-score and
precision, and for every category except Nightlife in recall. The remaining twenty cells
confirm the published claim, and HMRM is the lowest of the three values in all fourteen
F1-score and precision cells, so the chapter's conclusion for this task is unchanged. The
correction narrows a claim stated in the chapter's favor, so it runs against the chapter's
own interest. \\
\addlinespace
% [2026-09-02, emphasis] TWELFTH ROW. This is a DEPARTURE from the published emphasis, decided by the
% author on 2026-09-02, and it reverses a preservation choice this project made deliberately: ledger
% entry B7 recorded that the published bolding was "preserved exactly as published" PRECISELY because
% HMRM is the row maximum in one cell and never appeared in bold. The author's ruling makes one
% convention hold across every results table in the document -- bold the row maximum, underline the
% second, bold both when they tie at the printed precision -- carried down from the correction he made
% for the defense presentation (presentation/slides/main.tex:1620-1625). Applied by script over the
% printed values, with the proof that stripping all emphasis returns the file byte-identical, so no
% digit moved; independently re-checked row by row against a row-max rule (96 rows, 2 ties, 0
% mismatches, and the same check reports 91 mismatches on the pre-change tables, so it sees defects).
% It converges with the B-13 row above: Recall / Nightlife now shows HMRM in bold, so the table states
% visually what the corrected sentence states in words.
The emphasis marks the better of the two proposed models in each row and never the
baseline, so in the one cell where the baseline holds the largest value, recall in the
Nightlife category, the largest value carries no mark and a smaller one does. The
convention is not declared beyond the caption's own sentence, and it differs from the one
used in the article's other results table, where the row maximum is in bold and the second
is underlined. &
Brought under the convention the rest of the dissertation uses: in each row the largest of
the three values is in bold and the second largest is underlined. The recall entry for the
Nightlife category therefore shows the baseline in bold, which agrees with the corrected
sentence in the same section. No value changes; only which cells carry emphasis. \\
\addlinespace
% [2026-09-02, B-14/B-15] THIRTEENTH ROW. The banca reader marked the two target F1 values and then
% asked, on the next highlight, "so it only tests for one of them?". The table reports ONE epoch
% count for the MTL model, 3.2, which equals the Next row and differs from the Category row's 3.8,
% and the caption promises "epochs to reach the target F1-score" while the MTL model has two targets.
% THE AUTHOR DOES NOT REMEMBER which target stopped the clock and ruled on 2026-09-02 to close it
% with what exists: "nao lembro, se puder acho melhor corrigir com o que temos e deixar isso para
% tras; e ruim, mas nao critico para o resultado final". So the caption declares the gap.
% ⚠ IT DECLARES THE WAY OUT IN THE SAME BREATH, deliberately. A limitation stated without saying what
% it does not touch is what made the same reader stumble at B-12 two sections earlier, where the
% chapter confessed the sample-level split and the resolution sat fifteen pages back. Here the 2.3x
% wall-time comparison holds under either stopping criterion, because it sets one joint run against
% the sum of two dedicated runs, and the caption says so.
The convergence table reports a single epoch count for the joint model, which is trained
against two targets, one per task, and the caption describes the column as the epochs
needed to reach the target F1-score. Which of the two targets the reported count
corresponds to is not recoverable from the record. &
The caption states both facts: that the joint model has two targets and a single recorded
epoch count, and that which target stopped its clock is not recorded. It also states what
the gap does not reach: the wall-time comparison the section draws sets one joint run
against the sum of two dedicated runs, so it holds under either stopping criterion. No
value changes. \\
\end{longtable}
}
