...
To ensure a robust evaluation of our models, all experiments were conducted using a 5-fold cross-validation methodology. % [round14] STANDALONE. Two forward pointers to Chapter 5 removed from this paragraph so the
% article body carries no dissertation structure. What they disclosed is KEPT in full: the split is
% stratified by sample and not by user (so one user's check-ins may span the train/validation
% boundary), and the five folds are one repetition at a single seed. Those are the limitations; the
% comparison to a later chapter's stricter protocol was the frame speaking, and the frame still
% makes it (2_fundamentals.tex section on evaluation, and Appendix B's adaptation record).
...
%
% [NEEDS SIGN-OFF 16 | CLOSED] Author: "Nao precisamos comentar nada." The hyperparameter-search gap
% stays UNSTATED -- do not add a sentence about it. The strongest form the evidence would support is a
% claim about how the study was conducted, not a record recovered from code, so it is not ours to add.
% The cross-file consequence this block flagged is DONE: 2_fundamentals.tex no longer says Chapter 3
% reports cross-validation "without identifying the split axis"; it now states that Chapters 3 and 4
% stratify samples so one user's check-ins may occur in both training and validation. Verified
% 2026-08-04.
...
% [2026-09-02, B-13, author-approved; ledger entry B9, errata B.1 row added by the registry] The
% universal "in every POI category in terms of F1-score, precision, and recall" was false by one cell
% of 21 in tables/cbic/category.tex: Recall / Nightlife reads MTL 25.80, Single 36.77, HMRM 42.13, so
% HMRM has the higher recall there (same cell in the published table, CBIC___MTL/tables/
% category_result.tex:25; ledger entry B7 of round 4 saw the cell as a bolding question and did not
% connect it to this sentence). Reconciled to the table, weakening in favor of the baseline (the
% B2/B3 rule). The published sentence said "significantly outperform HMRM across all POI categories";
% "significantly" had already been removed by the earlier errata round, so this is the second edit
% to one published sentence and Table B.2 of the supplement quotes the current form. The Shopping
% and Food examples that follow are F1 values and remain correct.
As shown in Table~\ref{tab:cbic:category}, both our MTL and Single models outperform HMRM \cite{chen2020modeling} in every POI category in terms of F1-score and precision, and in every category except Nightlife in terms of recall. For instance, in the `Shopping' category, our MTL model achieves an F1-score of $62.51 \pm 0.94$ compared to HMRM's $46.69 \pm 0.81$. Similarly, for `Food', the MTL model scores $57.43 \pm 1.46$ against HMRM's $28.44 \pm 0.42$. While both MTL and Single models are competitive, the Single model shows marginally better F1-scores in several categories like `Community' ($53.11 \pm 0.58$) and `Outdoors' ($47.75 \pm 0.89$), whereas the MTL model excels in `Food' and `Shopping'. Overall, our approaches demonstrate a substantial improvement over the HMRM baseline for this task.
...
% [NEEDS SIGN-OFF 17 | CLOSED] Author: "This is done in the pendencias_resolvidas.md" -- the reading
% is settled there (PENDENCIAS_RESOLVIDOS 5.3): the sentence is about the per-category comparison
% looking worse than the aggregate, i.e. the split of the leads between the two models, NOT about the
% imbalance of the category distribution. Those are two different claims; this is the one that was
% written.
% WHAT THE REWRITE PRESERVED, kept because a future reader may think the sentence was strengthened:
% the published wording is "Also, it is important to notice that since we have an unbalanced result
% for the MTL and single, this could lead to the worse of other results" (verbatim from
% articles/CBIC___MTL/sections/results.tex:34). No number, table cell, or verb of verdict changed, and
% the hedge is still "could", not "does". The chapter reports no inferential test and this sentence
% asserts none. Errata row recorded in the supplementary volume (chapters/apx_b_errata.tex, Table B.2).
