...
\begin{itemize}
  ...
  % [2026-09-08, AUTHOR-AUTHORIZED errata batch, Table B.5 item] "the first work to treat ... as an end
  % target of equal standing" -> the hedged form the chapter's own related-work section already uses
  % ("is underexplored"). The frame's provenance record (1_introduction.tex) documents eight OpenAlex
  % probes that refuted "pioneering" and bans "the first"; this claim had survived the ban. Wording
  % copied from the article's revised introduction.
  \item A single model for joint next-category and next-region prediction, sharing semantic context across tasks while keeping a private spatial path for the region task; to our knowledge, fine-grained region as an end target of equal standing is underexplored (Section~\ref{sec:mobiwac:related-tasks}). In one forward pass, it outperforms a dedicated
