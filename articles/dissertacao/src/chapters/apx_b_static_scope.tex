...
%
% NUMBER CORRECTED WHILE REWRITING, and it was wrong in the published-adjacent text: the range was
% "between 284 and 365 distinct values per state". MEASURED from the five state parquets at
% /Users/vitor/Desktop/mestrado/data/checkins_by_state, column `spot_categories` (NOT `fclass` --
% that name appears only in docs/context/EMBEDDINGS.md and does not exist in the data):
%   Alabama 284 | Arizona 306 | Florida 325 | California 334 | Texas 377
%   places mapping to more than one top-level category: 0 in every state
% So the maximum is 377, not 365. The "none maps to more than one" claim is confirmed exactly.
%   df.groupby('spot_categories')['category'].nunique()  ->  max 1 in all five states
% The example strings are verbatim from Alabama's column: 'Coffee Shop', 'Seafood', 'Airport'.
...
Gowalla state subsets the collection uses: the venue type takes between 284 and 377 distinct values
