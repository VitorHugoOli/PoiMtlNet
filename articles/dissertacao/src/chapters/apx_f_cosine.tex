...
Chapter~\ref{ch:mobiwac}. The two tasks turn out not to disagree, and the way they fail to disagree
says more than the absence of conflict would.
...
The observations are 3,900 epoch-level cosines from four datasets. Florida contributes 3,150 of them
across twelve configurations, which vary the loss weight, the schedule of the weighting coefficient,
and the training procedure; Alabama, Arizona, and Georgia contribute 250 each at the canonical
configuration. Every run is five-fold user-disjoint cross-validation over fifty epochs, so one
configuration on one dataset is five series of fifty values, and two of Florida's configurations
carry a partial re-run on top of theirs.
...
Three of those four datasets, Florida, Alabama, and Arizona, are among the six
Chapter~\ref{ch:mobiwac} reports on. The fourth, Georgia, is a further Gowalla state the
dissertation does not otherwise use; it enters because the diagnostic could be run on it cheaply,
and it is reported because dropping a measured dataset would be a choice about the evidence. That
chapter reaches the same conclusion from a smaller measurement made during development, on an
earlier preparation of the data and over four seeds rather than per-epoch series. The two measure one
quantity separately and their numbers are not interchangeable, so this appendix supersedes nothing
there and repeats none of its figures.
...
training run, so a statistic that treats them as independent draws reports more confidence than
the data hold. The unit of independence is the fold, and for Florida, whose twelve configurations
reuse the same five folds, the configuration. Every test below therefore runs on fold means for
Alabama, Arizona, and Georgia, five per dataset, and on configuration means for Florida, twelve in
all. Where a count of observations appears, it describes the data and is not a test's sample size.
...
\cite{lakens2017tost}. That margin is a judgment about relevance, and worth stating as one. At
$|\cos| < 0.05$ the projection of one task's update onto the other's direction is under five percent
of its length, smaller than the epoch-to-epoch variation in either gradient and smaller than any
effect a gradient-balancing method could act on. The two-point margin Chapter~\ref{ch:mobiwac} uses
for the region task is a different quantity on a different scale; the two should not be read against
each other.
...
\begin{figure}[htbp]
    ...
    \caption{The two tasks' gradients on the shared trunk are orthogonal, on four datasets. The
    shaded band in each panel is the equivalence margin of $\pm 0.05$. Panel (a) pools all 3,900
    epoch-level cosines; panel (b) gives each dataset's mean and 95 percent confidence interval on
    the independent unit named beside it, that unit's own values behind them as faint dots; panel
    (c) follows the mean cosine over the fifty training epochs, one line per dataset, labeled with
    its mean per-fold slope and its count of negative folds. Every mean lies inside the margin, and
...
Two departures from that flat picture are worth reporting rather than smoothing, and both appear on
the smaller datasets. The first is a small positive tendency that recurs. All five of Alabama's fold
means are positive,
averaging $+0.0112$, and so are all five of Georgia's, averaging $+0.0039$; both have confidence
intervals excluding zero, and both are equivalent to zero under the margin. Two datasets pointing
the same way independently is a more interesting observation than either alone. It is still not a
significant effect, and the reason is a property of the design rather than of the data. The exact
sign test returns $0.0625$ for both, the smallest value it can return at five observations, so no
distribution-free test could reach significance here however consistent the pattern. A $t$-test
does reject at both ($p = 0.013$ and $p = 0.009$), but at five observations that rests entirely on
assuming normality. The defensible reading is a consistent positive tendency, recurring across
datasets, that five folds per dataset cannot establish. Its direction deserves a remark: a positive
cosine is mild cooperation rather than conflict, so what the smaller datasets hint at points away
from interference, not toward it.

The second is a decline over training. On Alabama the mean cosine falls from about $+0.058$ across the
first five epochs toward zero, with all five per-fold slopes negative; Georgia repeats the pattern,
also five of five, while Arizona is mixed at three of five and Florida, on sixty series, is flat. A
$t$-test on the five slopes would reject for Alabama and Georgia. That is the same $n = 5$ footing on
which this appendix has just declined to call the positive mean significant, the exact sign test again
returning $0.0625$, so the decline gets the same treatment: a consistent tendency, not an established
effect, held to the standard used one paragraph earlier rather than a weaker one. Florida, with sixty
independent units, is the only series able to settle the question, and it shows no trend. The decline
stays inside the margin throughout and moves the cosine toward zero rather than away, and no
...
parameters. Each requested update has almost no component along the other's direction, so the trunk
can serve both without either task's improvement being paid for by the other. Two consequences
follow, and together they are why this measurement belongs in the dissertation rather than in a
laboratory notebook.
...
reproject task gradients so a dominant task cannot overwrite a weaker one. Orthogonal gradients
leave such a method nothing to resolve, which is why hard sharing costs nothing in this architecture
and why Chapter~\ref{ch:mobiwac} reports that no balancer improved on a fixed loss weighting. The
measurement explains the finding.

The second concerns the arc of the three studies. The investigation opened on the hypothesis that the
sharing scheme was what limited joint training, and the first study's null result was read that way at
the time. Had the two tasks been in conflict, a better sharing scheme or a better balancer would have
been the remedy. They were not, so the limit lay elsewhere, and the second and third studies located
it in the input representation. This appendix is the mechanism behind that claim: there was little
interference to fix, and so little for a change of architecture to recover.
...
Taken together, the two axes suggest orthogonality belongs to this task pair rather than to the
tuning. Next category asks which of seven kinds of place a user visits next; next region asks which
tract or neighborhood. Both read the same trace, and each predicts a different projection of one
event. Those projections look close to statistically independent given the shared representation,
which would explain why the requested updates carry so little of each other.
...
decides where the two tasks meet and how much of the network they share, so it is at once the factor
most likely to change the answer and the one this appendix did not vary. Nothing here says the
gradients would stay orthogonal in a model that shares more of its depth, couples the tasks in a
cascade, or shares an output layer. Anyone who wants to know can run the same diagnostic and change
nothing else: record the per-epoch cosine on the candidate's shared parameters, under a fixed loss
weighting, across one dataset's folds, then test the fold means for equivalence against a margin
chosen before looking. A cosine equivalent to zero there makes the property the task pair's;
otherwise it is this architecture's, and Section~\ref{apx:cosine:mechanism} applies only to models
