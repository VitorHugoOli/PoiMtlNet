...
implementation, and the following chapters do not rely on it.
\end{chapterpreface}
...
To evaluate our model, we use a subset of the public \emph{Gowalla} check-in dataset, originally collected by Cho \textit{et al.}\,\cite{cho2011gowalla}. We focus our analysis on the data from the U.S. state of Florida, which is one of the most active regions in the original dataset. This subset comprises 10{,}460 users, 64{,}454 unique Points-of-Interest (POIs), and 960{,}520 check-ins.
% [VERIFY -- N_users basis, author sign-off before the advisor build]
% Numbers recomputed 2026-07-24 from the COMMITTED CBIC-era artifact
% data/output/florida_dgi.zip::florida_test/filtrado.csv -- the exact Florida DGI input the CBIC
% models consumed (independently re-counted this session: 960,520 rows / 10,460 users / 64,454
% POIs). This is the FAITHFUL CBIC-era record. NOTE: the current ETL code CANNOT reproduce these
% counts -- the category-mapping files were expanded 2026-04-14 (~11 months after the CBIC-era
% extraction), so a fresh run over-counts (raw 21052/76544/1407034; <5-filtered 13935/76266/
% 1392262) on the 2026 mapping, not CBIC-era. POI (64,454) and check-ins (960,520) are
% corroborated by CoUrb's published FL row (65,009 / 990,518, cross-check only per CBIC ERRATA,
% within a few %). OPEN [VERIFY]: N_users 10,460 (filtrado) vs CoUrb 20,301 -- a ~2x gap the
% committed artifacts cannot resolve (a 2025-vs-2026 convention/mapping difference); the author
% must confirm the basis or time-index it per chapter before the advisor build.
% Full analysis: ../../src_utils/cbic_recompute_result.md (the authoritative one, from the ETL worker).
% [VERIFY] The three dataset statistics above were never filled in the published paper
% (ERRATA.md, non-citation erratum #1). Sanctioned path: a repo-committed script over the
% CBIC-era Florida pipeline (Gowalla FL, users with <5 visits dropped), author-approved
% before insertion. CoUrb's published FL row is a cross-check only, not a source.
...
As shown in Table~\ref{tab:cbic:category}, both our MTL and Single models significantly outperform HMRM \cite{chen2020modeling} across all POI categories in terms of F1-score, precision, and recall. For instance, in the `Shopping' category, our MTL model achieves an F1-score of $62.51 \pm 0.94$ compared to HMRM's $46.69 \pm 0.81$. Similarly, for `Food', the MTL model scores $57.43 \pm 1.46$ against HMRM's $28.44 \pm 0.42$. While both MTL and Single models are competitive, the Single model shows marginally better F1-scores in several categories like `Community' ($53.11 \pm 0.58$) and `Outdoors' ($47.75 \pm 0.89$), whereas the MTL model excels in `Food' and `Shopping'. Overall, our approaches demonstrate a substantial improvement over the HMRM baseline for this task.
