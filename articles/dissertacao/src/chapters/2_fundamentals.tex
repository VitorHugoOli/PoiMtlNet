...
% GLOSSARY DEPENDENCY, fail-closed rule, unresolved: this passage uses two terms the registry
% does not hold, "bilinear discriminator" and "logistic function". GLOSSARY.md is not mine to
% edit, so the entries are PROPOSED in the round-6 report for the author to approve, and the rule
% says the entry lands before the term does. Until they are registered this paragraph is
% blocked on that approval, not merely awaiting sign-off on its wording. Precedent for the
% narrower word: "discriminator" is already used in published Ch.3 prose (3_cbic.tex:166, "$D$ is
% a discriminator that differentiates each positive example from its corrupted version"), so only
% the modifier "bilinear" and the name of $\sigma$ are new to the document.
