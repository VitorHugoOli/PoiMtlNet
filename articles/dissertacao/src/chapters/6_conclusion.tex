...
% (science/storyline/audit/capacity_baseline_experiment.md section 5).
...
% [2026-09-04, AUTHOR-APPROVED (advisor's list, last open item)] The introduction states four
% objectives (1_introduction.tex, sec:intro:objectives) and this chapter never revisited them by
% name: live count of "objective" in this file was 0. Three sentences below weave them into the
% per-chapter paragraphs, one clause per objective, in the order the introduction gives them:
% objective 1 at the end of the Chapter 3 paragraph, objective 2 at the end of the Chapter 4
% diagnosis paragraph, objectives 3 and 4 after the Chapter 5 protocol sentence. No claim, number
% or verdict is added; each sentence names which objective the paragraph's existing content meets.
% Pinned spans untouched: R13-foldseed9 ("four seeds, each drawing its own five-fold user
% partition") and the chapter opening (R13-aut32). Wording of the objectives follows the
% introduction as rewritten on 2026-09-03 (objective 4 = protocol differences + verdict reading).
...
limited to a place-level input under hard sharing. This is the first objective of
Section~\ref{sec:intro:objectives}, met with a negative answer. The next two studies
examine what happens when these conditions change.
...
representation as the main bottleneck in that configuration. This meets the second
objective, which asked how the input representation affects the result while the
...
Paired tests used the four per-seed means. The representation and the joint model are
the third objective of Section~\ref{sec:intro:objectives}; the protocol that reads them,
together with its differences from the protocols of the first two studies
(Section~\ref{sec:fund:split}), is the fourth.
...
% [2026-08-12] The parameter-count clause is withdrawn from this list with the appendix it named
% (relocated to the supplementary volume; reasoning at its former include site in content.tex). It
% read: "test parameter count as an alternative explanation for the next-category gains". The list
% now claims only what this volume reports.
input representation and identify a possible relation between problem scale and the next-region
gains.
...
% [2026-08-12, AUTHOR RULING] THE CAPACITY PARAGRAPH IS WITHDRAWN with the appendix it reported
% (the parameter-count control, now in the supplementary volume; reasoning at its former include
% site in content.tex). It read: model capacity offers a competing explanation for the next-category
% results; the control widens the dedicated model to the joint model's budget and finds the wider
% model 0.53 macro-F1 points BELOW the narrower one at Alabama, unanimous across twenty folds, with
% a single-seed reading at California consistent with it. The measurement stands and is preserved in
% the supplement. It leaves this chapter because a 0.53-point argument cannot carry weight in a
% document that reports category differences within half a point as equivalent to zero, and because
% the large joint category advantage it was built to explain is not on the final board.
...
% [2026-08-12] The sentence describing the parameter-count experiment and its scope is withdrawn
% with that appendix (see the note above and content.tex). What it said about the LIMITATION is
% unchanged and stated in the sentence that follows: no controlled ablation separates the two
% changes, and the greater similarity of the final pair may contribute to the improvement.
    diagnosis. The greater similarity of
