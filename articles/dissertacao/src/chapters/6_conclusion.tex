...
Does multitask learning help point-of-interest prediction? The answer is conditional, and
identifying those conditions is the dissertation's main finding. The benefit depends on the
representation that the two tasks read and on the topology through which they share
parameters.

Each study's result set the next study's question. The joint model of
Chapter~\ref{ch:cbic} combined a place-level embedding with naive hard sharing, and under
that design it did not consistently outperform the two dedicated models. Negative transfer
between the static and the sequential task was one possible explanation, and an input
representation that served neither task well was another. That verdict was the conclusion of
the time, for that configuration. The study that followed held the architecture fixed and
changed only the input, and it identified the representation, not the sharing scheme, as the
main bottleneck in that configuration. The next step implied by that diagnosis was a
representation built at the level of the check-in rather than the place.
Chapter~\ref{ch:mobiwac} tested one, together with a sharing topology designed for it. Under
that design, one model produces both predictions in a single forward pass. It outperforms
the dedicated single-task models on the category task at all six datasets and on the region
task at four of the six. At the other two, it remains statistically non-inferior within a
two-point margin (TOST).
...
representation. Two controls and the gradient measurements narrow that expectation.

The first control asks whether the category improvement comes from the training signal of
the next-region task, whose output space is much larger. Chapter~\ref{ch:mobiwac} tests this
explanation by fixing the region pathway at its initial values. This removes learning from
the region side, so that task cannot provide a training signal to the category task, but it
leaves the rest of the joint architecture in place. The control used one random
initialization over five folds and was designed for one matched comparison: the fixed-region
joint model against the dedicated category model under the same training configuration. At
Alabama, Arizona, and Florida, the fixed-region model retains its category advantage. Within
this control, the category improvement therefore does not require training transfer from the
region task. The result rules out that explanation, but it does not determine whether the
gain comes from the category encoder, the feed-forward blocks, the added depth,
cross-attention, or a combination of these components.
...
Read together, these results correct an expectation rather than a result. The controls
exclude two explanations within the settings in which they were evaluated: direct
training transfer from the region task and parameter count alone. They do not isolate which
remaining component of the joint architecture produces the gain. The gradient measurements
speak against a third account, since where directional conflict between the two tasks is
small, the benefit of joining them is not the resolution of such a conflict. The relatedness
of the two tasks was the reason to build one model for both, and it is not, on this evidence,
the source of that model's advantage.
...
negative result does not rule it out. Neither outcome follows from the relatedness of the
two tasks, which is the part of the initial expectation that this work corrected. This
dissertation demonstrates one path from the negative result to a positive one and
identifies the conditions tested along that path.
