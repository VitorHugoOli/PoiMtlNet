...
next-region prediction, and what the answer depends on ? The evidence supports a
conditional yes. In the final configuration, the joint model outperforms the dedicated
...
and Arizona, it is statistically non-inferior within a two-point margin (TOST). Thus, under
the evaluation defined in advance, one joint model therefore produces both predictions
in one forward pass without sacrificing either task.

This answer emerged through a chain of findings. Chapter~\ref{ch:cbic} began with a
place-level embedding and naive hard sharing. Its joint model did not consistently
...
\pagebreak
Chapter~\ref{ch:mobiwac} acted on this diagnosis. Check2HGI replaced one vector per
place with one vector per visit. The joint architecture also replaced naive hard sharing
with task-specific streams that exchange information through cross-attention, while the
region output retained a private spatial path. A controlled comparison in
Section~\ref{sec:mobiwac:results-part1} isolates the representation: with the dedicated
model, training configuration, windowing, and folds held fixed, Check2HGI produces
higher next-category macro-F1 than the place embedding HGI across the six evaluated
datasets. The complete joint result cannot be assigned to the representation alone,
however, because the interaction topology and the task pair also changed between the
early and final studies.

The initial premise was therefore incomplete. As Section~\ref{sec:fund:mtl} explains,
multitask learning trains related tasks together in the expectation that a shared
representation will improve generalization. The three studies show that task relatedness
and joint training are not sufficient by themselves. The first result remains valid for
its place-level input and hard-sharing architecture, but it is not a general answer about
multitask learning. The final gain also cannot be reduced to model size. In the
parameter-count control reported in Appendix~\ref{apx:parameter-count-control}, the
capacity-matched dedicated category model scored slightly below the original dedicated
model in Alabama and California and remained below the joint model. In these two
datasets, parameter count alone does not explain the joint model's next-category
advantage. The result points to mechanisms in the joint architecture or its multitask
training that widening the dedicated model does not reproduce.

The final results also suggest that the answer may depend on problem scale. Across the
five U.S. states, the joint model performs better relative to the dedicated model as the
number of regions grows. It is slightly worse in Alabama, matches the dedicated model in
Arizona, and outperforms it in Florida, Texas, and California. The improvement is larger
than two Acc@10 points in Texas and California. This pattern may mean that joint learning
becomes more useful when the model must choose among more regions. However, states with
more regions also tend to contain more check-ins. A controlled experiment is needed to
separate the number of regions from the amount of data and other differences between the
states.

The main lesson is therefore about the conditions under which multitask learning helps.
The strongest controlled finding concerns the input representation, and the final study
shows that a check-in-level representation and an architecture that connects task-specific
pathways form one successful joint design. It does not show that the interaction topology
alone caused the improvement. The controlled comparison with HGI also supports testing
Check2HGI as the input representation in other mobility prediction architectures, although
whether its benefit transfers to those architectures remains untested. The consolidated
answer is not that multitask learning always helps. It is that multitask learning helps
