...
next-region prediction, and what the answer depends on. For the final configuration
studied in this dissertation, the answer is yes. The joint model outperforms the
dedicated single-task models on next-category prediction in all six datasets. On
next-region prediction, it outperforms them in Istanbul, Florida, Texas, and California.
In Alabama and Arizona, it is statistically non-inferior within a two-point margin
(TOST). Thus, under the evaluation defined in advance, one joint model produces both
predictions in one forward pass without sacrificing either task.

This answer is conditional because the three studies evaluated different designs. In
Chapter~\ref{ch:cbic}, a place-level embedding and naive hard sharing did not consistently
outperform the two dedicated models. Chapter~\ref{ch:courb} then kept the architecture
fixed and changed the input. Performance improved after that change, including on the
sequential task in most states. This comparison identifies the input representation as
the main bottleneck for that architecture. It also motivated the check-in-level
representation, which gives each visit its own vector.
Chapter~\ref{ch:mobiwac} combined this representation with an architecture in which
task-specific streams exchange information through cross-attention and the region output
retains a private spatial path. The final results show that this complete design works.
They do not show how much each change contributed because the representation, the
interaction topology, and the task pair all changed between the early and final studies.

\pagebreak
The controlled comparison in Section~\ref{sec:mobiwac:results-part1} isolates the
representation.
With the dedicated model, training configuration, windowing, and folds held fixed,
Check2HGI produces higher next-category macro-F1 than the place embedding HGI across
the six evaluated datasets. This result supports testing Check2HGI as the input
representation in other mobility prediction architectures. Whether the benefit
transfers to those architectures remains untested.

Across the five U.S. states, the joint model performs better relative to the dedicated
model as the number of regions grows. It is slightly worse in Alabama, matches the
dedicated model in Arizona, and outperforms it in Florida, Texas, and California. The
improvement is larger than two Acc@10 points in Texas and California. This pattern may
mean that joint learning becomes more useful when the model must choose among more regions.
However, states with more regions also tend to contain more check-ins. We therefore
cannot tell whether this pattern comes from the number of regions, the amount of data,
or another difference. A controlled experiment is needed before concluding that either
factor causes the larger gains.

The parameter-count control in Appendix~\ref{apx:parameter-count-control} tests one
alternative explanation for the next-category result. In Alabama and California, the
capacity-matched dedicated category model scored slightly below the original dedicated
model and remained below the joint model. Thus, in these two datasets, parameter count
alone does not explain the joint model's next-category advantage. The result points to
architectural or joint-training mechanisms(such as MTL) that widening the dedicated model does not
reproduce.

The negative result of Chapter~\ref{ch:cbic} and the positive result of
Chapter~\ref{ch:mobiwac} therefore do not contradict each other. Multitask learning
provides no automatic benefit, and choosing related prediction tasks does not guarantee
one. The clearest controlled finding concerns the input. In Chapter~\ref{ch:courb},
performance changed after the input changed, while the architecture stayed fixed. The
final study shows that a check-in-level representation and an architecture that connects
task-specific pathways form one successful joint design. However, it does not show that
this interaction alone caused the improvement. The consolidated answer is not that
multitask learning always helps. It is that multitask learning helps next-category and
next-region prediction under the final design and evaluation protocol developed in this
dissertation.
