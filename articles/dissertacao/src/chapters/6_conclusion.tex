...
four superiority findings are secondary results outside that analysis plan. This finds lead for some key point on
why the joint model produces better results.

For the next-category, the controlled comparison between Check2HGI and HGI provides the first explanation. The controlled
comparison in Section~\ref{sec:mobiwac:results-part1} isolates this change: with the
dedicated model, training configuration, windowing, and folds held fixed, Check2HGI
produces higher next-category macro-F1 than the place embedding HGI across the six
evaluated datasets.This result establishes the input
representation as one condition that affects performance in this setting. It also
supports testing Check2HGI in other mobility prediction architectures, although its
benefit in those architectures has not yet been evaluated.
...
For next-region prediction, the results point to a different possible condition: problem
scale. Across the five U.S. states, the joint model performs better relative to the
dedicated model as the number of regions increases. In particular, the improvement
exceeds two Acc@10 points in Texas and California, the states with the largest number of
regions. This pattern suggests that joint learning becomes more useful when the model
must choose among more regions. However, states with more regions also tend to have more
check-ins. So, a controlled experiment is required to separate the effect of region
count from the amount of data and other differences between the states.
...
shows that it helps under the final configuration. Althought, this configuration is the decisive choices to ansrwer
on what the multitask learning depends on. Identifying these conditions is the dissertation's main finding.
...
responded to this diagnosis in two steps. First, it introduced Check2HGI, which replaced one vector per place with one
vector per visit. Follow, for the joint architecture, that replaced naive hard sharing with task-specific streams that
exchange information through cross-attention, while the region output retained a private spatial path.

Together, all these changes define the final
configuration evaluated in this dissertation, answering the second part of the research question: the result is
conditional. The initial premise was incomplete. As Section~\ref{sec:fund:mtl}
explains, multitask learning trains related tasks together in the expectation that a
shared representation will improve generalization. The three studies show that task
relatedness and joint training are not sufficient by themselves. The controlled
comparisons identify the input representation as one condition. The evidence also
suggests that the architecture and problem scale may matter, although their individual
effects were not isolated. The answer therefore depends on the design and evaluation
conditions, not on joint training alone.

These findings support a conditional rather than universal conclusion.
The consolidated answer is therefore not that multitask learning always helps. It is
that multitask learning helps
