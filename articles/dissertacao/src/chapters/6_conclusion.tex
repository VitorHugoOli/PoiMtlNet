...
(TOST). Thus, one joint model produces both predictions in one forward pass without
sacrificing either task under the registered comparison.
...
retains a private spatial path. The final results establish the effectiveness of this
complete design, but they do not isolate each component's contribution because the
representation, interaction topology, and task pair changed between the early and final
studies.

\pagebreak
The next-region results show a secondary pattern across the five U.S. states. The joint
model has a small deficit in Alabama, matches the dedicated model in Arizona, and
outperforms it in Florida, Texas, and California. The gains exceed two Acc@10 points in
Texas and California. This ordered pattern is consistent with a larger benefit in
datasets with more regions. Corpus size also tends to be larger in the same datasets, so
the two factors are confounded. The comparison cannot determine which factor accounts
for the pattern, and it does not show that scale caused the gains. A controlled study
that changes one factor at a time is needed to test this explanation.

A post-submission capacity control narrows one alternative explanation for the
next-category result. In Alabama and California, a dedicated category model was widened
to approximately the parameter budget of the joint model. In both datasets, the wider
model remained slightly below the dedicated model at its tuned, narrower width and well
below the joint model. Matching the parameter count therefore did not reproduce the
next-category gain in these two datasets. This control covers one width point per dataset
and a narrower hyperparameter search than the original dedicated models. It shows that
parameter count alone is not an adequate explanation within this scope, but it does not
identify which part of the joint architecture produces the gain.
...
one. The strongest isolated evidence concerns the input: changing the representation
changed the outcome while the architecture was held fixed in Chapter~\ref{ch:courb}.
The final study then shows that a check-in-level representation and an architecture that
connects task-specific pathways form one successful joint design. The independent effect
of that interaction topology remains open. The consolidated answer is therefore not that
multitask learning always helps, but that it helps next-category and next-region
prediction under the final design and evaluation protocol developed in this dissertation.
