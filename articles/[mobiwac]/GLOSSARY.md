...
| overlapping (stride-1) windows | **sliding windows** (the literature term; sliding implies overlap) | author ruling 2026-07-10: "sliding windows" is the ONE name everywhere (figure, captions, tables); the §5.2 definitional site says "overlapping sliding windows ... one starting at each visit" ONCE to keep the stride-1 property explicit for the leak argument; never "overlapping windows" as a second name. §6's "non-overlapping windowing" (the Markov floor caveat) stays, it names the contrast |
