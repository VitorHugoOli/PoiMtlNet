...
\begin{itemize}
  ...
  \item \textbf{A single model for both tasks.} One model predicts the next category and the next
    ...
    margin at the other four (statistical non-inferiority, assessed with two one-sided tests). The joint model is also the larger model (Section~\ref{sec:discussion}). On category it outperforms the dedicated model
    at one dataset, and the five remaining differences are equivalent to zero within half a point,
    even though the
    dedicated category model receives the wider search
