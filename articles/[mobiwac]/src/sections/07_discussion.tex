...
never seen during training are the single effect that we cannot fully isolate. A planned follow-up trains it on each fold's training places only and embeds unseen visits. Second, we do not build or evaluate a mobility-aware service; it is background motivation, and the paper's claims are the prediction results themselves.
