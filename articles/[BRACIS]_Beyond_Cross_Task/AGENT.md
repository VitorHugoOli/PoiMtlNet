...
- **Multi-seed:** seeds {0, 1, 7, 100} on top of the seed=42 anchor where available. **AL/AZ/CA/TX §0.1 headline rows are now pooled multi-seed (n = 20).** FL remains asymmetric: the cat-side §0.1 Wilcoxon is still the single-seed n = 5 paired comparison, while FL also has auxiliary multi-seed support on the reg-side Δm axis (n = 25) and on recipe selection (n = 25). Disclose this FL-only asymmetry in §7 instead of the older CA/TX/TX-open framing.
...
| 7 — Discussion & Limitations | 1 | Re-attribution: substrate carries cat, architecture costs reg, sign-consistent across 5 states. Limitations: FL cat-side n = 5 ceiling asymmetry; FL `next_region` Markov-saturation; ReHDM scope; encoder enrichment is future work | — |
