...
- **C1 — Substrate is task-asymmetric.** Check-in-level Check2HGI lifts matched-head single-task next-category macro-F1 over POI-stable HGI by **+14.5 pp at AL, +14.5 at AZ, +29.0 at FL, +28.8 at CA, +28.3 at TX**, paired Wilcoxon **p = 0.0312** each (5/5 folds positive at the n = 5 ceiling, head-invariant). On next-region the same matched-head ablation under STAN-Flow shows **HGI nominally ahead by 1.6 to 3.1 pp** — TOST non-inferiority at δ = 2 pp passes at CA/TX (statistically tied), δ = 3 pp passes at FL. A pooled-vs-canonical counterfactual at AL attributes **~72 %** of the cat substrate gap to per-visit context and ~28 % to Check2HGI's training signal — explaining the asymmetry mechanistically (per-visit variance is what cat needs; pooling smooths it away for reg).
- **C2 — Classic MTL tradeoff.** With Check2HGI fixed and a cross-attention MTL backbone, joint training lifts next-category at **four of five states** (AZ +1.20; FL +1.43; CA +1.94; TX +2.02 pp) and is **≈ tied at AL** (Δ = −0.78, within multi-seed STL σ = 0.17 pp). On next-region it pays **8 to 17 pp** vs. a matched-head STAN-Flow STL ceiling at every one of the five states — sign-consistent negative. Cost magnitude varies (FL −7.99 / CA −8.92 / TX −16.69) but is **not** monotone with scale — TX breaks the pattern, so the scale-progression is reported descriptively, not as an inferential claim. Drop-in alternatives (FAMO, Aligned-MTL, hierarchical-softmax reg head) do not recover the gap.
...
- **Multi-seed:** seeds {0, 1, 7, 100} on top of the seed=42 anchor where available — AL+AZ+FL have full multi-seed; **CA/TX are seed = 42 single-seed at submission** with multi-seed extension at {0, 1, 7, 100} as a **camera-ready audit item**. T3/T4 CA/TX cells therefore sit at the n = 5 paired-Wilcoxon ceiling (p_min = 0.0625 two-sided). Disclosed as a limitation in §7.
