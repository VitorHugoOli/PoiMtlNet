...
- **C2 — Classic MTL tradeoff, sign-consistent across states.** With Check2HGI fixed and a cross-attention MTL backbone, joint training lifts next-category by **0 to +2 pp** at every state (AL = −0.78 ≈ tied within seed σ; AZ +1.20; FL +1.43; CA +1.94; TX +2.02) and pays **8 to 17 pp** on next-region Acc@10 vs. a matched-head STAN-Flow STL ceiling at every state — sign-consistent. Cost magnitude varies (FL −7.99 / CA −8.92 / TX −16.69) but is *not* strictly monotone in scale — TX is non-monotone, so the scale-progression is reported descriptively, not as an inferential claim. Drop-in alternatives (FAMO, Aligned-MTL, hierarchical-softmax reg head) do not recover the gap.
