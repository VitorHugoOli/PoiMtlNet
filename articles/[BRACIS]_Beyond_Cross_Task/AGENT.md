...
- "We refute MTL." We do not. MTL gains on cat at four of five states (AZ p < 1e-4, FL p = 2e-06 (n=20), CA, TX directional); AL is small-significantly negative (Δ = −0.78 pp, p = 0.036, magnitude small). MTL is the deployment unit — we position the reg loss honestly as the cost paid for joint single-model deployment.
...
- **T3 — MTL vs STL headline (C2).** Cat F1 + reg Acc@10 + reg MRR for {STL `next_gru`, MTL B9 cat F1; STL STAN-Flow (`next_stan_flow`), MTL B9 reg} × 5 states. **Source: `RESULTS_TABLE.md §0.1` (v10) — sole canonical numerical source.**
- **T4 — Δm joint score (CH22 leak-free).** Δm-MRR / Δm-Acc@10 × 5 states with paired Wilcoxon p. Multi-seed for FL (n = 25 pairs) where available. Source: `RESULTS_TABLE.md §0.2` (v10).
- **T5 — External baselines.** POI-RGNN / MHA+PE for cat; Markov-1-region / STAN / ReHDM for reg; per state. Source: `RESULTS_TABLE.md §0.5–0.6` (v10) + `baselines/next_*/results/<state>.json`.
