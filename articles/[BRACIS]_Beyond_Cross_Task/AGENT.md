...
- **Working title (locked-default 2026-05-01 v3, post-Codex audit):** *Substrate Carries, Architecture Pays: Check-In-Level Embeddings for Multi-Task POI Prediction*. Reframed from the earlier "Scale-Sensitive" title — TX (−16.59 pp on reg) breaks the scale-shrinks-with-data pattern, so the title now leads with the substrate task-asymmetry (paper-grade significant at every state) and the MTL tradeoff (sign-consistent at every state).
...
The full study lives under `docs/`. **The single canonical numerical source is `docs/results/RESULTS_TABLE.md §0` (v10, 2026-05-02).** Read `docs/README.md` and `docs/AGENT_CONTEXT.md` for the post-cleanup study navigation. The earlier paper-closure note `PAPER_CLOSURE_RESULTS_2026-05-01.md` was moved to `docs/archive/check2hgi-post-paper-closure-2026-05-01/` as background provenance only; do not cite it as a primary source. **The leak-free closure says: MTL trails STL on `next_region` by 7–17 pp at every one of the five states**; the older "+6.48 pp MTL > STL on AL" framing in `MTL_ARCHITECTURE_JOURNEY.md` and the F49 docs was a leak artefact (asymmetric C4 leak inflated MTL more than STL).
...
- **C2 — Classic MTL tradeoff.** With Check2HGI fixed and a cross-attention MTL backbone, joint training adds a **small additional cat lift** at four of five states (AZ +1.20 pp p < 1e-4; FL +1.40 pp p = 2e-06 (n = 20); CA +1.68 pp p = 2e-06; TX +1.89 pp p = 2e-06). At AL, MTL is **small-significantly below STL on cat** (Δ = −0.78 pp, paired Wilcoxon p = 0.036 across n = 20 multi-seed fold-pairs; magnitude ~1.9 % relative on a 41 % F1 scale). On next-region it pays **7 to 17 pp on Acc@10** vs. a matched-head STAN-Flow STL ceiling at every one of the five states — sign-consistent negative. Cost magnitude varies (FL −7.34 / CA −9.50 / TX −16.59) but is **not** monotone with scale — TX breaks the pattern, so the scale-progression is reported descriptively, not as an inferential claim. Drop-in alternatives (FAMO, Aligned-MTL, hierarchical-softmax reg head) do not recover the gap.
...
- "We refute MTL." We do not. MTL gains on cat at four of five states (AZ p < 1e-4, FL p = 2e-06, CA p = 2e-06, TX p = 2e-06 — all n = 20); AL is small-significantly negative (Δ = −0.78 pp, p = 0.036, magnitude small). MTL is the deployment unit — we position the reg loss honestly as the cost paid for joint single-model deployment.
...
- **Multi-seed:** seeds {0, 1, 7, 100} on top of the seed=42 anchor where available. **All five states (AL/AZ/CA/TX/FL) §0.1 headline rows are now pooled multi-seed (n = 20).** The last single-seed asymmetry (FL cat-side §0.1) was closed in v11: FL Δ_cat = +1.40 pp p = 2e-06, FL Δ_reg = −7.34 pp p = 1.9e-06. FL also has auxiliary multi-seed support on Δm (n = 25) and recipe selection (n = 25).
...
| 7 — Discussion & Limitations | 1 | Re-attribution: substrate carries cat, architecture costs reg, sign-consistent across 5 states. Limitations: FL `next_region` Markov-saturation; ReHDM scope; encoder enrichment is future work | — |
...
- **T2 — Substrate comparison (CH16, head-invariant at AL/AZ; matched-head replicated at FL/CA/TX).** Cat F1 STL across {Linear probe, `next_gru`, `next_single`, `next_lstm`} × {Check2HGI, HGI} × {AL, AZ, FL, CA, TX}. Source: `FINAL_SURVEY.md` §1-2 + `RESULTS_TABLE.md §0.3`.
- **T3 — MTL vs STL headline (C2).** Cat F1 + reg Acc@10 + reg MRR for {STL `next_gru`, MTL B9 cat F1; STL STAN-Flow (`next_stan_flow`), MTL B9 reg} × 5 states. **Source: `RESULTS_TABLE.md §0.1` (v11, all five states n=20) — sole canonical numerical source.**
- **T4 — Δm joint score (CH22 leak-free).** Δm-MRR / Δm-Acc@10 × 5 states with paired Wilcoxon p. Multi-seed for FL (n = 25 pairs) where available. Source: `RESULTS_TABLE.md §0.2` (v11).
- **T5 — External baselines.** POI-RGNN / MHA+PE for cat; Markov-1-region / STAN / ReHDM for reg; per state. Source: `RESULTS_TABLE.md §0.5–0.6` (v11) + `baselines/next_*/results/<state>.json`.
