...
For each user we form non-overlapping length-9 windows over the chronological sequence and predict the category and region of the tenth check-in (shorter sequences padded). Folds are \textbf{user-disjoint} via \texttt{StratifiedGroupKFold(groups=userid, shuffle=True, random\_state=42)} with five folds, so train/val/test never share a user. Splits are reused across substrates, heads, and recipes so paired tests are well defined. The next-region transition prior is rebuilt per fold from training-fold edges only and seed-tagged. The Check2HGI and HGI substrates are pre-trained on each state's full check-in graph under self-supervised mutual-information maximisation; the user-disjoint protocol applies to the downstream classifier and the per-fold transition prior, not to the substrate's exposure, so the setup is transductive at the substrate level.
