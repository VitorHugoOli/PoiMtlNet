...
Two ceiling protocols sit side by side. The \textbf{substrate axis} (Table~\ref{tab:substrate}) uses the seed-42 five-fold paired Wilcoxon at the $n=5$ ceiling; multi-seed mean and seed-$\sigma$ are shown for Check2HGI at AL/AZ/FL where multi-seed runs exist, but the paired test stays single-seed. The \textbf{architectural axis} (Tables~\ref{tab:mtl-stl}, \ref{tab:external}) pools across four extra seeds $\{0,1,7,100\}$ on top of seed-42, yielding $n=20$ multi-seed fold-pairs per state; the FL $\Delta_m$ pair additionally carries $n=25$ fold-pairs from a second sweep. Effect sizes are in pp for F1, Acc@$k$, and MRR; $\Delta_m$ is in percent. Code, fold splits, seeds, and per-fold priors are in the anonymous snapshot referenced in \S\ref{sec:method}.
