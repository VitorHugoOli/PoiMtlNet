...
Category metrics: macro-F1 (primary) and Acc@1/Acc@3 (top-$k$ accuracy, secondary, since the 7-way distribution is skewed). Region metrics: Acc@10 (primary), Acc@5 and MRR (mean reciprocal rank, secondary). The joint score $\Delta_m$~\cite{maninis2019attentive,vandenhende2022mtl} averages the per-task relative improvement of MTL over the matched-head STL ceiling (positive $=$ MTL wins); we report it on a primary pair (macro-F1, MRR) and a secondary pair (macro-F1, Acc@10).
