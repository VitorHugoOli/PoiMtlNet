...
For the headline multi-task vs.\ STL contrast we pool across four extra seeds $\{0,1,7,100\}$ on top of seed-42, yielding $n=20$ multi-seed fold-pairs per state. The FL $\Delta_m$ axis additionally carries $n=25$ fold-pairs from a second sweep. Effect sizes are in pp for F1, Acc@$k$, and MRR; $\Delta_m$ is in percent. Code, fold splits, seeds, and per-fold priors are in the anonymous snapshot referenced in \S\ref{sec:method}.
