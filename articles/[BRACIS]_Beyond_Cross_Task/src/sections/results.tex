...
We report results across five Gowalla state splits, organised around two findings: the substrate's effect is asymmetric across the two tasks (\S\ref{subsec:results-substrate}), and joint multi-task training over Check2HGI adds a small category lift while paying the expected region cost (\S\ref{subsec:results-mtl-stl}). Section~\ref{subsec:results-deltam} reports the joint $\Delta_m$ score; Section~\ref{subsec:results-external} situates the model against external baselines.
