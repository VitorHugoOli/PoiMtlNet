...
> - **Five-state architectural-Δ closure (v11):** All five states (AL/AZ/CA/TX/FL) have pooled multi-seed §0.1 evidence (n = 20) and are paper-grade significant on both tasks. The last single-seed asymmetry (FL cat-side §0.1) was closed in v11.
...
> - **AL/AZ/FL/CA/TX cat-Δ Wilcoxon (v11 closure, 2026-05-02):** AL p = **0.036** (small-significantly negative, Δ = −0.78 pp); AZ p **< 1e-04** (significantly positive, Δ = +1.20 pp); FL p = **2e-06** (significantly positive, Δ = **+1.40 pp**); CA p = **2e-06** (Δ = +1.68 pp); TX p = **2e-06** (Δ = +1.89 pp) — all n = 20 multi-seed. Wilcoxon JSONs: `GAP_FILL_WILCOXON.json`, `ARCH_DELTA_WILCOXON.json`, `FL_CAT_DELTA_WILCOXON.json`.

### §0.1 Codex audit log (post-commit `7a60e1c`, accepted 2026-05-01 PM)

> ⚠ **HISTORICAL — DO NOT CITE FOR DRAFTING.** This section is the dated audit-log narrative of what each Codex audit caught and what we fixed at the time. The numbers and source pointers below are correct *as of the dated audit*, not the current canonical. **The current canonical is `RESULTS_TABLE.md §0` (v11; FL §0.1 closed at n=20, all five states paper-grade) and the §0 status block above.** If you need a number, read §0; if you need a source pointer, read §0; this §0.1 is for historical traceability only.
...
|---|---|---|---|---|
| **D0** | Title direction | **Substrate Carries, Architecture Pays: Check-In-Level Embeddings for Multi-Task POI Prediction** (10 words, Pattern D phenomenon-and-method). Macro-audit (2026-05-02) flagged that "Substrate Carries" overclaims relative to the reg side (HGI ≥ Check2HGI by 1.6–3.1 pp under matched-head STL); the title is kept and the body adds an explicit task-scope concession in §1 Beat 3 (the substrate "carries" cat, not reg) so a reviewer reading T2 panel (b) does not catch a slippage. | *Beyond Cross-Task Transfer: A Task-Asymmetric Substrate for Multi-Task POI Category and Region Prediction* (14 words, Pattern A acronym+colon) — preserves "Beyond" hook with task-asymmetry framing; alternate if reviewers prefer the colon-shape | locked-default 2026-05-02 (v4 — title kept + body concession added) |
| **D1** | Abstract reframe | **Locked at 145 words 2026-05-02 (v4 — post-macro-audit trim).** Reordered so the cat-gain leads the MTL sentence ("delivers a category lift at four of five states; the cost is a sign-consistent 7 to 17 pp drop on next-region top-10 accuracy"), preventing a reader's snap-classification of the paper as "MTL doesn't work, with substrate caveats". Carries C1-substrate-asymmetry (cat +14 to +29 pp, p = 0.0312 each; reg HGI ties or marginally exceeds Check2HGI under matched-head ceilings) + C2 classic MTL tradeoff + C1-mechanism (~72 % per-visit at Alabama, single-state). C3 (cross-attn λ = 0 note) NOT in abstract — §6.3 only. | Keep prior 150-word abstract — superseded | locked-default 2026-05-02 (v4) |
...
| **D6** | Anonymous code link | **`https://anonymous.4open.science/r/PoiMtlNet-FE6A/`** (Anonymous GitHub snapshot generated 2026-05-02). Cited in §3.5 (Training protocol) and §Reproducibility footer. | Anonymous Dropbox | locked 2026-05-02 |
...
**Beat 3 — The pivot, and the answer in one sentence (one paragraph).** Both prior works treat the embedding as a *what* axis (which features to encode: graph topology in CBIC; spatial + temporal + categorical sub-encoders in CoUrb). We test an orthogonal *granularity* axis instead: the embedding's emission rate. POI-stable embeddings (HGI, DGI) emit one vector per place — same POI visited twice yields the same vector. Check-in-level contextual embeddings emit one vector per visit, capturing the user's intent shift across two visits to the same café (lunchtime vs. evening). The natural question is whether per-visit context — a different bottleneck axis from per-modality decomposition — finally enables multi-task learning to deliver bidirectional gains. **Our headline answer is task-asymmetric: per-visit context lifts next-category by 14 to 29 percentage points at every state, but on next-region per-place embeddings tie or marginally exceed it under matched-head ceilings — the substrate "carries" the easier task, not both, and the architecture then pays a sign-consistent cost on the harder one.**

**Beat 4 — What we do (one paragraph).** We adopt **Check2HGI**, a check-in-level extension of HGI's contrastive objective: same POI visited twice yields different embeddings because the visit's temporal and co-visit context perturbs the contrastive view. We hold Check2HGI fixed as the substrate and run a controlled comparison along three axes. (i) **Substrate axis:** Check2HGI vs. HGI under matched-head single-task baselines, with a four-head invariance sweep at **AL/AZ** (linear / GRU / single / LSTM) and matched-head replication at **FL/CA/TX**, on Gowalla U.S.-state splits with **FL/CA/TX as the headline scale and AL/AZ as smaller-scale anchors**. (ii) **MTL axis:** a cross-attention multi-task backbone over Check2HGI with per-task input modality (check-in stream → cat head, region stream → reg head), `static_weight(cat = 0.75)`, GRU cat head and STAN-Flow (`next_stan_flow`) reg head. (iii) **Joint axis:** paired Wilcoxon and Δm (Maninis 2019; Vandenhende 2022) against matched-head STL ceilings on both heads. All `next_region` measurements use leak-free per-fold transition priors.

**Beat 4b — Contribution shape (one paragraph; pre-empts the BRACIS novelty pushback).** Our contribution is empirical and methodological, not architectural — we do **not** propose a new MTL balancer, cat head, reg head, or attention variant. The novelty shape, in BRACIS Main Track terms, has three pieces. (i) A **controlled-ablation reattribution** of a common MTL-for-POI reading: with a leak-free five-state protocol and matched-head ceilings, the substrate carries the lift on next-category while the architecture pays a sign-consistent cost on next-region. (ii) A **task-asymmetry mechanism**: a pooled-vs-canonical counterfactual at one state isolates per-visit context (rather than the embedding's training procedure) as the load-bearing axis, explaining *why* the substrate helps cat but does not help reg. (iii) A **transferable methodological pitfall** generalising beyond POI: under cross-attention MTL, loss-side `task_weight = 0` ablations are unsound because the silenced encoder co-adapts via attention K/V; the point is backed by regression tests in the anonymous code, but in the paper it remains a methodological note rather than a headline contribution. The empirical reattribution and the methodological pitfall are the contribution — not Check2HGI itself, which is a derivative substrate, and not the MTL recipe, which is robustness evidence rather than a proposal.

**Beat 5 — Findings (one paragraph, three sentences carrying C1, C1-mechanism, C2).** First, **the substrate is task-asymmetric**: per-visit context (Check2HGI) lifts single-task next-category macro-F1 by **+15.5 pp at AL, +14.5 at AZ, +29.0 at FL, +28.8 at CA, +28.3 at TX** under matched-head STL — paired Wilcoxon p = 0.0312 each (the maximum at n = 5 folds), all 5/5 folds positive, head-invariant at the **AL/AZ ablation scale** across linear / GRU / single / LSTM probes and matched-head replicated at **FL/CA/TX** — while on the harder next-region task POI-stable HGI is **nominally ahead by 1.6 to 3.1 pp** under matched-head STL STAN-Flow, with TOST non-inferiority at δ = 2 pp passing at CA/TX (statistically tied) and δ = 3 pp passing at FL. Second, a pooled-vs-canonical counterfactual at Alabama attributes **~72 % of the cat substrate gap to per-visit context** and ~28 % to Check2HGI's training signal (single-state mechanism evidence) — explaining the asymmetry: per-visit variance is what cat needs; per-POI pooling smooths it away for reg. Third, with Check2HGI fixed as substrate, joint MTL over a cross-attention backbone adds a small additional cat lift at four of five states (AZ +1.20 pp p < 1e-4; FL +1.40 pp p = 2e-06 (n = 20); CA +1.68 pp p = 2e-06; TX +1.89 pp p = 2e-06) while AL is small-significantly negative (Δ = −0.78 pp, p = 0.036 n = 20; magnitude < 2 % relative). On next-region MTL pays a sign-consistent 7 to 17 pp cost on Acc@10 at every state — the textbook MTL tradeoff (cat mostly gains, reg pays); drop-in MTL fixes (FAMO, Aligned-MTL, hierarchical-additive-softmax reg head) do not recover the reg gap, and a methodological side-finding falls out (cross-attention `task_weight = 0` ablations co-adapt via K/V; encoder-frozen isolation is required).

**Beat 6 — Contributions (bulleted, end of intro, two contributions plus a methodological note):**
- **C1.** A controlled five-state ablation showing that check-in-level contextual embeddings (Check2HGI) provide a **task-asymmetric substrate** for joint POI category and region prediction. On next-category, per-visit context lifts matched-head STL macro-F1 by +14.5 to +29.0 pp at every state, paired Wilcoxon p = 0.0312, head-invariant at the AL/AZ ablation scale and matched-head replicated at FL/CA/TX. On next-region, per-place embeddings (HGI) are nominally ahead by 1.6 to 3.1 pp under matched-head STAN-Flow (TOST non-inferiority δ = 2 pp passes at CA/TX). A pooled-vs-canonical counterfactual at AL attributes ~72 % of the cat substrate gap to per-visit context.
- **C2.** A direct measurement of the **classic multi-task tradeoff** under the load-bearing substrate. With Check2HGI fixed, joint MTL over a cross-attention backbone gains on next-category at four of the five states (AZ +1.20 pp, p < 1e-04; FL +1.40 pp, p = 2e-06; CA +1.68 pp, p = 2e-06; TX +1.89 pp, p = 2e-06 — all across n = 20 multi-seed fold-pairs) while AL is small-significantly negative (Δ = −0.78 pp, paired Wilcoxon p = 0.036 across n = 20 multi-seed fold-pairs; magnitude small at < 2 % relative on a 41 % F1 scale). On the harder next-region task it pays a sign-consistent 7 to 17 pp cost on Acc@10 vs. a matched-head STAN-Flow STL ceiling at every one of the five states (paper-grade at p ≤ 1.9e-06 across n = 20). The reg-cost magnitude varies across states (FL smallest at −7.34; TX largest at −16.59 — non-monotone outlier) — we report the broadly-downward pattern descriptively, not as an inferential scaling claim. Drop-in alternatives (FAMO, Aligned-MTL, hierarchical-softmax reg head) do not recover the reg gap.
- **Methodological note.** Under cross-attention MTL, loss-side `task_weight = 0` ablations are unsound — the silenced encoder co-adapts via attention K/V projections; encoder-frozen isolation is the only clean architectural decomposition. The claim is backed by regression tests in the anonymous code. (This note is a §6.3 sub-section, not a top-level contribution; it generalises beyond our study to cross-task attention MTL methodology.)
...
**Land sentence (end of §2):** *"Our work re-examines a default assumption in MTL-for-POI — that joint training transfers signal between heads to lift both tasks. In our cross-attention regime, joint training adds a bounded cat gain at most states but pays a sign-consistent cost on next-region, motivating a reattribution: substrate first, MTL coupling second."*
...
One paragraph. Cat primary: macro-F1; secondary: Acc@1/3. Reg primary: Acc@10; secondary: Acc@5, MRR. Joint: Δm (Maninis 2019; Vandenhende 2022) with primary `Δm-MRR = ½(r_cat F1 + r_reg MRR)` and secondary `Δm-Acc@10 = ½(r_cat F1 + r_reg Acc@10)`. Tests: paired Wilcoxon signed-rank `alternative='greater'` for directional claims; TOST non-inferiority at δ ∈ {2, 3} pp where the claim is "tied". State once: paired Wilcoxon at n = 5 has a maximum significance ceiling of p = 0.0312 (5 / 5 folds positive); pooled multi-seed (5 seeds × 5 folds = 25 fold-pairs at FL) gives sub-1e-7 p-values where available.
...
**Beat 1 — The classic MTL tradeoff (one paragraph; the headline).** *"With Check2HGI fixed as substrate, joint MTL over a cross-attention backbone lifts next-category macro-F1 at four of the five states (AZ +1.20 pp p < 1e-4; FL +1.40 pp p = 2e-06; CA +1.68 pp p = 2e-06; TX +1.89 pp p = 2e-06 — all n = 20) while AL is small-significantly negative (Δ = −0.78 pp, paired Wilcoxon p = 0.036 across n = 20 fold-pairs; magnitude small at < 2 % relative on a 41 % F1 scale). On next-region MTL pays 7 to 17 pp on Acc@10 vs. a matched-head STAN-Flow STL ceiling at every one of the five states (paper-grade at p ≤ 1.9e-06, n = 20) — the textbook MTL tradeoff (Caruana 1997; Vandenhende 2022), easier task mostly gains, harder task pays."* n = 20 paired Δs at all five states (4 seeds × 5 folds, multi-seed pooled paired Wilcoxon). FL also carries auxiliary multi-seed support on Δm and recipe selection (n = 25). Forward to T3. Frame as a tradeoff, not a refutation: the model produces both predictions in one forward pass, and we accept the reg cost in exchange for joint single-model deployment with the cat-side lift at most states.

**Beat 1b — The AL edge case (one paragraph; pre-empts the reviewer "if the tradeoff is universal, why does AL flip?" objection).** AL is the smallest of the five states (~10 K check-ins, ~1.1 K regions), and it is the only state where the cat-side MTL delta turns negative. We do **not** claim a separate mechanistic law from this one cell. The safe reading is narrower: AL is the small-state edge case of the four-of-five-states cat-gain pattern, and the sign flip is consistent with the broader picture that the substrate carries the easier task while MTL coupling does not lift both tasks uniformly. AZ, the next-smallest state, already shows the same pattern in attenuated form (+1.20 pp rather than the +1.7 to +1.9 pp seen at CA/TX). In the paper, the AL result should therefore be framed as an honest exception inside the tradeoff, not as the basis for a new scale-dependent theorem.

**Beat 2 — Magnitude varies non-monotonically (one paragraph; honest scale comment).** AL/AZ as smaller-scale anchors (T3-supp) extend the picture: Δreg is **−11.04** at AL (1.1 K regions, 10 K check-ins) → **−12.27** at AZ (1.5 K, 26 K) → **−7.34** at FL (4.7 K, 127 K) → **−9.50** at CA (8.5 K, 230 K) → **−16.59** at TX (6.5 K, 187 K). The pattern is broadly downward on the AL → AZ → FL trajectory and CA preserves the regime, but **TX breaks monotonicity** — state-specific factors (transition-graph density, per-user trajectory geometry, region-cardinality vs check-in-density coupling) plausibly explain TX's outlier behaviour but are not directly testable at our state count. *"We report this pattern descriptively; the cost varies non-monotonically across states and we leave the scale observation as honest secondary description rather than an inferential claim — quantifying it would require within-state density ablation, deferred to follow-up."*
...
**Beat 2 — Recipe sensitivity (one paragraph).** Cross-state B9-vs-H3-alt comparison (28 paper-closure runs + 8 small-state H3-alt gap-fills + later CA/TX multi-seed closures): B9's three additions over H3-alt (alternating-SGD + cosine + α-no-WD) are paper-grade significant at FL (+3.48 pp Δreg, p = 2.98 × 10⁻⁸ across 25 fold-pairs), at CA (+4.18 pp Δreg, +0.51 pp Δcat, both p < 1e-04 across n=20), and at TX (+1.87 pp Δreg p = 7e-04, +0.52 pp Δcat p = 2e-04, n=20), but hurt cat at small states (AL/AZ Δcat −0.96 to −2.22 pp, both p ≤ 7 × 10⁻⁴ across n=20). Headline: *"the optimal MTL recipe is scale-conditional — B9 paper-grade at FL/CA/TX, H3-alt paper-grade at AL/AZ on cat — but no single recipe in our search closes the MTL-vs-STL gap on next-region."* Source: `RESULTS_TABLE.md §0.4` (v11) + `GAP_FILL_WILCOXON.json`.
...
**Beat 1 — Substrate task-asymmetry, not cross-task transfer (one paragraph).** The five-state evidence collapses into a clean reading: the multi-task picture is dominated by the **substrate's task-asymmetric value**, not by cross-task transfer through joint training. Per-visit context (Check2HGI) is the load-bearing substrate property for next-category prediction (paper-grade significant at every state, +14.5 to +29 pp under matched-head STL); on next-region the same substrate ties or marginally trails per-place embeddings (HGI nominally ahead by 1.6 to 3.1 pp; TOST tied at CA/TX), because pooling to region cardinality smooths the per-visit variance away. The MTL architecture, on top of the substrate, adds a small cat lift at four of five states (AZ +1.20, FL +1.40, CA +1.68, TX +1.89 pp; all paper-grade at p ≤ 2e-06 across n = 20 multi-seed fold-pairs) but is small-significantly negative at AL (Δ = −0.78 pp, p = 0.036, n = 20) and pays a structural reg cost of 7 to 17 pp at every state (paper-grade at p ≤ 1.9e-06, n = 20) — the textbook tradeoff. The conventional MTL framing — *"joint training transfers signal between heads to lift both"* — is in this regime empirically vacuous on the harder task. Honest reattribution: **substrate first (asymmetric), MTL coupling second (tradeoff-paying).**

**Beat 2 — Cost magnitude varies; the scale story is descriptive, not inferential (one paragraph).** The reg cost magnitude varies across states (−7.34 pp at FL → −16.59 pp at TX) but its sign does not. The pattern is broadly downward on the AL → AZ → FL trajectory (−11.04 → −12.27 → −7.34) and CA preserves the regime (−9.50, 8.5 K regions); **TX (−16.59 pp at 6.5 K regions) breaks monotonicity**, suggesting state-specific factors beyond raw class count — per-user trajectory geometry, transition-graph density, short-horizon Markov coverage are plausible drivers but not directly testable at our state count. We report this pattern descriptively rather than as an inferential claim — quantifying scale dependence formally would require within-state density ablation, deferred to follow-up. The recipe-selection finding (B9 FL-tuned, H3-alt small-state) tracks the same axis at the optimiser level: B9's FL-targeted ingredients (alt-SGD, cosine, α-no-WD) help reg at FL by addressing the "reg encoder saturates by epoch 5–6" pattern, but cost cat-side signal at small states that cannot afford the per-step temporal gradient separation.

**Beat 3 — Limitations (one paragraph).** Six honest limitations. (i) **The per-visit-context mechanism (CH19, ~72 % of the cat substrate gap) is single-state evidence, run only at Alabama** via the pooled-vs-canonical counterfactual. The matched-head decomposition is robust in the AL data, but a second-state replication has not been included in the current paper package; the abstract and §6.1 framing therefore explicitly carry the "at Alabama" qualifier and the mechanism claim is scoped accordingly. (ii) FL `next_region` Acc@10 sits in a Markov-saturated regime; we report Acc@5 + MRR alongside to characterise the small-margin band fairly. (iii) **External baseline coverage is asymmetric: ReHDM (Li et al., IJCAI 2025) is reported at AL/AZ/FL but deferred at CA/TX — the dual-level hypergraph's collaborator-pool grows quadratically with region cardinality and exceeded our compute budget at 8.5K and 6.5K regions on a single H100.** (iv) **POI-RGNN reproduction caveat:** our faithful POI-RGNN reproduction uses user-disjoint folds; the published numbers used non-user-disjoint folds, which tends to inflate absolute scores. The +27 to +29 pp matched-head Check2HGI−POI-RGNN gap we report is therefore a conservative lower bound against the published configuration. (v) sklearn-version reproducibility caveat: `StratifiedGroupKFold(shuffle = True)` produces different fold splits across sklearn 1.3.2 → 1.8.0 (PR #32540); paired tests within a single env are unaffected, but absolute leak-magnitude attribution requires single-env re-runs documented in `FINAL_SURVEY.md §8`. (vi) **Sequences use non-overlapping length-9 windows — one (history → next) target per 9 check-ins — which under-supervises relative to standard next-POI practice.** The field segments trajectories non-overlapping but supervises *densely* (prefix-expansion, ≈ one target per position); our one-target-per-window choice trains on ≈ 8× fewer (history → next) pairs, so absolute small-state scores are conservative (a single-state Alabama dense-supervision probe lifts single-task cat by +9.8 and reg by +5.1 pp). Two things bound the impact. First, it does **not** affect the comparative claims: every internal Δ (Check2HGI − HGI, MTL − STL) is measured under identical non-overlapping windows on both arms, apples-to-apples. Second, it does **not** explain away the reg tradeoff — under dense supervision the Alabama MTL → STL reg gap *widens* (8.34 → 12.96 pp) rather than closing, indicating the cost is architectural (the shared backbone cannot absorb the extra supervision) rather than a supervision artifact. The dense-supervision rebuild (transition priors, single-task ceilings, the full MTL board, the §0.1 re-pin, and a multi-state regime re-confirmation) is deferred to a dedicated follow-up study (`docs/future_works/overlapping_windows.md`); the Alabama probe is the single-state, single-seed pre-emption carried here.
