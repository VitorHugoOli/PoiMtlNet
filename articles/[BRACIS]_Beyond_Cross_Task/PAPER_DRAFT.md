...
>
> **Late-session update — CA/TX MTL multi-seed:** in flight on H100 at submission, ETA ~1 h. Acceptance gate: direction-consistent with the seed = 42 anchor (Δ_cat ∈ [+1.6, +2.0] pp; Δ_reg ∈ [−9, −17] pp). If consistent → T3 / T4 cells upgrade to n = 20 pooled paired Δs; if any direction flips, surface as a finding in §6.2. Sub-agent A5 must check before T3 commits.
>
> **Late-session update — ReHDM scope:** ReHDM (Li et al., IJCAI 2025) reported at AL/AZ/FL only; CA/TX deferred — the dual-level hypergraph's collaborator pool scales quadratically with region cardinality and the per-state run cost on a single H100 exceeded the compute budget at 8.5 K (CA) and 6.5 K (TX) regions. Deferred to camera-ready; T5 footnote and §7 Limitations disclose.
...
**Beat 5 — Findings (one paragraph, three sentences carrying C1, C1-mechanism, C2).** First, **the substrate is task-asymmetric**: per-visit context (Check2HGI) lifts single-task next-category macro-F1 by **+14.5 pp at AL, +14.5 at AZ, +29.0 at FL, +28.8 at CA, +28.3 at TX** under matched-head STL — paired Wilcoxon p = 0.0312 each, all 5/5 folds positive at the n = 5 ceiling, head-invariant across linear / GRU / single / LSTM probes — while on the harder next-region task POI-stable HGI is **nominally ahead by 1.6 to 3.1 pp** under matched-head STL STAN-Flow, with TOST non-inferiority at δ = 2 pp passing at CA/TX (statistically tied) and δ = 3 pp passing at FL. Second, a pooled-vs-canonical counterfactual at AL attributes **~72 % of the cat substrate gap to per-visit context** and ~28 % to Check2HGI's training signal — explaining the asymmetry mechanistically (per-visit variance is what cat needs; per-POI pooling smooths it away for reg). Third, with Check2HGI fixed as substrate, joint MTL over a cross-attention backbone gains 0 to +2 pp on cat at every state and pays a sign-consistent 8 to 17 pp cost on next-region — the textbook MTL tradeoff (cat gains, reg pays); drop-in MTL fixes (FAMO, Aligned-MTL, hierarchical-additive-softmax reg head) do not recover the reg gap, and a methodological side-finding falls out (cross-attention `task_weight = 0` ablations co-adapt via K/V; encoder-frozen isolation is required).
...
- **C2.** A direct measurement of the **classic multi-task tradeoff** under the load-bearing substrate. With Check2HGI fixed, joint MTL over a cross-attention backbone gains +1.20 to +2.02 pp on next-category at four of the five states (AZ +1.20, FL +1.43, CA +1.94, TX +2.02) and is ≈ tied at AL (Δ = −0.78 pp, within the multi-seed STL σ of 0.17 pp; paired Wilcoxon pending re-run against the v7 multi-seed STL ceiling). On the harder next-region task it pays a sign-consistent 8 to 17 pp cost vs. a matched-head STAN-Flow STL ceiling at every one of the five states. The reg-cost magnitude varies across states (FL smallest; TX largest non-monotone outlier) — we report the broadly-downward pattern descriptively, not as an inferential scaling claim. Drop-in alternatives (FAMO, Aligned-MTL, hierarchical-softmax reg head) do not recover the reg gap.
...
**Beat 1 — Headline (one paragraph; the substrate task-asymmetry).** *"Under matched-head single-task baselines, the substrate is sharply task-asymmetric. On next-category, check-in-level Check2HGI outperforms POI-stable HGI by **+14.5 pp at AL, +14.5 at AZ, +29.0 at FL, +28.8 at CA, +28.3 at TX** (paired Wilcoxon p = 0.0312 each, all 5/5 folds positive, head-invariant across linear / GRU / single / LSTM probes). On next-region, the same matched-head ablation under STL STAN-Flow shows POI-stable HGI nominally ahead by 1.6 to 3.1 pp; TOST non-inferiority at δ = 2 pp passes at CA/TX (statistically tied) and δ = 3 pp passes at FL (within 3 pp of HGI), but the cat-side picture is the load-bearing finding."* Forward to T2 (two-panel) for the full grid.

**Beat 2 — Head invariance and external comparison (one paragraph).** Head invariance: across four head probes (linear / `next_gru` / `next_single` / `next_lstm`) the cat substrate Δ is uniformly positive at every state — 8 / 8 head-state cells positive at AL+AZ Phase 1, plus the matched-head extension at FL/CA/TX (FINAL_SURVEY §2). External: our STL `next_gru` Check2HGI cat F1 exceeds POI-RGNN's published Gowalla state-level numbers (Capanema et al. 2022: ~31.8 % FL, ~34.5 % CA) by ≥ 28 pp at every matched state — a conservative lower bound (POI-RGNN's published evaluation used non-user-disjoint folds). Forward to T5.
...
**Beat 1 — The classic MTL tradeoff, sign-consistent across states (one paragraph; the headline).** *"With Check2HGI fixed as substrate, joint MTL over a cross-attention backbone gains 0 to +2 pp on next-category macro-F1 at every state and pays 8 to 17 pp on next-region Acc@10 vs. a matched-head STAN-Flow STL ceiling — the textbook MTL tradeoff (Caruana 1997; Vandenhende 2022), easier task gains and the harder task pays."* Per-state cat Δ ranges from **−0.78 pp at AL (≈ tied within the multi-seed STL σ of 0.17 pp)** to **+2.02 pp at TX**; reg Δ from −7.99 at FL to −16.69 at TX. n = 20 paired Δs at AL/AZ (4 seeds × 5 folds, multi-seed pooled paired Wilcoxon at p = 1.9 × 10⁻⁶ on reg); n = 5 single-seed at FL/CA/TX with p = 0.0625 at the n = 5 ceiling, **multi-seed pass in flight at submission to lift this**. Forward to T3. Frame as a tradeoff, not a refutation: the model produces both predictions in one forward pass, and we accept the reg cost in exchange for joint single-model deployment with a small but consistent cat-side lift.

**Beat 2 — Magnitude varies, pattern is broadly downward but non-monotone (one paragraph; honest scale comment).** AL/AZ as smaller-scale anchors (T3-supp) sharpen the picture: Δreg is **−11.04** at AL (1.1 K regions, 10 K check-ins) → **−12.27** at AZ (1.5 K, 26 K) → **−7.99** at FL (4.7 K, 127 K) — the cost shrinks by ~5 pp going from the small-scale anchors to the smallest headline state. CA preserves the regime (−8.92, 8.5 K regions). **TX breaks the pattern at −16.69 pp (6.5 K regions, 187 K check-ins)** — state-specific factors (transition-graph density, per-user trajectory geometry, region-cardinality vs check-in-density coupling) plausibly explain TX's outlier behaviour but are not directly testable at our state count. *"We report this pattern descriptively; the cost varies non-monotonically across states and we leave the scale story as honest secondary observation rather than an inferential claim — quantifying it would require within-state density ablation, deferred to follow-up."*
...
**Beat 1 — Recap (one paragraph, 4–6 lines).** *"We tested whether check-in-level contextual graph embeddings (Check2HGI) carry the multi-task POI prediction win on next-category and next-region across U.S.-state Gowalla splits. The substrate is **task-asymmetric**: per-visit context lifts single-task next-category macro-F1 by +14.5 to +29 pp at every one of five states, paired Wilcoxon p = 0.0312 each, and the lift is head-invariant; on next-region, the same substrate ties or marginally trails per-place embeddings under matched-head single-task ceilings (HGI nominally ahead by 1.6 to 3.1 pp; TOST tied at CA/TX). A pooled-vs-canonical counterfactual at AL attributes ~72 % of the cat substrate gap to per-visit context. With Check2HGI fixed, joint multi-task learning over a cross-attention backbone gains 0 to +2 pp on cat at every state and pays a sign-consistent 8 to 17 pp cost on reg — the textbook MTL tradeoff. The reg cost is robust to drop-in fixes (FAMO, Aligned-MTL, head-capacity scaling); a methodological side-finding generalises beyond our study (cross-attention `task_weight = 0` ablations co-adapt via K/V; encoder-frozen isolation is required)."*
