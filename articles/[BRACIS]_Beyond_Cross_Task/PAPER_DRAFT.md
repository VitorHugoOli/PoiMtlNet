...
>
> **Provenance.** This draft is the BRACIS-2026 article-side reframe of the (now archived) study-side `PAPER_DRAFT.md`. **The sole canonical numerical source for paper tables is `docs/studies/check2hgi/results/RESULTS_TABLE.md §0` (v10, 2026-05-02 — CA/TX §0.1 arch-Δ upgraded to n=20 after the v8/v9 closures).** Background provenance only (do not cite as primary): `docs/studies/check2hgi/archive/post_paper_closure_2026-05-01/PAPER_CLOSURE_RESULTS_2026-05-01.md` (the file was moved to archive in the 2026-05-01 study cleanup) and `CLAIMS_AND_HYPOTHESES.md §CH22 2026-05-01 reframe`. This file supersedes the archived study-side draft for the BRACIS submission.
...
> - **Five-state architectural-Δ closure (v10):** AL/AZ/CA/TX now have pooled multi-seed §0.1 evidence (n = 20) and are paper-grade on both tasks; the earlier CA/TX/TX-open framing is closed. The only remaining asymmetry in the headline package is **FL**: its cat-side §0.1 Wilcoxon remains the single-seed n = 5 paired comparison, while FL also has auxiliary multi-seed support on Δm (n = 25) and recipe selection (n = 25). Disclosed in §7.
...
**Beat 1 — The classic MTL tradeoff (one paragraph; the headline).** *"With Check2HGI fixed as substrate, joint MTL over a cross-attention backbone lifts next-category macro-F1 at four of the five states (AZ +1.20 pp p < 1e-4; FL +1.52 pp p = 0.0625 at n = 5 ceiling; CA +1.68 pp p = 2e-06; TX +1.89 pp p = 2e-06) while AL is small-significantly negative (Δ = −0.78 pp, paired Wilcoxon p = 0.036 across n = 20 fold-pairs; magnitude small at < 2 % relative on a 41 % F1 scale). On next-region MTL pays 8 to 17 pp on Acc@10 vs. a matched-head STAN-Flow STL ceiling at every one of the five states — the textbook MTL tradeoff (Caruana 1997; Vandenhende 2022), easier task mostly gains, harder task pays."* n = 20 paired Δs at AL/AZ/CA/TX (4 seeds × 5 folds, multi-seed pooled paired Wilcoxon; CA/TX p = 2e-06 on both axes); FL cat stays at the n = 5 single-seed ceiling, while FL carries auxiliary multi-seed support on Δm and recipe-selection. Forward to T3. Frame as a tradeoff, not a refutation: the model produces both predictions in one forward pass, and we accept the reg cost in exchange for joint single-model deployment with the cat-side lift at most states.

**Beat 1b — Why AL flips on cat (one paragraph; pre-empts the reviewer "if the tradeoff is universal, why does AL flip?" objection).** AL is the smallest of the five states (~10 K check-ins, ~1.1 K regions). At small data scale the cat task is already substrate-bound: the matched-head STL ceiling at AL is 41.35 ± 0.17 — much lower than at FL/CA/TX (62-67) — because there is less per-visit context to learn from. The MTL coupling cost (gradient interference from the reg head's longer optimisation horizon under static_weight) is paid in the same flat compute budget at every state, but at AL there is no headroom in the cat F1 distribution for the joint training to reclaim. Mechanically, this matches the "cat needs per-visit variance, but MTL siphons cat-specific capacity into the shared backbone" reading: the variance is thin to begin with at AL, so siphoning is net-negative. The flip is therefore consistent with the substrate-task-asymmetry mechanism — it is the *small-state edge of* the four-of-five-states pattern, not a separate phenomenon. Cross-state replication of this small-state-flip prediction is left as an audit item (the natural test would be a smaller US-state cell; AZ at ~1.5 K regions is the next-smallest in our set, and its Δ_cat is only +1.20 pp — directionally consistent with the prediction).

**Beat 2 — Magnitude varies non-monotonically (one paragraph; honest scale comment).** AL/AZ as smaller-scale anchors (T3-supp) extend the picture: Δreg is **−11.04** at AL (1.1 K regions, 10 K check-ins) → **−12.27** at AZ (1.5 K, 26 K) → **−7.99** at FL (4.7 K, 127 K) → **−9.50** at CA (8.5 K, 230 K) → **−16.59** at TX (6.5 K, 187 K). The pattern is broadly downward on the AL → AZ → FL trajectory and CA preserves the regime, but **TX breaks monotonicity** — state-specific factors (transition-graph density, per-user trajectory geometry, region-cardinality vs check-in-density coupling) plausibly explain TX's outlier behaviour but are not directly testable at our state count. *"We report this pattern descriptively; the cost varies non-monotonically across states and we leave the scale observation as honest secondary description rather than an inferential claim — quantifying it would require within-state density ablation, deferred to follow-up."*
