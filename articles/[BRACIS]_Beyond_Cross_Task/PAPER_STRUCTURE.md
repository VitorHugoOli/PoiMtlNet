...
- Audit hub: `docs/baselines/{next_category,next_region}/comparison.md`.
...
- Inline prose sentence — **FAMO** (NeurIPS 2023), **Aligned-MTL** (CVPR 2023), and a **hierarchical-additive softmax reg head** at FL only — none reaches paired-Wilcoxon significance against H3-alt's reg ceiling. The architectural cost is robust to balancer / head-capacity drop-in fixes. Source: `research/F50_T1_RESULTS_SYNTHESIS.md`.
- One sentence on **recipe sensitivity**: H3-alt is universal at small states, B9 is FL-tuned, neither closes the MTL-vs-STL gap on next-region. The recipe-selection finding is itself reported inline or in appendix prose; it does not change the headline.
...
| **T3** | **Headline.** MTL vs STL ceilings on both tasks in a **single merged five-state table**: FL/CA/TX above a `\midrule`, AL/AZ below as smaller-scale anchors. cat F1 (vs STL `next_gru`), reg Acc@10 + MRR (vs STL `next_stan_flow`). | **`RESULTS_TABLE.md §0.1` (v11, 2026-05-02)** | §5.2 |
...
|---|---|---|:-:|
| **T6** | Drop-in MTL ablations at FL: FAMO, Aligned-MTL, HSM-reg-head. **Not a standalone table in the paper; reduced to one inline sentence in §6.2** — see TABLES_FIGURES.md §2.3. | `research/F50_T1_RESULTS_SYNTHESIS.md` (already inline) | (cut to prose) |
...
|---|---|---|---|
| **C1-cat** | Check2HGI > HGI cat F1, matched-head STL, 5 states, paired Wilcoxon p = 0.0312 each. Head-invariant at AL/AZ. | **`RESULTS_TABLE.md §0.3`** (v11); cat STL `next_gru` Δ = +15.50 / +14.52 / +29.02 / +28.81 / +28.34 pp at AL/AZ/FL/CA/TX. | §5.1, T2 panel (a) |
...
| **C2-cat** | MTL vs STL on cat F1: positive at four of five states (AZ +1.20 p < 1e-4 n=20; FL +1.40 p = 2e-06 n=20; CA +1.68 p = 2e-06 n=20; TX +1.89 p = 2e-06 n=20); AL **small-significantly negative** (Δ = −0.78 pp, paired Wilcoxon p = 0.036 across n=20 multi-seed fold-pairs; magnitude < 2 % relative). | **`RESULTS_TABLE.md §0.1` (v11, 2026-05-02)** + `GAP_FILL_WILCOXON.json` + `ARCH_DELTA_WILCOXON.json` + `FL_CAT_DELTA_WILCOXON.json`. | §5.2, T3 |
...
| A5 — Results | `sections/results.tex` | `PAPER_DRAFT.md §5`, **`docs/results/RESULTS_TABLE.md §0` (v11) — sole canonical numerical source**, `FINAL_SURVEY.md` (substrate panel context only) | 4 pp prose + T2 + T3 + T4 + T5 |
...
- Not the live results table. **The sole canonical numerical source for paper tables is `docs/results/RESULTS_TABLE.md §0` (v11).** The earlier `PAPER_CLOSURE_RESULTS_2026-05-01.md` was moved to `docs/archive/check2hgi-post-paper-closure-2026-05-01/` in the 2026-05-01 study cleanup and is background provenance only; numbers there have been superseded by later `RESULTS_TABLE` upgrades, including v8's cat-Δ Wilcoxon, v10's CA/TX §0.1 n=20 closure, and v11's FL §0.1 n=20 closure (the last headline asymmetry).
- Not the claim catalogue. See `docs/CLAIMS_AND_HYPOTHESES.md`.
