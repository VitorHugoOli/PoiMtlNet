...
> **Locked 2026-05-02 v4+v10 (post-Codex audit + v9/v10 closures, per `PAPER_DRAFT.md §0`):** the headline is **substrate task-asymmetry** — per-visit context (Check2HGI) lifts cat by +14.5 to +29 pp at every state under matched-head STL (paired Wilcoxon p = 0.0312 each, head-invariant at the AL/AZ ablation scale and matched-head replicated at FL/CA/TX), while on reg HGI is nominally ahead by 1.6 to 3.1 pp (TOST tied at CA/TX). The **MTL tradeoff** (cat lift at four of five states: AZ +1.20 p < 1e-4, FL +1.52 p = 0.0625 n=5, CA +1.68 p = 2e-06, TX +1.89 p = 2e-06; AL **small-significantly negative** at Δ = −0.78 pp, p = 0.036 n=20; reg loss 8 to 17 pp sign-consistent at every state) is the secondary finding. The reg-cost magnitude varies non-monotonically (TX outlier at −16.59 pp); the scale-progression is reported descriptively, not as an inferential claim. The earlier "Scale-Sensitive" title was demoted after Codex caught (a) stale cat numbers, (b) the +33 pp substrate/MTL-counterfactual conflation, (c) TX breaking scale monotonicity. **Headline tables T3/T4/T5 report FL/CA/TX; AL/AZ live in T3-supp; T2 (two-panel substrate cat+reg) reports all five states.**
...
- T3 — five-state MTL vs STL ceilings on **both** tasks. **Source:** `RESULTS_TABLE.md §0.1` (v10, 2026-05-02) + `GAP_FILL_WILCOXON.json` + `ARCH_DELTA_WILCOXON.json`.
  - cat: Δ ∈ [−0.78, +1.89] pp; positive at four of five states (AZ +1.20 p < 1e-4 / FL +1.52 p = 0.0625 n=5 / CA +1.68 p = 2e-06 / TX +1.89 p = 2e-06), **small-significantly negative at AL** (Δ = −0.78 pp, paired Wilcoxon p = 0.036 across n = 20 multi-seed fold-pairs; magnitude < 2 % relative).
  - reg Acc@10: Δ ∈ [−7.99, −16.59] pp (FL **−7.99** from paired Δ; CA −9.50; TX −16.59; AL −11.04; AZ −12.27), sign-consistent across all five states (always negative).
...
- **Limitations.** (i) FL retains the lone headline asymmetry: cat-side §0.1 still sits at the n = 5 paired ceiling even though AL/AZ/CA/TX are now n = 20 pooled multi-seed and FL has auxiliary multi-seed support on Δm and recipe selection. (ii) ReHDM (Li et al., IJCAI 2025) is reported at AL/AZ/FL only; CA/TX are deferred for compute reasons (dual-level hypergraph collaborator pool scales quadratically with region cardinality). (iii) FL `next_region` Acc@10 sits in a Markov-saturated regime; we report Acc@5 + MRR alongside to honestly characterise the small-margin band. (iv) sklearn-version reproducibility caveat for fold splits — paired tests within an env are unaffected but absolute leak-magnitude footnoting requires single-env re-runs (see `FINAL_SURVEY.md §8`).
...
| **T2** | Substrate ablation, two-panel: cat (Check2HGI vs HGI macro-F1, matched-head STL `next_gru`, four-head invariance grid for AL+AZ) and reg (matched-head STL `next_stan_flow` Acc@10 with TOST). All five states. Paired Wilcoxon p = 0.0312 each cat cell; substrate Δ on cat = +15.5 / +14.5 / +29.0 / +28.8 / +28.3 pp (AL/AZ/FL/CA/TX) — broadly scales with data but not strictly monotone (CA +28.81 < FL +29.02). | `RESULTS_TABLE.md §0.3` (v10); `FINAL_SURVEY.md §1, §2, §4` | §5.1 |
| **T3** | **Headline.** MTL vs STL ceilings on both tasks across **FL/CA/TX**. cat F1 (vs STL `next_gru`), reg Acc@10 + MRR (vs STL `next_stan_flow`). | **`RESULTS_TABLE.md §0.1` (v10, 2026-05-02)** | §5.2 |
| **T3-supp** | **Smaller-scale anchors supplement.** Same metrics for **AL/AZ**, included to show the cost-magnitude pattern across the five states. Δreg = −11.04 / −12.27 pp at AL/AZ vs FL's −7.99, CA's −9.50, TX's −16.59 pp. **Reported descriptively; not an inferential scale claim** — TX breaks monotonicity. | **`RESULTS_TABLE.md §0.1` (v10)** | §5.2 |
| **T4** | Δm joint score (cat F1 + reg MRR) and Δm-Acc@10, paired Wilcoxon. **FL multi-seed (n = 25 fold-pairs) is the strongest joint-score cell; AL/AZ/CA/TX remain at the single-seed ceiling on this axis.** | **`RESULTS_TABLE.md §0.2` (v10, leak-free CH22 2026-05-01)** | §5.2 |
| **T5** | External baselines: cat (POI-RGNN, MHA+PE) and reg (Markov-1-region, STL STAN, ReHDM) **per headline state (FL/CA/TX)**, with our STL ceiling and MTL row. ReHDM at AL/AZ/FL only (CA/TX deferred — limitation in §7). AL/AZ in T5-supp if pages allow. | **`RESULTS_TABLE.md §0.5–0.6` (v10)** + `baselines/next_*/results/<state>.json` | §5.3 |
...
| **C2-cat** | MTL vs STL on cat F1: positive at four of five states (AZ +1.20 p < 1e-4 n=20; FL +1.52 p = 0.0625 n=5 ceiling; CA +1.68 p = 2e-06 n=20; TX +1.89 p = 2e-06 n=20); AL **small-significantly negative** (Δ = −0.78 pp, paired Wilcoxon p = 0.036 across n=20 multi-seed fold-pairs; magnitude < 2 % relative). | **`RESULTS_TABLE.md §0.1` (v10, 2026-05-02)** + `GAP_FILL_WILCOXON.json` + `ARCH_DELTA_WILCOXON.json`. | §5.2, T3 |
| **C2-reg** | MTL < STL reg Acc@10, sign-consistent (≤ 0) at 5 states; AL −11.04, AZ −12.27, FL **−7.99**, CA −9.50, TX −16.59 pp. | **`RESULTS_TABLE.md §0.1`** (v10, paired Δs). | §5.2, T3 |
| **C2-Δm** | Joint Δm-Acc@10 negative at 5 states; Δm-MRR positive at FL multi-seed only (+2.33 %, p = 2.98e-8 across 25 fold-pairs). | **`RESULTS_TABLE.md §0.2`** (v10, leak-free CH22). | §5.2, T4 |
...
| **C2-recipe** | B9 is FL/CA/TX paper-grade; H3-alt is small-state universal (AL/AZ paper-grade). Recipe-selection is scale-conditional. | **`RESULTS_TABLE.md §0.4` (v10)** + `GAP_FILL_WILCOXON.json` (CA/TX n=20). | §6.2 footnote / appendix |
...
| A5 — Results | `sections/results.tex` | `PAPER_DRAFT.md §5`, **`docs/studies/check2hgi/results/RESULTS_TABLE.md §0` (v10) — sole canonical numerical source**, `FINAL_SURVEY.md` (substrate panel context only) | 4 pp prose + T2 + T3 + T4 + T5 |
...
- Not the live results table. **The sole canonical numerical source for paper tables is `docs/studies/check2hgi/results/RESULTS_TABLE.md §0` (v10).** The earlier `PAPER_CLOSURE_RESULTS_2026-05-01.md` was moved to `docs/studies/check2hgi/archive/post_paper_closure_2026-05-01/` in the 2026-05-01 study cleanup and is background provenance only; numbers there have been superseded by later `RESULTS_TABLE` upgrades, including v8's cat-Δ Wilcoxon and v10's CA/TX §0.1 n=20 closure.
