...
5. **Findings.** (i) The substrate is task-asymmetric: per-visit context lifts cat by +15.5 pp at AL, +14.5 at AZ, and +28.3 to +29.0 pp at FL/CA/TX under matched-head STL (paired Wilcoxon p = 0.0312 each — the n = 5 maximum, head-invariant at AL+AZ); on reg HGI is nominally ahead by 1.6 to 3.1 pp under matched-head STL (TOST tied at CA/TX). (ii) MTL on top of Check2HGI adds a small additional cat lift at four of five states (+1.20 to +2.02 pp at AZ/FL/CA/TX; ≈ tied at AL within multi-seed STL noise) and pays an 8 to 17 pp sign-consistent cost on next-region Acc@10 vs. a matched-head STAN-Flow STL ceiling at every state. (iii) Drop-in MTL fixes (FAMO, Aligned-MTL, head-capacity scaling) do not recover the reg gap.
