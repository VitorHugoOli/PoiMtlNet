...
> ⚠ **DEFAULTS & ANTI-STUMBLE (2026-06-19, read [`docs/studies/pre_freeze_gates/DEFAULTS_AND_GUARDS.md`](docs/studies/pre_freeze_gates/DEFAULTS_AND_GUARDS.md)).** The champion **recipe** is now the DEFAULT: a bare `train.py --task mtl` auto-injects **v17** via `--canon` (the `DEFAULT_CANON` constant in `src/configs/canon.py`; this text said v16 until 2026-09-08 — the default moved to v17 on 2026-07-01 and the doc did not follow) — the 6 "silently-wrong-flags" below are handled by the bundle. ⚠ **That bundle pins `--engine check2hgi_design_k_resln_mae_l0_1`, the v14 substrate** (the `_V16` list in `src/configs/canon.py`, whose `--engine` token v17 inherits), which is pre-v18 and carries the leak mechanism (bidirectional consecutive-visit edges + category in the node features). So a bare run selects a leaked substrate **without the user ever typing `--engine`**, and it fires even with no `--task` at all (the pre-parser reads a missing `--task` as `mtl`). No delivered number is affected — every v18 cell and the public reproduction command pass `--canon none` with the recipe written out — but do not read a bare run as reproducing anything. **But four board values are deliberately NOT global defaults** (flipping them silently breaks frozen-§0.1 reproduction): **MIN_SEQUENCE_LENGTH=10**, **stride-1 (overlap)**, **`--compile`**, **`--tf32`** live ONLY in the P3 board recipe/driver, not `core.py`/`canon.py`. `train.py` now emits WARN guards (`_preflight_canon_guards`; `MTL_STRICT=1` hard-fails) for the three silent stumbles: **dev-seed 42** (paper needs `--seed {0,1,7,100}`), **champion-recipe-on-wrong-substrate**, and **torch ≠ 2.11.0+cu128**. Never flip the four board values to global defaults; see the TRAPS list in that doc.
>
