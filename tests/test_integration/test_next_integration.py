"""
Integration test for next-POI training pipeline.
...
class TestNextIntegration:
    """End-to-end next-POI training pipeline on synthetic data."""

    @pytest.fixture(autouse=True)
    def setup(self):
        seed_everything()

    def test_next_pipeline_runs_to_completion(self):
        ...
        for fold_idx in range(INTEGRATION_FOLDS):
            ...
            X_train, y_train = make_next_data(
                NUM_TRAIN // NUM_CLASSES, seed=SEED + fold_idx
            )
            X_val, y_val = make_next_data(
            ...
            for epoch in range(INTEGRATION_EPOCHS):
                ...
                fold_history.log_train("next", loss=loss.item(), accuracy=0.0, f1=0.0)
                fold_history.log_val(
                    "next",
                    loss=0.0,
                    accuracy=0.0,
                    f1=val_f1,
                    model_state=model.state_dict() if epoch == 0 or val_f1 > fold_history.task("next").best.best_value else None,
                    elapsed_time=0.0,
                )
        ...
        for fold in history.folds:
            assert len(fold.task("next").train["loss"]) == INTEGRATION_EPOCHS
            assert len(fold.task("next").val["f1"]) == INTEGRATION_EPOCHS
            assert fold.task("next").best.best_state is not None

    def test_next_model_registry_roundtrip(self):
    ...
    def test_next_training_improves_loss(self):
        """Training should decrease loss over 3 epochs."""
        from models.registry import create_model

        seed_everything()
        X_train, y_train = make_next_data(NUM_TRAIN // NUM_CLASSES)
    ...
    def test_next_sequence_shapes_preserved(self):
        """Verify input/output shapes through full forward pass."""
        from models.registry import create_model
        ...
        X, y = make_next_data(10, seed=SEED)
        assert X.shape == (10 * NUM_CLASSES, SEQ_LEN, EMBED_DIM)
        assert y.shape == (10 * NUM_CLASSES,)

        model.eval()
        with torch.no_grad():
            out = model(X.to(DEVICE))
        assert out.shape == (10 * NUM_CLASSES, NUM_CLASSES)
