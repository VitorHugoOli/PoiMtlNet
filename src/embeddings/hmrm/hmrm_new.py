...
class Optimizer:
    ...
    def _create_user_time_frequency_matrix(self, users_checkins):
        ...
        datetimes = pd.to_datetime(users_checkins["local_datetime"]).tolist()
        total_users = len(users_checkins["userid"].unique())
    ...
    def _create_location_coocurrency_matrix(self, users_checkins):
        ...
        try:
            ...
            users_checkins_sorted = users_checkins.sort_values(by=["local_datetime"])
...
class HmrmBaselineNew:
    ...
    def start(self) -> pd.DataFrame:
        ...
        # Read input file - support both CSV and Parquet formats
        file_size = os.path.getsize(self.input_file)
        file_ext = os.path.splitext(self.input_file)[1].lower()

        # Parquet is more efficient - threshold can be higher (2GB vs 500MB for CSV)
        # Parquet has ~5x better compression and faster I/O than CSV
        if file_ext == '.parquet':
            threshold_mb = 2048  # 2GB for Parquet
            logger.info(f"Reading Parquet file ({file_size / 1024 / 1024:.1f} MB)")

            if file_size > threshold_mb * 1024 * 1024:
                # For very large Parquet files, read in batches
                logger.info(f"Large Parquet file detected, reading in batches")
                parquet_file = pd.read_parquet(self.input_file, engine='pyarrow')
                users_checkin = parquet_file
            else:
                users_checkin = pd.read_parquet(self.input_file)
        else:
            # CSV file handling
            threshold_mb = 500  # 500MB for CSV
            chunksize = 1000000

            if file_size > threshold_mb * 1024 * 1024:
                logger.info(f"Large CSV file detected ({file_size / 1024 / 1024:.1f} MB), reading in chunks")
                chunks = []
                for chunk in tqdm(pd.read_csv(self.input_file, chunksize=chunksize), desc="Reading file chunks"):
                    chunks.append(chunk)
                users_checkin = pd.concat(chunks, ignore_index=True)
            else:
