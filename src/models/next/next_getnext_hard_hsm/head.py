...
@register_model("next_getnext_hard_hsm")
class NextHeadGETNextHardHSM(nn.Module):
    """Hierarchical-additive variant of ``next_getnext_hard``.

    Parameters mirror ``NextHeadGETNextHard`` plus:
    ...
    instantiated identically to ``next_getnext_hard`` so the only
    architectural delta is the parent + child classifier decomposition.
    """
