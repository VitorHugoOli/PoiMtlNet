...
def create_folds(
    ...
    df_next = IoPaths.load_next(state, embedding_engine)
    ...
    df_category = IoPaths.load_category(state, embedding_engine)
    logger.info(f"Category POI data shape: {df_category.shape}")


    # Get unique place IDs
    duplicate_indices = df_category.index[df_category.index.duplicated(keep=False)]
    logger.info(f"Number of unique places: {len(duplicate_indices.unique())}")
