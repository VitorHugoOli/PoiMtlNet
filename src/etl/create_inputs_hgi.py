...
from configs.paths import OUTPUT_DIR, EmbeddingEngine, IoPaths
...
def generate_sequences(
    ...
    Generate non-overlapping sequences of fixed length where:
      - The first (window_size) positions are historical visits (padded at end if needed).
...
def generate_sequences_dataframe(users_ids: List[int], sequences: pd.DataFrame, output_path: str) -> pd.DataFrame:
    """
    Generate a DataFrame of sequences and save to Parquet more efficiently.
...
def generate_next_input(df_embb, df_filter,
    ...
    Args:
        df_embb: DataFrame containing POI embeddings
        df_filter: DataFrame containing check-ins data
        sequences_path: Path to save sequences Parquet
        next_input_path: Path to save next input Parquet
        max_sequences_per_user: Maximum number of sequences to generate per user
    ...
    print(len(embeddings_with_category.columns))
    print(len(padding_embedding))
    ...
    # Generate input data directly without re-reading the Parquet
...
def generate_category_input(df_embb: pd.DataFrame, category_path: str):
    """
    Generate category input file from embeddings using Parquet.

    Args:
        df_embb: DataFrame containing embeddings with category (indexed by placeid)
        category_path: Path to save category input file
    """
    df_embb.to_parquet(category_path)
    print(f"Category input saved to {category_path}")


def create_input(state: str, engine: EmbeddingEngine):
    """
    Process a single state: merge embeddings and generate inputs.
    HGI-specific: merges DGI embeddings with POI encoder localizations.
    ...
    # Load embeddings
    df_embb = pd.read_csv(OUTPUT_DIR / engine.value / state.lower() / 'embeddings.csv')
    df_embb_localization = pd.read_csv(OUTPUT_DIR / engine.value / state.lower() / 'embeddings-poi-encoder.csv')

    print(f"Embeddings shape: {df_embb.shape}, Localization shape: {df_embb_localization.shape}")

    num_cols = [c for c in df_embb.columns if c.isdigit()]

    print(f"Number of columns: {len(num_cols)}")
    ...
    next_input_path = IoPaths.get_next(state, engine)
    category_input_path = IoPaths.get_category(state, engine)
    ...
    generate_category_input(df_embb, str(category_input_path))
    generate_next_input(df_embb_next, df_filter, str(sequences_path), str(next_input_path))
