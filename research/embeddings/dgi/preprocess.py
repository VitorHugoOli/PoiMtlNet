...
from configs.paths import IoPaths, IO_CHECKINS, Resources
import pandas as pd
...
class Preprocess:
    ...
    def _read_boroughs_data(self):
        ...
        with open(str(self.temp_path / 'le_first_level_name_mapping.pkl'), 'wb') as f:
            pkl.dump(le_first_level_name_mapping, f)

        with open(str(self.temp_path / 'le_second_level_name_mapping.pkl'), 'wb') as f:
...
def preprocess_dgi(city, city_shapefile, cta_file=None):
    # PATHS
    temp_folder = IoPaths.DGI.get_temp_dir(city)
    ...
    # OUT
    output_folder = IoPaths.DGI.get_output_dir(city)
    output_folder.mkdir(parents=True, exist_ok=True)
    boroughs_path = Path(cta_file) if cta_file else IoPaths.DGI.get_cta_file(city)
    ...
    pre = Preprocess(str(checkins), str(boroughs_path), temp_folder)
    data = pre.get_data_torch()

    with open(IoPaths.DGI.get_inter_file(city), "wb") as f:
        pkl.dump(data, f)
